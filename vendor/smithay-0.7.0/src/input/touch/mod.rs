...
impl<D: SeatHandler + 'static> TouchInternal<D> {
    ...
    fn cancel(&mut self, data: &mut D, seat: &Seat<D>, seq: Serial) {
        // Cancellation ends the sequence, including touches whose last event
        // was already framed. Frame de-duplication must not suppress it or
        // retain a focus that can receive motion after a session-lock boundary.
        // Draining also discards released slots and pending final frames: no
        // frame may follow cancel for the cancelled sequence.
        for (_, mut state) in self.focus.drain() {
            if let Some((focus, _)) = state.focus.take() {
