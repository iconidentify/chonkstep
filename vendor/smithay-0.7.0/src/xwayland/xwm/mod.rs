...
struct IncomingTransfer {
    ...
    window: X11Window,
    // The source's lifetime is independent of the proxy requestor and of
    // later selection owners. Its destruction must close a waiting reader.
    owner: X11Window,

    incr: bool,
    source_data: incoming_buffer::IncomingBuffer,
    incr_done: bool,
}
...
impl fmt::Debug for IncomingTransfer {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("IncomingTransfer")
            ...
            .field("source_data_len", &self.source_data.len())
...
impl IncomingTransfer {
    ...
    fn write_selection(&mut self, fd: BorrowedFd<'_>) -> std::io::Result<bool> {
        self.source_data.write_with(|bytes| rustix::io::write(fd, bytes).map_err(Into::into))
    }
...
impl OutgoingTransfer {
    fn flush_data(&mut self) -> Result<usize, ReplyOrIdError> {
        ...
        // Retain this bounded staging allocation for the next chunk instead
        // of allocating a new vector and copying its tail on every flush.
        drop(self.source_data.drain(..len));
        self.conn.flush()?;
...
impl X11Wm {
        ...
        D: 'static,
    {
        ...
        handle.insert_source(source, move |event, _, data| match event {
            ...
            calloop::channel::Event::Closed => {
                // FD sources outlive the X event channel unless explicitly
                // removed, including sources disabled for INCR backpressure.
                // Retire them while this generation's XWM is still present;
                // disconnected() may drop it or start a replacement.
                let xwm = data.xwm_state(id);
                for selection in [&mut xwm.clipboard, &mut xwm.primary] {
                    for transfer in selection.incoming.drain(..) {
                        transfer.destroy(&event_handle);
                    }
                    for transfer in selection.outgoing.drain(..) {
                        transfer.destroy(&event_handle);
                    }
                }
        ...
        D: XwmHandler + 'static,
    {
        ...
        let atom = match &*mime_type {
            ...
            x => {
                // TARGETS was delivered through _WL_SELECTION and consumed
                // when the offer was announced. It is not a persistent
                // property named TARGETS. Resolve the MIME atom by name;
                // only_if_exists avoids creating atoms for stale/bogus offers.
                let atom = self
                    .conn
                    .intern_atom(true, x.as_bytes())?
                    .reply_unchecked()?
        ...
        // Owners may be nested or unmapped helper windows, not only managed
        // root children. Preserve the XWM's existing subscriptions while
        // observing this exact producer for the lifetime of the transfer.
        let owner = selection.owner;
        let selected = self.conn.get_window_attributes(owner)?.reply().map_err(ReplyOrIdError::from)?.your_event_mask;
        if !selected.contains(EventMask::STRUCTURE_NOTIFY) {
            self.conn.change_window_attributes(
                owner,
                &ChangeWindowAttributesAux::new().event_mask(selected | EventMask::STRUCTURE_NOTIFY),
            )?;
        }
        ...
        let transfer = IncomingTransfer {
            ...
            owner,
            incr: false,
            source_data: incoming_buffer::IncomingBuffer::default(),
            incr_done: false,
        };
    ...
    D: 'static,
{
    ...
    match event {
        ...
        Event::DestroyNotify(n) => {
            // One requestor can hold both selection kinds. A single destroy
            // event must release every associated pipe and event-loop source.
            for selection in [&mut xwm.clipboard, &mut xwm.primary] {
                // TODO: drain_filter
                let mut i = 0;
                while i < selection.incoming.len() {
                    if selection.incoming[i].window == n.window || selection.incoming[i].owner == n.window {
                ...
                _ => {
                        ...
                        .reply_unchecked()?
                    {
                        ...
                        if type_ == xwm.atoms.INCR {
                            // INCR's initial property is a size hint, not
                            // payload. Deleting it starts the chunk handshake.
                            transfer.incr = true;
                            return Ok(());
                        }
            ...
            if allow_access {
                match n.target {
                    ...
                    target => {
                        ...
                        // A selection requestor need not be a managed/root
                        // child window. Subscribe explicitly so INCR acks and
                        // destruction reach us, preserving any other XWM mask.
                        let selected = conn.get_window_attributes(n.requestor)?.reply()?.your_event_mask;
                        let needed = EventMask::PROPERTY_CHANGE | EventMask::STRUCTURE_NOTIFY;
                        if !selected.contains(needed) {
                            conn.change_window_attributes(
                                n.requestor,
                                &ChangeWindowAttributesAux::new().event_mask(selected | needed),
                            )?;
                        }
                        ...
                        let token = loop_handle.insert_source(
                            ...
                            move |_, fd, data| {
                                ...
                                {
                                    let transfer = &mut selection.outgoing[position];
                                    match read_selection_callback(&xwm.conn, &xwm.atoms, fd.as_fd(), transfer)
                                    {
                                        Ok(OutgoingAction::WaitForReadable) => {
                                            return Ok(PostAction::Continue);
                                        } // transfer ongoing
                                        Ok(OutgoingAction::WaitForProperty) => {
                                            return Ok(PostAction::Disable);
                                        } // consumer backpressure; keep the source token
                                        Ok(OutgoingAction::DoneReading) => {
                                            // INCR still owes its final property
                                            // handshake after source EOF.
                                    ...
                                    let _ = transfer.token.take();
                                    // A non-INCR reply or aborted transfer is
                                    // complete now. Do not retain its staging
                                    // buffer until the requestor window dies.
                                    selection.outgoing.remove(position);
                                }
            ...
            if n.state == Property::NEW_VALUE && n.atom == xwm.atoms._WL_SELECTION {
                if let Some(selection) = if xwm.clipboard.incoming.iter().any(|t| t.window == n.window) {
                    ...
                    if transfer.incr {
                        if let Some(prop) = conn
                            .get_property(
                                // Acknowledge only after the Wayland consumer
                                // drains this chunk. Deleting here *and* in
                                // write_selection_callback lets the owner
                                // overwrite unread chunks or finish early.
                                false,
                                transfer.window,
                                xwm.atoms._WL_SELECTION,
                                AtomEnum::ANY,
            ...
            if n.state == Property::DELETE {
                    ...
                    .any(|t| t.incr && t.request.requestor == n.window && t.request.property == n.atom)
                {
                    ...
                    if transfer.flush_property_on_delete {
                        ...
                        if let Some(token) = transfer.token.as_ref() {
                            // One buffered chunk has been consumed. Resume
                            // the pipe only now, rather than retaining the
                            // entire selection while an X client is stalled.
                            let _ = loop_handle.enable(token);
                        } else {
                        ...
                        .cloned()
                    {
                        ...
                        if let Some(wl_surface) =
                            xwayland_shell::XWaylandShellHandler::xwayland_shell_state(state)
                                .surface_for_xwm_serial(xwm_id, serial)
                        {
                            debug!(
...
fn read_selection_callback(
    ...
    let available = INCR_CHUNK_SIZE.saturating_sub(transfer.source_data.len());
    if available == 0 {
        return Ok(OutgoingAction::WaitForProperty);
    }
    let len = match rustix::io::read(fd, &mut buf[..available]) {
        Ok(len) => len,
    ...
    } else if transfer.source_data.len() >= INCR_CHUNK_SIZE {
        Ok(OutgoingAction::WaitForProperty)
    } else {
        Ok(OutgoingAction::WaitForReadable)
    ...
    match transfer.write_selection(fd) {
        Ok(true) => {
            if transfer.incr {
                ...
                // This callback runs from the destination FD source, outside
                // the X11 event dispatcher that normally flushes requests.
                // The owner cannot send its next chunk until it sees this ack.
                conn.flush()?;
