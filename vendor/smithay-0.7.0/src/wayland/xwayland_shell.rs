...
impl XWaylandShellState {
    ...
    /// Retrieves the live surface for a given serial, if unambiguous.
    ///
    /// Serials belong to one XWayland server, not the compositor. Prefer
    /// [`Self::surface_for_xwm_serial`] when the server is known.
    pub fn surface_for_serial(&self, serial: u64) -> Option<WlSurface> {
        let mut matches = self.by_serial.iter().filter_map(|((_, candidate), surface)| {
            (*candidate == serial && surface.is_alive()).then_some(surface)
        });
        let surface = matches.next()?;
        matches.next().is_none().then(|| surface.clone())
    }

    /// Retrieves a live surface belonging to the specified XWayland server.
    /// A restarted server may reuse serials from its predecessor.
    pub fn surface_for_xwm_serial(&self, xwm: XwmId, serial: u64) -> Option<WlSurface> {
        self.by_serial
            .get(&(xwm, serial))
    ...
    fn request(
        ...
        match request {
            ...
            xwayland_surface_v1::Request::SetSerial { serial_lo, serial_hi } => {
                ...
                if serial == 0 {
                    resource.post_error(xwayland_surface_v1::Error::InvalidSerial, "Serial must be nonzero.");
                    return;
                }
                ...
                compositor::with_states(&data.wl_surface, |states| {
                    ...
                    if cached.current().serial.is_some() {
                        resource.post_error(
                            xwayland_surface_v1::Error::AlreadyAssociated,
                            "Surface association was already committed.",
                        );
                    } else {
                        cached.pending().serial = Some(serial);
                    }
                });
            }
...
fn serial_commit_hook<D: XWaylandShellHandler + XwmHandler + 'static>(
    ...
    if let Some(serial) = compositor::with_states(surface, |states| {
        let mut cached = states.cached_state.get::<XWaylandShellCachedState>();
        // An association is committed once, not refreshed on every frame.
        if cached.current().serial.is_some() {
            None
        } else {
            cached.pending().serial
        }
    }) {
        if let Some(client) = surface.client() {
            ...
            {
                let key = (*xwm_id, serial);
                compositor::add_destruction_hook::<D, _>(surface, move |state, surface| {
                    let by_serial = &mut state.xwayland_shell_state().by_serial;
                    // Do not let late destruction remove a replacement entry.
                    if by_serial.get(&key) == Some(surface) {
                        by_serial.remove(&key);
                    }
                });
