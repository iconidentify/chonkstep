...
/// Returns the MIME types of the seat's current selection, including a
/// compositor-provided bridge offer. This does not request or copy its data.
pub fn current_selection_mime_types<D>(seat: &Seat<D>, target: SelectionTarget) -> Vec<String>
where
    D: SelectionHandler + 'static,
{
    use std::cell::RefCell;
    ...
    selection.map(|source| source.mime_types()).unwrap_or_default()
}
