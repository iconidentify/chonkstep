...
impl<U: Clone + Send + Sync + 'static> SeatData<U> {
    ...
    pub fn device_statistics(&self) -> super::SelectionDeviceStatistics {
        let mut statistics = super::SelectionDeviceStatistics::default();
        for device in &self.known_devices {
            let alive = match device {
            ...
            statistics.dead += usize::from(!alive);
        }
        statistics
    }
