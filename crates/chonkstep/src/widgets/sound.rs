//! Sound instrument: system volume level and mute control.
//! Currently the SDK's dead-screen placeholder face - the sampling and
//! the `wm_theme::soundctl` renderer are being built on the
//! `wm_theme::panel` SDK.
...
impl DockWidget for SoundWidget {
    ...
    fn render(&self, theme: &Theme, tile: u32) -> DecorationBuffer {
        ...
        panel::render_dead_tile(theme, &mut font_system, &mut swash_cache, tile, "SND")
    }
}
