//! Render reviewable BeOS chrome and shell specimens without a display server.
...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: beos_preview NEW_DIRECTORY");
    ...
    for scale in [1.0, 1.25, 1.5, 2.0] {
        ...
        let theme = wm_theme::beos::theme("beos", Appearance::Light)
            .unwrap()
            .scaled(scale);
        ...
        screen.fill(wm_theme::paint::sk_color(wm_theme::beos::DESKTOP));
        for (title, x, y, w, h, focused) in [
            ...
            let lines = if focused {
                vec![
                    ...
                    "$ desktop: BeOS Blue",
                    "",
                    "Short yellow tabs. Crisp beveled frames.",
