//! Deterministic, freely licensed Swiss-compatible text at the source pixel grid.
//! Atlas coverage is antialiased at 1x, then replicated without interpolation.
...
pub(super) fn draw(
    ...
    let y = rect.pos.y + (rect.size.h as i32 - m.px(15) as i32).div_euclid(2 * unit) * unit;
    let mut advance = 0;
