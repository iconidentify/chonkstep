//! Tracker/Deskbar vocabulary applied to ChonkStep's shell surfaces.
use super::{control, empty, fill, text, Metrics};
use crate::{
    beos::{BLUE, INK, MENU_SELECTION, PANEL, WHITE, YELLOW},
    icon, menu,
    model::Color,
...
impl BeosUi {
    ...
    fn panel(&self, image: &mut DecorationBuffer, rect: Rect, sunken: bool) {
        ...
        let (light, dark) = if sunken {
            (Color::rgb(96, 96, 96), WHITE)
        } else {
            (WHITE, Color::rgb(96, 96, 96))
        };
    ...
    pub fn menu(
        ...
        let (line, row, header) = (self.px(1).max(1), self.px(20).max(1), self.px(26));
        ...
        self.folder(&mut image, self.px(8), self.px(6), false);
        self.label_at(
            &mut image,
            title,
            Rect::new(
                Point::new(self.px(29) as i32, line as i32),
                Size::new(w.saturating_sub(self.px(53)), header - line),
            ),
            INK,
            true,
        );
        let close_rect = closable.then(|| {
            Rect::new(
                Point::new(w.saturating_sub(self.px(20)) as i32, self.px(5) as i32),
                Size::new(self.px(14), self.px(14)),
            )
        });
        if let Some(rect) = close_rect {
            control(&mut image, rect, false, false, false, self.m);
        }
        fill(
            &mut image,
            line,
        ...
        fill(&mut image, line, header - line, w - line * 2, line, WHITE);
        ...
        for (index, item) in items.iter().enumerate() {
            ...
            if selected {
                self.paint(&mut image, rect, MENU_SELECTION);
            }
            let color = INK;
            let inset = if item.is_submenu() {
                self.folder(&mut image, self.px(8), y + self.px(3), selected);
                self.px(29)
            } else {
                self.px(12)
            };
            ...
            if item.is_submenu() {
                // R5's hollow, beveled cascade triangle, on the integer grid.
                for n in 0..5 {
                    let c = if selected {
                        WHITE
                    } else {
                    ...
                    fill(
                        &mut image,
                        w - self.px(15) + self.px(n),
                        y + self.px(6 + n),
                        self.px(n + 1) - self.px(n),
                        self.px(7 + n) - self.px(6 + n),
                        c,
                    );
                    fill(
                        &mut image,
    ...
    fn folder(&self, image: &mut DecorationBuffer, x: u32, y: u32, _selected: bool) {
        // Original tiny folder drawing; no platform icon assets are required.
        let rows = [
            "...kkkk........",
            "...kllkkk......",
            "...kllyykkk....",
        ...
        for (dy, row) in rows.iter().enumerate() {
            for (dx, c) in row.bytes().enumerate() {
                let color = match c {
                    b'k' => INK,
                    b'l' => Color::rgb(252, 236, 168),
                    b'y' => Color::rgb(232, 184, 56),
                    _ => continue,
                };
                fill(
                    image,
                    x + self.px(dx as u32),
                    y + self.px(dy as u32),
                    self.px(dx as u32 + 1) - self.px(dx as u32),
                    self.px(dy as u32 + 1) - self.px(dy as u32),
                    color,
                );
            }
        }
    ...
    fn card(
        ...
        self.centered(image, title, tab, INK, true);
