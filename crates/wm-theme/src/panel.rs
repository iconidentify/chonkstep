...
/// The theme's signature LED color: the most vivid bright color among
/// the terminal cursor and ANSI palette. Vividness is saturation times
/// brightness — a theme whose cursor is a plain gray (the flagship's
/// NeXT-gray, say) falls through to its strongest terminal color, while
/// a theme with a signature cursor (Amber Phosphor's `#FFB000`) keeps
/// exactly that. Dark accents get lifted toward white until they can
/// actually glow.
pub fn panel_accent(theme: &Theme) -> Color {
    ...
    let mut best = theme.terminal.cursor;
    for c in &theme.terminal.ansi {
...
pub fn draw_led_matrix(
    ...
    for col in 0..cols {
        ...
        for row in 0..rows {
            let lit = if row < half_rows {
                (half_rows - 1 - row) < top_lit
            } else {
                (row - half_rows) < bottom_lit
            };
            let cx = x as f32 + col as f32 * cell_w + (cell_w - dot) / 2.0;
            let cy = y as f32 + row as f32 * cell_h + (cell_h - dot) / 2.0;
            frect(pixmap, cx, cy, dot, dot, if lit { &ink } else { &ghost });
