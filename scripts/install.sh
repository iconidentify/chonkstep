...
# wpctl, which Omarchy's audio controls use (already
...
# omarchy-export-themes is a release binary beside the session ones. ln -sfn makes a rerun (or
...
cat <<DONE
  - Wayland, nested: for development or a look before you log in, run
    ...
    same chrome, menus, and themes, with X11 apps through
    XWayland.
DONE
...
cat <<DONE
  - HiDPI: set "scale = 2.0" in ${config}
    ...
    backends read it).
  - On PATH, as links into this checkout: chonkrec (record demos with
