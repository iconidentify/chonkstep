...
requested geometry is committed at that density. Rendering, input, and frame
...
for every decoration style. The other suites cover map-time window states and
logical IPC coordinates at different output scales.

Before integration with current `main`, the original patch was also exercised
with the real ChonkBlocker/JBR Vulkan build in a separate visible compositor on
an NVIDIA GPU. The first requested 1500x900 size was honored; 100 corner-drag
samples stayed within the intended range (1498–1615 wide, 898–956 high). The final
999x599 Java frame, root and content aligned with the 1498x898 physical frame,
within fractional rounding, and the captured Endless pause screen was complete.
