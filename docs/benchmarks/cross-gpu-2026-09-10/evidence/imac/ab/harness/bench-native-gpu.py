...
def telemetry_config(device):
    vendor = (Path("/sys/class/drm") / device.name / "device/vendor").read_text().strip()
    if vendor == "0x1002":
...
def transition_modes(modes, width, height, hz):
    ...
    chosen = min(alternate, key=lambda m: ((m["width"], m["height"]) != (width, height),
                 abs(m["refresh"] - 60), -(m["width"] * m["height"])))
...
def stress_session(args, runtime, door, client_env, root):
    ...
    for selected in transition_modes(output["modes"], args.width, args.height, args.hz):
