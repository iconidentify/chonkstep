...
def snapshot(pid):
    ...
    return {"monotonic_ns": time.monotonic_ns(), "cpu_ticks": int(fields[11]) + int(fields[12]),
...
def verify_pixels(path, size, fullscreen, content_rect=None):
...
def stress_session(args, runtime, door, client_env, root):
    ...
    def snapshot_phase(label, fullscreen=True):
        ...
        pixels = verify_pixels(path, [args.width, args.height], fullscreen, rect)
        if label == "popup":
