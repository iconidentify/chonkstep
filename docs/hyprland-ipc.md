...
| `configerrors` | Every retained configuration refusal, one per line or as JSON `{"error": "…"}` objects: each `config.toml` key that was skipped (the same lines `--check-config` prints and counts), each live-Hyprland directive that was refused, and — first, while it lasts — `reload rejected: …` when the last `reload` found a `config.toml` that could not be read at all, in which case that `reload` was not answered `ok` and the working configuration is still in force |
