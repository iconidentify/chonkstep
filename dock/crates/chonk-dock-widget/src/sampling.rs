...
impl DockInput {
    ...
    pub fn local(&self) -> Option<Point> {
        match *self {
            ...
            | DockInput::Release { local, .. }
            | DockInput::Scroll { local, .. } => Some(local),
    ...
    pub fn translated(self, origin: Point) -> DockInput {
        ...
        match self {
            DockInput::Press { local, button } => DockInput::Press {
                local: shift(local),
                button,
            },
            DockInput::Release { local, button } => DockInput::Release {
                local: shift(local),
                button,
            },
            DockInput::Scroll { local, delta } => DockInput::Scroll {
                local: shift(local),
                delta,
            },
            other => other,
...
impl SampleBench {
    ...
    pub fn set_text(&mut self, id: SourceId, contents: &str) {
        self.snapshot[id.0 as usize] = Slot {
            reading: Reading::Text(contents.to_string()),
            unusable: false,
            fresh: true,
        };
    }
    ...
    pub fn set_clock(&mut self, id: SourceId, hms: (u32, u32, u32)) {
        self.snapshot[id.0 as usize] = Slot {
            reading: Reading::Clock(hms.0, hms.1, hms.2),
            unusable: false,
            fresh: true,
        };
    }
...
mod tests {
    ...
    fn accessors_only_answer_for_their_own_source_kind() {
        ...
        let tree = bench.tree(vec![TreeEntry {
            name: "BAT0".into(),
            files: vec![Some("Battery".into())],
            dirs: vec![true],
        }]);
        ...
        assert_eq!(
            samples.tree(tree)[0].file(9),
            None,
            "an out-of-range field is absent, not a panic"
        );
            ...
            Effect::Run {
                ...
                assert_eq!(
                    ...
                    [
                        "move-sink-input",
                        "42",
                        "alsa_output.pci-0000_00_1f.3 [100%]"
                    ]
                );
    ...
    fn a_refused_runtime_value_produces_no_command_at_all() {
        let ssid_shaped_like_an_option = Argv::new("nmcli")
            .word("dev")
            .word("wifi")
            .word("connect")
            .value("--terminate");
        ...
        assert!(
            ssid_shaped_like_an_option.effect(None).is_none(),
            "and it does not run as a shorter command"
        );

        assert!(Argv::new("nmcli")
            .word("connection")
            .word("up")
            .value("")
            .effect(None)
            .is_none());
        assert!(Argv::new("pactl")
        ...
        assert!(Argv::new("pactl")
            .word("set-default-sink")
            .value("x".repeat(Argv::MAX_VALUE + 1))
            .effect(None)
            .is_none());
        assert!(
            Argv::new("pactl")
                .word("set-default-sink")
                .value("x".repeat(Argv::MAX_VALUE))
                .effect(None)
                .is_some(),
