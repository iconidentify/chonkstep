...
mod tests {
    ...
    fn a_frame_starts_transparent_and_exactly_granted_sized() {
        ...
        assert!(
            frame.buffer().pixels.iter().all(|&b| b == 0),
            "unpainted regions must show the empty well through"
        );
    }
    ...
    fn adopt_takes_a_matching_buffer_and_refuses_a_stale_grant() {
        ...
        assert!(
            frame.adopt(good),
            "a renderer's matching buffer replaces the frame"
        );
        assert_eq!(frame.buffer().pixels[0], 0xFF);

        let wrong_size = DecorationBuffer {
            width: 3,
            height: 2,
            pixels: vec![0xAA; 3 * 2 * 4],
        };
        assert!(
            !frame.adopt(wrong_size),
            "a stale-grant render is refused, not rescaled"
        );
        assert_eq!(
            frame.buffer().pixels[0],
            0xFF,
            "and the last good pixels stay"
        );

        let lying_header = DecorationBuffer {
            width: 4,
            height: 2,
            pixels: vec![0xBB; 7],
        };
        assert!(
            !frame.adopt(lying_header),
            "a buffer whose payload does not match its header is refused"
        );
    }
    ...
    fn the_two_run_arities_flatten_to_the_same_ordered_list() {
        ...
        let many = PanelReaction::run_all(vec![
            Effect::Run {
                program: "pactl",
                args: vec!["set-default-sink".into(), "hdmi".into()],
                then: None,
            },
            Effect::Run {
                program: "pactl",
                args: vec!["move-sink-input".into(), "7".into(), "hdmi".into()],
                then: None,
            },
        ])
        .effects();
        ...
        match (&many[0], &many[1]) {
            (Effect::Run { args: first, .. }, Effect::Run { args: second, .. }) => {
                assert_eq!(
                    first[0], "set-default-sink",
                    "the authoritative command is first, as listed"
                );
        ...
        assert!(
            matches!(PanelReaction::run_all(Vec::new()), PanelReaction::None),
            "nothing to run is nothing to do"
        );
