...
impl BluetoothWidget {
    ...
    fn derive(present: bool, bluez: &BluezState) -> BtState {
        ...
        let connected: Vec<&str> = bluez
            .connected()
            .map(|device| device.name.as_str())
            .collect();
        match connected.first() {
            Some(first) => BtState::Connected {
                count: connected.len().min(u8::MAX as usize) as u8,
    ...
    fn power_effect(&self) -> Vec<Effect> {
        ...
        vec![Effect::Run {
            program: "busctl",
            args: set_powered_args(&adapter.path, true),
            then: Some(self.bluez_src),
        }]
    }
}
...
impl DockWidget for BluetoothWidget {
    ...
    fn sources(&self) -> Vec<Source> {
        vec![
            Source::Tree {
                root: PathBuf::from(BT_CLASS_ROOT),
                files: &[],
                dirs: &[],
            ...
            Source::Command {
                program: "busctl",
                args: managed_objects_args(),
                interval: SAMPLE_INTERVAL,
            },
            Source::Tree {
                root: PathBuf::from(RFKILL_ROOT),
                files: RFKILL_FIELDS,
                dirs: &[],
                interval: RFKILL_INTERVAL,
            },
        ]
    }
    ...
    fn update(&mut self, samples: &Samples) -> bool {
        ...
        self.controller = samples
            .tree(self.controllers)
            .iter()
            .find(|entry| entry.name.starts_with("hci"))
    ...
    fn render(
        ...
        let reading = match &self.state {
            ...
            BtState::Connected { count, name } => BtReading::Connected {
                count: *count,
                name,
            },
        };
    ...
    fn render_panel(&mut self, frame: &mut PanelFrame, ctx: &mut PanelCtx<'_>) {
        ...
        let buffer = self
            .panel
            .render(ctx.theme, ctx.tile, width, height, ctx.fonts, ctx.swash);
        frame.adopt(buffer);
    }
...
mod tests {
    ...
    fn a_machine_with_no_adapter_is_absent_and_its_click_is_inert() {
        ...
        assert!(
            widget.on_input(press(), 56).is_empty(),
            "there is no radio here to offer to turn on"
        );
    }
    ...
    fn a_present_adapter_with_a_silent_daemon_is_off_not_absent() {
        ...
        assert!(
            !widget.on_input(press(), 56).is_empty(),
            "a present adapter's click must still try"
        );
    }
    ...
    fn update_folds_the_three_sources_into_one_face() {
        ...
        assert_eq!(
            widget.state,
            BtState::Idle,
            "a powered radio with nothing on it is READY"
        );
    ...
    fn the_connected_count_is_the_connected_devices_not_the_known_ones() {
        ...
        .expect("parses");
        assert_eq!(
            BluetoothWidget::derive(true, &state),
            BtState::Connected {
                count: 2,
                name: "A".into()
            }
    ...
    fn a_soft_block_is_unblocked_rather_than_powered_through() {
        ...
        match &widget.on_input(press(), 56)[..] {
            [Effect::Run {
                ...
                assert_eq!(
                    *program, "rfkill",
                    "never bluetoothctl, and never a power-on into a block"
                );
                assert_eq!(args, &["unblock".to_string(), "bluetooth".to_string()]);
                assert_eq!(
                    *then,
                    Some(bluez),
                    "the reading that decided the direction is what gets resampled"
                );
            }
    ...
    fn an_unblocked_dark_adapter_is_powered_through_busctl() {
        ...
        match &widget.on_input(press(), 56)[..] {
            ...
            other => panic!(
                "expected one busctl set-property, got {} effects",
                other.len()
            ),
        }
    }
