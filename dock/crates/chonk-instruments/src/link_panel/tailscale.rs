...
pub fn parse_status(text: &str) -> Option<TailscaleStatus> {
    ...
    let exit_node = exit.and_then(|e| {
        ...
        addr.or_else(|| e.get("ID").and_then(Json::as_str).map(str::to_string))
            .filter(|s| !s.is_empty())
    });
    let exit_node_online = exit_node.is_some()
        && exit
...
mod tests {
  ...
  "ExitNodeStatus": {{
    ...
    fn an_exit_node_without_an_address_falls_back_to_its_id() {
        let text =
            r#"{"BackendState":"Running","ExitNodeStatus":{"ID":"nSTABLE1CNTRL","Online":true}}"#;
        assert_eq!(
            parse_status(text).unwrap().exit_node.as_deref(),
            Some("nSTABLE1CNTRL")
        );
    }
    ...
    fn status_reads_the_stopped_and_needs_login_states() {
        assert_eq!(
            parse_status(&status_doc("Stopped", false, ""))
                .unwrap()
                .backend,
            BackendState::Stopped
        );
        assert_eq!(
            parse_status(&status_doc("NeedsLogin", false, ""))
                .unwrap()
                .backend,
            BackendState::NeedsLogin
        );
        assert_eq!(
            parse_status(&status_doc("SomethingNew", false, ""))
                .unwrap()
                .backend,
            BackendState::Other
        );
    ...
    fn status_keeps_health_warnings_verbatim() {
        ...
        assert_eq!(
            status.health,
            vec![
                "update available: 1.103.0".to_string(),
                "DNS is broken".to_string()
            ]
        );
    }
        ...
        match classify_toggle_output(output) {
            ToggleVerdict::Denied { hint } => {
                assert!(
                    ...
                    "the hint must carry the remedy: {hint}"
                );
            }
    ...
    fn a_denial_without_the_remedy_line_still_hints_the_grant() {
        match classify_toggle_output("Access denied: some future phrasing\n") {
            ToggleVerdict::Denied { hint } => {
                assert_eq!(hint, "sudo tailscale set --operator=$USER")
            }
            other => panic!("expected a denial, got {other:?}"),
        }
    }
    ...
    fn quiet_or_unrelated_output_is_indeterminate() {
        ...
        assert_eq!(
            classify_toggle_output("Success.\n"),
            ToggleVerdict::Indeterminate
        );
        assert_eq!(
            classify_toggle_output("failed to connect to local tailscaled\n"),
            ToggleVerdict::Indeterminate
        );
    }
}
