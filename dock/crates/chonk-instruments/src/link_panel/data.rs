...
pub(crate) fn split_terse(line: &str) -> Vec<std::borrow::Cow<'_, str>> {
    super::terse::fields(line)
        .map(super::terse::decode)
        .collect()
}
...
pub fn parse_devices(text: &str) -> Vec<NmDevice> {
    ...
    for line in text.lines() {
        ...
        out.push(NmDevice {
            name: super::terse::decode(name).into_owned(),
            kind,
            state: parse_device_state(state_raw),
            connection,
        });
    }
    out
...
pub fn parse_wifi_networks(text: &str) -> Vec<WifiNetwork> {
    ...
    for line in text.lines() {
        ...
        let Ok(signal) = fields[n - 2].trim().parse::<u8>() else {
            continue;
        };
        ...
        match best.iter_mut().find(|b| b.ssid == net.ssid) {
            Some(prev) => {
                if (net.in_use && !prev.in_use)
                    || (net.in_use == prev.in_use && net.signal > prev.signal)
                {
                    *prev = net;
                }
            }
...
pub fn known_profile<'a>(connections: &'a [NmConnection], ssid: &str) -> Option<&'a NmConnection> {
    connections
        .iter()
        .find(|c| c.kind == ConnKind::Wifi && c.name == ssid)
}
...
";
    ...
    fn device_states_parse_by_prefix_with_externally_kept_distinct() {
        ...
        assert_eq!(
            parse_device_state("connected (externally)"),
            DeviceState::ConnectedExternally
        );
        assert_eq!(
            parse_device_state("connecting (getting IP configuration)"),
            DeviceState::Connecting
        );
        assert_eq!(
            parse_device_state("connecting (prepare)"),
            DeviceState::Connecting
        );
        assert_eq!(
            parse_device_state("disconnecting"),
            DeviceState::Disconnecting
        );
        assert_eq!(
            parse_device_state("disconnected"),
    ...
    fn device_garbage_lines_are_skipped_not_fatal() {
        ...
        assert_eq!(
            parse_devices("eno1:ethernet:connected:Name\\: with colon\n")[0].connection,
            "Name: with colon"
        );
    }
...
";
    ...
    fn connections_keep_the_togglable_kinds_in_order() {
        ...
        assert_eq!(
            conns
                .iter()
                .map(|c| (c.name.as_str(), c.kind, c.active))
                .collect::<Vec<_>>(),
            vec![
    ...
    fn connection_names_with_escaped_colons_stay_whole() {
        let conns = parse_connections(
            "Cafe\\: Guest:802-11-wireless:no:12345678-1234-1234-1234-123456789012\n",
        );
    ...
    fn connection_rows_without_a_uuid_or_a_kind_are_dropped() {
        ...
        assert!(
            parse_connections("br0:bridge:yes:11111111-2222-3333-4444-555555555555\n").is_empty()
        );
        assert!(parse_connections(
            "weird:802-3-ethernet:maybe:11111111-2222-3333-4444-555555555555\n"
        )
        .is_empty());
    }
...
";
    ...
    fn wifi_list_dedups_sorts_and_reads_security() {
        ...
        assert_eq!(
            nets.iter()
                .map(|n| (n.ssid.as_str(), n.signal, n.secured, n.in_use))
                .collect::<Vec<_>>(),
            vec![
    ...
    fn an_in_use_bss_wins_its_ssid_even_when_weaker() {
        ...
        assert_eq!(
            nets[0].signal, 40,
            "the associated BSS is the honest reading"
        );
    }
    ...
    fn known_profile_matches_wifi_profiles_by_ssid_name() {
        let conns = parse_connections(CONNECTIONS);
        assert_eq!(
            known_profile(&conns, "HomeBase").map(|c| c.uuid.as_str()),
            Some("11111111-2222-3333-4444-555555555555")
        );
        assert!(known_profile(&conns, "Cafe").is_none());
        assert!(
            known_profile(&conns, "Wired connection 1").is_none(),
            "an ethernet profile is not a wifi credential"
        );
    }
    ...
    fn escaped_device_fields_do_not_shift_the_connection_tail() {
        assert_eq!(
            parse_devices(r"wl\an0:wi\fi:connec\ted:Lab\:5G:Wing"),
            vec![NmDevice {
                name: "wlan0".into(),
                kind: DeviceKind::Wifi,
                state: DeviceState::Connected,
                connection: "Lab:5G:Wing".into(),
            }]
        );
    }
    ...
    fn connection_tail_selection_preserves_escaped_and_raw_name_colons() {
        assert_eq!(
            parse_connections(r"Lab\:5G:North:802-11-wire\less:y\es: id\:value "),
            vec![NmConnection {
                name: "Lab:5G:North".into(),
                kind: ConnKind::Wifi,
                active: true,
                uuid: "id:value".into(),
            }]
