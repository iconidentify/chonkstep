...
impl PanelLayout {
    ...
    pub fn row_at(&self, x: i32, y: i32) -> Option<RowKey> {
        if x < self.glass_x
            || x >= self.glass_x + self.glass_w as i32
            || y < 0
        ...
        self.bands
            .iter()
            .find_map(|(band, band_y, band_h)| match band {
...
impl Ramp {
    fn new(style: &PanelStyle, m: &Metrics) -> Ramp {
        Ramp {
            ...
            tag: PanelFont {
                tracking: 0.0,
                ..style.typeface(TypeRole::Section, m.label_h * 2)
            },
        }
    }
}
...
pub fn panel_layout(view: &PanelView, tile: u32) -> PanelLayout {
    ...
    let content_bottom = bands
        .iter()
        .map(|(_, y, h)| y + *h as i32)
        .max()
        .unwrap_or(0);
...
fn exit_node_field(ts: &TailscaleRow) -> Option<String> {
    ...
    Some(if status.exit_node_online {
        node
    } else {
        format!("{node} · OFFLINE")
    })
...
fn row_enabled(view: &PanelView, key: &RowKey) -> bool {
    match key {
        RowKey::Conn(uuid) => view
            .connections
            .iter()
            .any(|c| &c.uuid == uuid && c.lamp != Lamp::Pending),
        RowKey::Net(ssid) => match &view.wifi {
            WifiSection::Networks(nets) => nets
                .iter()
                .any(|n| &n.ssid == ssid && !n.in_use && !n.pending),
        ...
        RowKey::Tailscale => match &view.tailscale {
            Some(ts) => {
                !ts.pending
                    ...
                    && matches!(
                        ts.status.as_ref().map(|s| s.backend),
                        Some(BackendState::Running) | Some(BackendState::Stopped)
                    )
            }
            None => false,
        },
...
fn render_layout(
    ...
    let mut pixmap =
        tiny_skia::Pixmap::new(width.max(1), height.max(1)).expect("nonzero panel size");
    ...
    let (gx, gy, gw, gh) =
        ip::draw_panel_ground(&mut pixmap, 0, 0, layout.width, layout.height, theme);
    let _ = (gy, gh);
    ...
    for (band, y, h) in &layout.bands {
        ...
        match band {
            ...
            Band::Label(text) => {
                ip::draw_section_header(&mut pixmap, fonts, swash, &style, text, tx, y, tw, h)
            }
            Band::Row(key) => {
            ...
            Band::Empty(legend) => draw_empty_state(
                &mut pixmap,
                fonts,
                swash,
                &style,
...
fn draw_header(
    ...
    let (name, detail) = match header {
        ...
        LinkHeader::Wifi { ssid, signal } => {
            (ssid.to_uppercase(), format!("WIFI · {}%", signal.min(&100)))
        }
        LinkHeader::Wired {
            interface,
            speed_mbps,
...
fn draw_row(
    ...
    match key {
        RowKey::Conn(uuid) => {
            let Some(conn) = view.connections.iter().find(|c| &c.uuid == uuid) else {
                return;
            };
            ip::draw_lamp(
                pixmap,
                x + style.bevel as i32,
                y + ((h - lamp) / 2) as i32,
                lamp,
                style,
                conn.lamp.state(),
            ...
            let tag_x =
                (x + m.cell_inner as i32 - tag_w as i32).max(x + (lamp + style.bevel * 2) as i32);
            ip::draw_type(
                pixmap,
                fonts,
                swash,
        ...
        RowKey::Net(ssid) => {
            ...
            let Some(net) = nets.iter().find(|n| &n.ssid == ssid) else {
                return;
            };
            ...
            ip::draw_meter(
                pixmap,
                x,
                y + ((h - meter_h) / 2) as i32,
                meter_w,
                meter_h,
                style,
                net.signal as f32 / 100.0,
                glow,
            );
            ...
            if net.secured {
                ...
                let ink = if lit {
                    ...
                    style.recede(style.pal.ink_dim)
                };
        ...
        RowKey::Rescan => {
            ...
            let (text, lit) = if view.rescan_cooling {
                ("RESCAN · WAIT", false)
            } else {
                ("RESCAN", true)
            };
...
mod tests {
    ...
    fn rendering_is_byte_stable() {
        ...
        assert_eq!(
            a.pixels, b.pixels,
            "the same view must always produce the same bytes"
        );
    ...
    fn every_interactive_row_is_hit_testable_and_disjoint() {
        ...
        assert_eq!(
            layout.row_at(0, layout.height as i32 / 2),
            None,
    ...
    fn hover_and_press_change_the_bytes_of_that_row_only_when_present() {
        ...
        assert_ne!(
            hover_face.pixels, pressed_face.pixels,
            "press must be visible over hover"
        );
    }
    ...
    fn the_designed_states_are_pairwise_distinct() {
        ...
        if let Some(ts) = &mut v.tailscale {
            ts.operator = OperatorState::NeedsOperator {
                hint: "Use 'sudo tailscale set --operator=chris'".into(),
            };
        }
        variants.push(v);
        let mut v = full_view();
        ...
        variants.push(v);
        let faces: Vec<Vec<u8>> = variants
            .iter()
            .map(|v| render_link_panel(&theme, &mut fs, &mut sc, 56, v).pixels)
            .collect();
    ...
    fn needs_operator_swaps_the_toggle_face_for_the_remedy() {
        ...
        let layout = panel_layout(&view, 56);
        let hint_bands = layout
            .bands
            .iter()
            .filter(|(band, ..)| matches!(band, Band::Hint(_)))
            .count();
    ...
    fn the_wifi_row_cap_holds() {
        let mut view = full_view();
        let many: Vec<NetRow> = (0..20)
            .map(|i| net(&format!("NET{i:02}"), 90 - i, true, false, false))
            .collect();
        ...
        let net_rows = layout
            .bands
            .iter()
            .filter(|(band, ..)| matches!(band, Band::Row(RowKey::Net(_))))
            .count();
    ...
    fn every_theme_renders_a_substantial_glass_in_both_appearances() {
        ...
        for theme in all_themes() {
            ...
            assert!(
                glass > 2000,
                "theme {}: expected a substantial glass area, found {glass}",
                theme.id
            );
        }
    }
    ...
    fn a_grant_that_matches_the_request_renders_exactly_the_natural_face() {
        ...
        let granted = render_link_panel_into(
            &theme,
            &mut fs,
            &mut sc,
            56,
        ...
        assert_eq!(
            natural.pixels, granted.pixels,
            "the usual case must not be a second code path"
        );
    }
    ...
    fn a_grant_is_obeyed_to_the_byte_in_either_direction() {
        ...
        for (w, h) in [
            ...
            (natural.width + 60, natural.height + 90),
            ...
            assert_eq!(
                (buffer.width, buffer.height),
                (w, h),
                "the grant, not the request, sizes the frame"
            );
            assert_eq!(
                buffer.pixels.len(),
                (w * h * 4) as usize,
                "a buffer whose header lies is refused by adopt"
            );
        }
    }
    ...
    fn a_short_grant_clips_the_tail_and_stops_offering_it_to_the_pointer() {
        ...
        assert!(
            !reachable.contains(&RowKey::Rescan),
            "a row past the glass must not answer clicks"
        );
        assert!(!reachable.contains(&RowKey::Tailscale));
        assert!(
            reachable.contains(&RowKey::Conn("uuid-eth".into())),
            "and the rows that did fit still work"
        );
    }
    ...
    fn a_wide_grant_widens_the_hit_zone_with_the_glass() {
        ...
        assert_eq!(
            wide.row_at(natural.width as i32 + 20, y),
            Some(RowKey::Tailscale),
            "the row spans the granted glass"
        );
        assert_eq!(
            natural.row_at(natural.width as i32 + 20, y),
            None,
            "which the natural layout would not have reached"
        );
    }
    ...
    fn an_empty_scan_list_says_so_rather_than_showing_nothing() {
        ...
        assert!(
            layout
                .bands
                .iter()
                .any(|(band, ..)| matches!(band, Band::Empty("NO NETWORKS IN RANGE"))),
        ...
        assert!(
            layout
                .bands
                .iter()
                .any(|(band, ..)| matches!(band, Band::Empty("NO WI-FI HARDWARE"))),
        ...
        assert!(
            layout
                .bands
                .iter()
                .any(|(band, ..)| matches!(band, Band::Label("WI-FI NETWORKS"))),
    ...
    fn the_tailnet_row_never_says_ellipsis() {
        let states = [
            ...
            (
                Some(BackendState::NeedsLogin),
                OperatorState::Unknown,
                false,
            ),
            (
                Some(BackendState::NeedsMachineAuth),
                OperatorState::Unknown,
                false,
            ),
            ...
            (
                ...
                OperatorState::NeedsOperator { hint: "x".into() },
                false,
            ),
        ...
        for (backend, operator, pending) in states {
            ...
            assert!(
                !verdict.contains('\u{2026}'),
                "{backend:?} rendered as an ellipsis"
            );
            assert!(
                verdict.chars().all(|c| c.is_ascii_uppercase()),
                "{backend:?} said {verdict:?}"
            );
        }
    }
    ...
    fn the_exit_node_is_a_labelled_field_or_absent() {
        ...
        assert_eq!(
            exit_node_field(&with(Some("100.64.0.5"), true, BackendState::Running)).as_deref(),
            Some("100.64.0.5")
        );
        assert_eq!(
        ...
        assert_eq!(
            exit_node_field(&with(None, false, BackendState::Running)),
            None,
            "no exit node, no field"
        );
        assert_eq!(
        ...
        let layout = panel_layout(&view, 56);
        assert!(
            layout.bands.iter().any(
                |(band, ..)| matches!(band, Band::Field("EXIT NODE", value) if value == "GATEWAY")
            ),
    ...
    fn the_disabled_ground_tracks_the_action_table() {
        ...
        for key in [
            ...
            assert!(
                row_enabled(&view, &key),
                "{key:?} should be actionable in the full view"
            );
        }
        assert!(
            !row_enabled(&view, &RowKey::Net("HomeBase".into())),
            "the associated network is a fact, not a control"
        );
        assert!(
            !row_enabled(&view, &RowKey::Conn("uuid-vpn".into())),
            "a pending toggle takes no second click"
        );
        ...
        assert_eq!(
            row_state(&hovered, &RowKey::Net("HomeBase".into())),
            RowState::Disabled
        );
        hovered.hover = Some(RowKey::Net("Cafe".into()));
        assert_eq!(
            row_state(&hovered, &RowKey::Net("Cafe".into())),
            RowState::Hover
        );
    }
    ...
    fn the_type_and_ink_ramps_are_ramps() {
        ...
        for tile in [40u32, 56, 112] {
            ...
            let ramp = Ramp::new(&style, &m);
            assert!(
                ramp.title.spec.size > ramp.readout.spec.size,
                "tile {tile}: the header must outrank a readout"
            );
            assert!(
                ramp.readout.spec.size > ramp.row.spec.size,
                "tile {tile}: a verdict outranks a name"
            );
            assert!(
                ramp.row.spec.size > ramp.micro.spec.size,
                "tile {tile}: a name outranks a label"
            );
            assert!(
                ramp.micro.tracking > 0.0,
                "tile {tile}: the label step is the tracked one"
            );
        ...
        for theme in all_themes() {
            ...
            assert!(
                lum(readout) > lum(row),
                "theme {}: a readout burns hotter than a name",
                theme.id
            );
            assert!(
                lum(row) > lum(section),
                "theme {}: a name burns hotter than a label",
                theme.id
            );
            assert!(
                lum(section) > lum(style.recede(section)),
                "theme {}: receding costs current",
                theme.id
            );
        }
    }
    ...
    fn dividers_are_engraved_rather_than_hairlines() {
        ...
        assert_ne!(
            at(2),
            at(3),
            "the score and the highlight are different colours"
        );
        ...
        assert_eq!(
            layout
                .bands
                .iter()
                .filter(|(b, ..)| matches!(b, Band::Label(_)))
                .count(),
            3
        );
    }
    ...
    fn the_glass_starts_where_the_shared_ground_puts_it() {
        for theme in all_themes() {
            assert_eq!(
                ip::ground_inset(&theme),
                GLASS_INSET,
                "theme {} moved the glass out from under the hit test",
                theme.id
            );
        }
    }
...
mod preview {
    ...
    fn blit(dst: &mut tiny_skia::Pixmap, x: i32, y: i32, src: &DecorationBuffer) {
        let Some(src) = tiny_skia::PixmapRef::from_bytes(&src.pixels, src.width, src.height) else {
            return;
        };
        dst.draw_pixmap(
            x,
    ...
    fn dock_column(
        ...
        fonts: &mut FontSystem,
        swash: &mut SwashCache,
    ) -> Vec<DecorationBuffer> {
        let history: Vec<u32> = (0..nettraffic::NET_TRAFFIC_COLUMNS as u32)
            .map(|i| (i * 7) % 5)
            .collect();
        let up_history: Vec<u32> = (0..nettraffic::NET_TRAFFIC_COLUMNS as u32)
            .map(|i| (i * 3) % 4)
            .collect();
        vec![
            ...
            nettraffic::render_nettraffic_tile(
                ...
                &nettraffic::TrafficLane {
                    readout: nettraffic::RateReadout {
                        digits: [Some(8), Some(1), Some(5)],
                        unit: nettraffic::RateUnit::Kilo,
                    },
                    now: 3,
                    history: &history,
                },
                &nettraffic::TrafficLane {
                    readout: nettraffic::RateReadout {
                        digits: [None, Some(8), Some(4)],
                        unit: nettraffic::RateUnit::Mega,
                    },
                    now: 2,
                    history: &up_history,
                },
            ),
            sysload::render_sysload_tile(
                theme,
                fonts,
                swash,
                tile(),
                &[2, 3, 5, 4, 6, 3, 2, 4, 7, 5, 3, 2],
                6,
                false,
            ),
            soundctl::render_soundctl_tile(theme, fonts, swash, tile(), 0.62, false),
            wifi::render_wifi_tile(
                theme,
                fonts,
                swash,
                tile(),
                &wifi::LinkReading::Wifi {
                    ssid: "HomeBase",
                    signal_pct: 87,
                },
            ),
    ...
    fn plate(
        ...
        let y = (LINK_SLOT * tile() as i32)
            .min(h as i32 - panel.height as i32)
            .max(0);
    ...
    fn locked_view() -> PanelView {
        ...
        if let Some(ts) = &mut view.tailscale {
            ts.operator = OperatorState::NeedsOperator {
                hint: "sudo tailscale set --operator=chris".into(),
            };
    ...
    fn preview_plates() {
        ...
        for id in ids {
            for appearance in [Appearance::Dark, Appearance::Light] {
                let Some(theme) = theme_variant(id, appearance) else {
                    continue;
                };
                for (name, view) in [
                    ("wifi", wifi_view()),
                    ("thismachine", this_machine_view()),
                    ("locked", locked_view()),
                ] {
