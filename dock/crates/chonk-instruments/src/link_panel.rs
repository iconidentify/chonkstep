...
use data::{
    parse_connections, parse_devices, parse_wifi_networks, DeviceKind, DeviceState, NmConnection,
    NmDevice, WifiNetwork,
};
use render::{
    panel_layout, render_link_panel_into, ConnRow, Lamp, LinkHeader, NetRow, PanelView, RowKey,
    TailscaleRow, WifiSection,
};
use tailscale::{
    classify_toggle_output, parse_status, BackendState, OperatorState, TailscaleStatus,
    ToggleVerdict,
};
...
impl LinkPanel {
    ...
    pub fn update(&mut self, samples: &Samples) -> bool {
        ...
        if samples.unusable(self.connections_src) {
            self.pending
                .retain(|p| !matches!(p.key, RowKey::Conn(_) | RowKey::Net(_)));
        }
        ...
        for (bit, id) in [
            (1, self.devices_src),
            (2, self.connections_src),
            (4, self.wifi_src),
        ] {
            if samples.fresh(id) {
                self.fresh_panel =
                    (self.fresh_panel & !bit) | if samples.text(id).is_some() { bit } else { 0 };
            }
        }
        ...
        self.wifi_hw =
            self.devices.iter().any(|d| d.kind == DeviceKind::Wifi) || !self.networks.is_empty();
    ...
    fn reconcile_connections(&mut self) {
        ...
        self.pending.retain_mut(|p| {
            let RowKey::Conn(uuid) = &p.key else {
                return true;
            };
            let confirmed = connections
                .iter()
                .any(|c| &c.uuid == uuid && c.active == p.want_on);
    ...
    fn reconcile_networks(&mut self) {
        ...
        self.pending.retain_mut(|p| {
            let RowKey::Net(ssid) = &p.key else {
                return true;
            };
            if networks
                .iter()
                .any(|n| &n.ssid == ssid && n.in_use == p.want_on)
            {
                return false;
            }
            p.samples_left = p.samples_left.saturating_sub(1);
    ...
    fn reconcile_tailscale(&mut self) {
        ...
        if proven {
            ...
            self.operator = OperatorState::NeedsOperator {
                hint: "sudo tailscale set --operator=$USER".to_string(),
            };
        }
    }
    ...
    pub fn view(&self) -> PanelView {
        let header = if self.header == LinkHeader::Unknown {
            self.derive_header()
        } else {
            self.header.clone()
        };
            ...
            .map(|c| ConnRow {
                ...
                external: c.active
                    && self.devices.iter().any(|d| {
                        d.connection == c.name && d.state == DeviceState::ConnectedExternally
                    }),
            })
            .collect();
    ...
    fn derive_header(&self) -> LinkHeader {
        if let Some(net) = self.networks.iter().find(|n| n.in_use) {
            return LinkHeader::Wifi {
                ssid: net.ssid.clone(),
                signal: net.signal,
            };
        }
        if let Some(dev) = self.devices.iter().find(|d| {
            d.kind == DeviceKind::Ethernet
                && matches!(
                    d.state,
                    DeviceState::Connected | DeviceState::ConnectedExternally
                )
        }) {
            return LinkHeader::Wired {
                interface: dev.name.clone(),
                speed_mbps: None,
            };
        }
        LinkHeader::Unknown
    ...
    fn push_pending(&mut self, key: RowKey, want_on: bool) {
        self.pending.push(PendingToggle {
            key,
            want_on,
            samples_left: PENDING_DEADLINE_SAMPLES,
        });
    }
}
...
";
 ...
 :OpenMesh:52:--
";
    const TS_RUNNING: &str =
        r#"{"BackendState":"Running","Self":{"Online":true},"Health":[],"Peer":{}}"#;
    const TS_STOPPED: &str =
        r#"{"BackendState":"Stopped","Self":{"Online":false},"Health":[],"Peer":{}}"#;
    ...
    fn closing_preserves_bounded_reconciliation_and_source_failure_retires_it() {
        ...
        assert!(!click(&mut r.panel, &RowKey::Conn("uuid-wg".into()))
            .effects()
            .is_empty());
        r.panel.visibility_changed(false);
        assert!(
            r.panel.needs_sampling(),
            "closing does not discard an accepted toggle"
        );
        ...
        assert!(
            !r.panel.needs_sampling(),
            "unconfirmed toggles eventually stop closed-panel polling"
        );
        ...
        assert!(
            !r.panel.needs_sampling(),
            "no grace sample will ever arrive from a stopped source"
        );
    }
    ...
    fn reopening_requires_only_the_relevant_fresh_sources_for_each_control() {
        ...
        assert!(
            click(&mut r.panel, &RowKey::Net("HomeBase".into()))
                .effects()
                .is_empty(),
        ...
        r.panel.update(&r.bench.samples());
        assert!(!click(&mut r.panel, &RowKey::Net("HomeBase".into()))
            .effects()
            .is_empty());
    }
    ...
    fn reopening_does_not_authorize_cached_tailscale_state_from_empty_or_malformed_success() {
        for text in [
            ...
            r.panel.update(&r.bench.samples());
            assert_eq!(
                r.panel.view().tailscale.unwrap().status,
                cached,
                "cached pixels stay visible"
            );
            assert!(click(&mut r.panel, &RowKey::Tailscale).effects().is_empty(),
                "invalid fresh status must not execute the cached Running state's down action: {text:?}");
            assert!(
                !r.panel.needs_sampling(),
                "a declined click creates no reconciliation work"
            );
            ...
            assert_eq!(
                args,
                ["up"],
                "recovery uses the new status, not cached Running"
            );
    ...
    fn malformed_status_also_revokes_authority_after_a_valid_opening_sample() {
        ...
        assert!(click(&mut r.panel, &RowKey::Tailscale).effects().is_empty());
        assert_eq!(
            r.panel.view().tailscale.unwrap().status.unwrap().backend,
            BackendState::Running
        );
    }
    ...
    fn the_fold_produces_the_full_view() {
        ...
        assert_eq!(
            ...
            "derived header from the live ethernet device"
        );
        ...
        let WifiSection::Networks(nets) = &view.wifi else {
            panic!("wifi hardware is present")
        };
    ...
    fn an_active_connection_click_is_connection_down_by_uuid() {
        ...
        assert!(
            matches!(
                click(&mut r.panel, &RowKey::Conn("uuid-eth".into())),
                PanelReaction::Repaint
            ),
            "pending row must not fire twice"
        );
        // The confirming sample is the truth.
        r.bench.set_text(
            r.connections,
            "Wired connection 1:802-3-ethernet:no:uuid-eth\n",
        );
    ...
    fn a_pending_toggle_expires_back_to_reality() {
        ...
        assert_eq!(
            r.panel.view().connections[2].lamp,
            Lamp::Off,
            "an unconfirmed toggle must stop claiming BUSY"
        );
    }
    ...
    fn an_unknown_secured_network_spawns_the_join_dialog() {
        ...
        assert_eq!(
            then,
            Some(r.wifi),
            "when the dialog exits, the list catches up immediately"
        );
        assert!(
            !r.panel.is_pending(&RowKey::Net("Cafe".into())),
            "the dialog owns the attempt"
        );
    }
    ...
    fn the_in_use_network_row_is_a_fact_not_a_control() {
        let mut r = rig();
        r.bench
            .set_text(r.wifi, "*:HomeBase:74:WPA2\n :Cafe:61:WPA2\n");
        r.panel.update(&r.bench.samples());
        assert!(matches!(
            click(&mut r.panel, &RowKey::Net("HomeBase".into())),
            PanelReaction::Repaint
    ...
    fn an_unconfirmed_tailscale_toggle_locks_the_row_with_the_remedy() {
        ...
        let ts = r.panel.view().tailscale.unwrap();
        match &ts.operator {
            OperatorState::NeedsOperator { hint } => assert!(
                hint.contains("--operator="),
                "the remedy must be shown: {hint}"
            ),
            other => {
                panic!("an unconfirmed toggle against an unproven grant must lock, got {other:?}")
            }
        }
        assert!(!ts.pending);
    ...
    fn the_observed_output_hook_classifies_a_denial_exactly() {
        ...
        assert!(r
            .panel
            .command_finished(ObservedRun::TailscaleToggle, denied));
        let ts = r.panel.view().tailscale.unwrap();
        assert!(
            matches!(&ts.operator, OperatorState::NeedsOperator { hint } if hint.contains("--operator="))
        );
        assert!(!ts.pending, "a denied toggle is not in flight");
        assert!(
            !r.panel.command_finished(ObservedRun::TailscaleToggle, ""),
            "quiet output decides nothing"
        );
    }
    ...
    fn rescan_fires_once_then_cools_down_by_samples() {
        ...
        assert!(
            matches!(click(&mut r.panel, &RowKey::Rescan), PanelReaction::Repaint),
            "the cooldown gates the radio"
        );
        ...
        assert!(!r.panel.view().rescan_cooling);
        assert!(matches!(
            click(&mut r.panel, &RowKey::Rescan),
            PanelReaction::Run(_)
        ));
    }
    ...
    fn hover_follows_motion_and_a_cross_row_release_fires_nothing() {
        ...
        assert!(matches!(
            r.panel.on_event(PanelEvent::Motion { local: at }, 56),
            PanelReaction::Repaint
        ));
        ...
        let reaction = r
            .panel
            .on_event(PanelEvent::LeftRelease { local: elsewhere }, 56);
        assert!(
            matches!(reaction, PanelReaction::Repaint),
            "press on one row, release on another is a change of mind"
        );
        assert!(!r.panel.view().rescan_cooling, "rescan must not have fired");
        assert!(
            !r.panel.is_pending(&RowKey::Tailscale),
            "and neither did the pressed row"
        );

        assert!(matches!(
            r.panel.on_event(PanelEvent::Leave, 56),
            PanelReaction::Repaint
        ));
    ...
    fn no_wifi_hardware_is_a_designed_state() {
        ...
        for y in 0..layout.height as i32 {
            assert_ne!(
                layout.row_at(layout.width as i32 / 2, y),
                Some(RowKey::Rescan),
                "no radio, no rescan row"
            );
        }
    }
    ...
    fn the_panel_answers_clicks_where_it_actually_drew_them() {
        ...
        let buffer = r
            .panel
            .render(&theme, 56, &mut fonts, &mut swash, short.0, short.1);
        assert_eq!(
            (buffer.width, buffer.height),
            short,
            "the grant sizes the frame"
        );
        ...
        assert!(
            rescan_y >= short.1 as i32,
            "the fixture must actually clip the rescan row"
        );
        let at = Point::new(natural.width as i32 / 2, rescan_y);
        r.panel.on_event(PanelEvent::LeftPress { local: at }, 56);
        let reaction = r.panel.on_event(PanelEvent::LeftRelease { local: at }, 56);
        assert!(
            matches!(reaction, PanelReaction::None),
            "a click outside the drawn glass is not a command"
        );
        assert!(
            !r.panel.view().rescan_cooling,
            "and the radio was not asked to scan"
        );
    }
    ...
    fn the_tile_header_wins_over_the_derived_one() {
        let mut r = rig();
        assert!(r.panel.set_link_header(LinkHeader::Wired {
            interface: "eno1".into(),
        ...
        assert!(
            !r.panel.set_link_header(LinkHeader::Wired {
                interface: "eno1".into(),
                speed_mbps: Some(1000)
            }),
