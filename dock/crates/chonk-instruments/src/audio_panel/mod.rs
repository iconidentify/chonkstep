    ...
    inputs
        ...
        .filter(|input| {
            input
                .app_name
                .as_deref()
                .is_some_and(|app| app != EASYEFFECTS)
        })
...
impl PanelMetrics {
    ...
    pub fn request(tile: u32, rows: usize) -> (u32, u32) {
        ...
        (
            m.width,
            m.chrome_h() + rows * m.row_h + (rows - 1) * m.gap + m.pad + INSET as u32,
        )
    }
    ...
    pub fn granted(tile: u32, width: u32, height: u32, rows: usize) -> PanelMetrics {
        ...
        let stack =
            height.saturating_sub(base.chrome_h() + base.pad + INSET as u32 + (n - 1) * base.gap);
        PanelMetrics {
            width,
            height,
            row_h: base.row_h.min(stack / n).max(MIN_ROW_H),
            ..base
        }
    }
    ...
    pub fn row_at(&self, point: Point, rows: usize) -> Option<usize> {
        ...
        (within < self.row_h as i32 && (row as usize) < self.visible_rows(rows))
...
impl AudioPanel {
    ...
    fn target_at(&self, point: Point, metrics: &PanelMetrics) -> Option<PanelTarget> {
        ...
        let zone = if point.x >= metrics.mute_zone_left() {
            PanelZone::Mute
        } else {
            PanelZone::Row
        };
    ...
    pub fn on_scroll(
        ...
        (next != current).then_some(PanelAction::SetVolume {
            sink: target.sink,
            percent: next,
        })
    }
        ...
        let action = match pressed.zone {
            ...
            PanelZone::Row => {
                self.prediction = Some(Prediction {
                    name: pressed.sink.clone(),
                    samples_left: PREDICTION_SAMPLES,
                });
