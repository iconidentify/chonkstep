...
fn parses_the_real_sink_capture_field_for_field() {
    ...
    assert!(
        !sinks[2].available,
        "a sink whose every port is 'not available' greys out"
    );
}
...
fn sink_edge_cases_degrade_per_field_not_per_document() {
    ...
    assert!(
        sinks[0].available,
        "no ports means nothing can be unplugged"
    );
    assert_eq!(sinks[0].volume_percent, None);
    assert_eq!(
        sinks[0].description, "null.sink",
        "blank description falls back to the name"
    );
...
fn damaged_sink_documents_are_no_reading() {
    for bad in ["", "not json", "{\"a\": 1}", "[{\"name\": \"x\""] {
        assert_eq!(
            parse_sinks(bad),
            None,
            "{bad:?} must not produce a sink list"
        );
    }
}
...
fn percent_strings_parse_the_pactl_way() {
    ...
    assert_eq!(
        parse_percent("40.6%"),
        Some(41),
        "a future decimal rounds instead of blanking"
    );
...
fn default_sink_is_one_bare_name_line() {
    assert_eq!(
        parse_default_sink("alsa_output.foo.analog-stereo\n"),
        Some("alsa_output.foo.analog-stereo".to_string())
    );
    assert_eq!(parse_default_sink(""), None);
    assert_eq!(parse_default_sink("   \n"), None);
    assert_eq!(
        parse_default_sink("No default sink set.\n"),
        None,
        "an error sentence is not a name"
    );
}
...
fn sink_inputs_parse_and_the_move_filter_spares_the_dsp_chain() {
    ...
    assert_eq!(
        inputs[1].app_name, None,
        "a filter-chain stream carries no application.name"
    );
    assert_eq!(
        eligible_moves(&inputs),
        vec![19288],
        "only the real application moves"
    );
    assert!(
        parse_sink_inputs("garbage").is_empty(),
        "damage degrades to no moves, not no switch"
    );
}
...
fn a_switch_is_set_default_plus_migrations_all_by_name() {
    ...
    let effects = action_effects(
        &PanelAction::SwitchTo(HDMI.to_string()),
        &inputs,
        Some(confirm),
    );
    ...
    match &effects[0] {
        Effect::Run {
            ...
            assert_eq!(
                *then,
                Some(confirm),
                "the default sampler confirms the switch"
            );
        }
...
fn the_request_scales_with_the_tile() {
    ...
    let m = m56();
    assert_eq!(
        PanelMetrics::request(56, 3),
        (392, 3 + 4 + m.header_h + 2 + 3 * 32 + 2 * 2 + 4 + 3)
    );
    assert_eq!(
        PanelMetrics::request(56, 0),
        PanelMetrics::request(56, 1),
        "the empty panel keeps one row of face"
    );
...
fn a_granted_panel_hit_tests_where_it_draws() {
    ...
    assert_eq!(
        m.row_at(Point::new(10, top + 32), 3),
        None,
        "the groove belongs to nobody"
    );
    assert_eq!(m.row_at(Point::new(10, top + 34), 3), Some(1));
    assert_eq!(
        m.row_at(Point::new(10, top - 1), 3),
        None,
        "the header is not a row"
    );
    assert_eq!(
        m.row_at(Point::new(10, m.height as i32 - 1), 3),
        None,
        "past the last row is nothing"
    );
    assert_eq!(
        m.row_at(Point::new(0, top + 10), 3),
        None,
        "the gasket is not the panel"
    );
    assert_eq!(m.row_at(Point::new(392, top + 10), 3), None);
    assert_eq!(
        m.mute_zone_left(),
        392 - 3 - 4 - 32,
        "the mute key sits a pad in from the glass edge"
    );
}
...
fn a_clamped_grant_is_obeyed_rather_than_overdrawn() {
    ...
    assert_eq!(
        narrow.mute_zone_left(),
        240 - 3 - 4 - 32,
        "the mute key follows the right edge in"
    );
    assert_eq!(narrow.row_at(Point::new(236, top + 10), 3), Some(0));
    assert_eq!(
        narrow.row_at(Point::new(240, top + 10), 3),
        None,
        "past the grant is not the panel"
    );
    ...
    assert_eq!(
        short.visible_rows(3),
        3,
        "and every device survives the clamp"
    );
    assert!(
    ...
    assert_eq!(
        tiny.row_h, MIN_ROW_H,
        "compression stops at the legibility floor"
    );
    assert_eq!(tiny.visible_rows(3), 2);
    assert_eq!(tiny.row_at(Point::new(10, tiny.row_top(0) + 2), 3), Some(0));
    assert_eq!(
        tiny.row_at(Point::new(10, tiny.row_top(2) + 2), 3),
        None,
        "a row that is not drawn is not a target"
    );
    ...
    assert_eq!(
        sliver.row_at(Point::new(40, top + 10), 3),
        Some(0),
        "and the body is still the switch target"
    );

    let hairline = PanelMetrics::granted(56, 60, natural_h, 3);
    assert!(
        hairline.mute_zone_left() >= hairline.width as i32,
        "narrower still, the mute key is gone too"
    );
}
...
fn folding_reports_repaints_only_for_visible_change() {
    ...
    assert!(
        !panel.fold_default(Some(VOLT.to_string())),
        "the same lamp is not a repaint"
    );
    assert!(
        !panel.fold_inputs(parse_sink_inputs(SINK_INPUTS_JSON)),
        "streams are invisible"
    );
...
fn press_then_release_on_the_same_row_switches_and_predicts() {
    ...
    assert_eq!(
        panel.shown_default(),
        Some(HDMI),
        "the lamp moves before pactl answers — the prediction"
    );
}
...
fn unavailable_rows_are_inert_scenery() {
    let mut panel = fixture_panel();
    assert!(
        !panel.on_press(on_row(2), &m56()),
        "row 2's port is physically absent"
    );
...
fn the_wheel_over_a_row_steps_that_devices_level() {
    ...
    assert_eq!(
        panel.on_scroll(on_row(0), 0, &m56()),
        None,
        "no travel asks for nothing"
    );
...
fn the_wheel_stops_at_the_ceiling_and_the_floor() {
    ...
    assert_eq!(
        ...
        None,
        "the knob does not climb past full scale"
    );
    assert_eq!(
        panel.on_scroll(on_row(1), -1, &m56()),
        Some(PanelAction::SetVolume {
...
fn a_press_survives_the_list_reordering_under_it() {
    ...
    assert_eq!(
        action, None,
        "row 0 is a different device now — the press must not fire on it"
    );
}
...
fn an_unconfirmed_prediction_falls_back_to_the_sampled_truth() {
    ...
    for i in 1..PREDICTION_SAMPLES {
        assert!(
            !panel.fold_default(Some(VOLT.to_string())),
            "reading {i}: still believing the click"
        );
        assert_eq!(panel.shown_default(), Some(HDMI));
    }
    assert!(
        panel.fold_default(Some(VOLT.to_string())),
        "the expiry is a visible fall-back"
    );
    assert_eq!(
        panel.shown_default(),
        Some(VOLT),
        "the sample is the truth; the lamp was a prediction"
    );
}
...
fn a_clamped_grant_is_filled_floor_to_floor() {
    ...
    for (w, h) in [(240, 72), (336, 40), (400, 300), (336, 108)] {
        ...
        assert!(
            buf.pixels.as_chunks::<4>().0.iter().all(|px| px[3] == 255),
            "grant {w}x{h}: no hole under a clamped panel"
        );
    }
}
...
fn the_default_lamp_follows_the_shown_default() {
    ...
    assert_ne!(
        volt_default, hdmi_default,
        "the lamp must move with the default"
    );
}
...
fn the_empty_panel_is_a_face_not_a_hole() {
    ...
    for theme in all_themes() {
        ...
        assert!(
            buf.pixels.as_chunks::<4>().0.iter().all(|px| px[3] == 255),
            "theme {}: fully painted, fully opaque",
            theme.id
        );
    }
}
...
fn every_theme_renders_the_populated_panel_opaquely() {
    ...
    for theme in all_themes() {
        ...
        assert!(
            buf.pixels.as_chunks::<4>().0.iter().all(|px| px[3] == 255),
            "theme {}: the panel ground must be opaque",
            theme.id
        );
    }
}
