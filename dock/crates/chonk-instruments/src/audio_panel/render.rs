...
fn draw_row(
    ...
    let readout_w =
        ip::type_width(fonts, &readout, "100%").max(ip::type_width(fonts, &readout, &readout_text));
    ...
    ip::draw_meter(
        pixmap, meter_x, meter_y, meter_w, meter_h, style, level, glow,
    );
    ...
    let name = style
        ...
        .colored(style.ink_for(TypeRole::Row, row_state));
