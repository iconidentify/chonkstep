...
mod tests {
    ...
    fn unescapes_the_pactl_format_field() {
        ...
        assert_eq!(
            doc.as_str(),
            Some(r#"format.sample_format = "\"float32le\"""#)
        );
    }
    ...
    fn a_lone_surrogate_degrades_to_replacement_not_rejection() {
        ...
        assert_eq!(
            Json::parse(r#""x\ud800Ay""#).unwrap().as_str(),
            Some("x\u{FFFD}Ay"),
            "a lone high surrogate costs only itself"
        );
        assert_eq!(
            Json::parse(r#""x\ud800\u0041y""#).unwrap().as_str(),
            Some("x\u{FFFD}y"),
            "a mismatched escape pair eats both units"
        );
    }
    ...
    fn as_u64_takes_integers_only() {
        ...
        assert_eq!(
            Json::parse("4294967295").unwrap().as_u64(),
            Some(4294967295)
        );
