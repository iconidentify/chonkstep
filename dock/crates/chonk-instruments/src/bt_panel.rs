...
impl BtPanel {
    ...
    fn status(&self) -> BtStatus {
        ...
        let connected = self
            ...
            .count();
        BtStatus::On {
            connected: connected.min(u8::MAX as usize) as u8,
        }
    }
        ...
        if !matches!(status, BtStatus::NoRadio | BtStatus::NoDaemon) {
            let mut push = |device: &bluez::Device| {
                let armed = self
                    .armed
                    .as_ref()
                    .is_some_and(|(path, _)| path == &device.path);
                devices.push(DeviceRow::from_device(
                    device,
                    self.is_pending(&device.path),
                    armed,
                ));
            };
        ...
        match event {
            ...
            PanelEvent::LeftRelease { local } => {
                ...
                let fired = self
                    .pressed
                    .take()
                    .filter(|key| Some(key) == target.as_ref());
                let Some(key) = fired else {
                    ...
                    return if had_highlight || disarmed {
                        PanelReaction::Repaint
                    } else {
                        PanelReaction::None
                    };
        ...
        let reaction = match key {
            ...
            BtRowKey::Forget(path) => {
                ...
                let confirmed = armed.as_ref().is_some_and(|(armed_path, at)| {
                    armed_path == path
                        && now.is_none_or(|now| now.duration_since(*at) < FORGET_GRACE)
                });
                if confirmed {
                    self.dirty = true;
    ...
    fn toggle_device(&mut self, path: &str, connected: bool) -> PanelReaction {
        ...
        self.pending.push(Pending {
            path: path.to_string(),
            want_connected: !connected,
            budget: PENDING_DEADLINE_SAMPLES,
        });
        self.dirty = true;
        PanelReaction::Run(Effect::Run {
            program: "busctl",
