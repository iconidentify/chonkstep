...
use chonk_dock_widget::{
    DockInput, DockWidget, Effect, Samples, Source, SourceId, SAMPLE_INTERVAL,
};
...
impl DockWidget for NetTrafficWidget {
    ...
    fn update(&mut self, samples: &Samples) -> bool {
        ...
        let elapsed = self
            .last_fold
            .replace(Instant::now())
            .map_or(SAMPLE_INTERVAL, |at| at.elapsed())
            .as_secs_f32();
        ...
        for (name, rx_total, tx_total) in &readings {
            if let Some(existing) = self.interfaces.iter_mut().find(|i| &i.name == name) {
                if let Some((prev_rx, prev_tx)) =
                    existing.last_totals.replace((*rx_total, *tx_total))
                {
                    ...
                    existing.peak_bps = (existing.peak_bps * 0.98)
                        .max(rx_bps)
                        .max(tx_bps)
                        .max(1024.0);
                    ...
                    existing
                        .rx_history
                        .push_back((rx_bps / existing.peak_bps).clamp(0.0, 1.0));
                    existing.tx_history.pop_front();
                    existing
                        .tx_history
                        .push_back((tx_bps / existing.peak_bps).clamp(0.0, 1.0));
                }
            } else {
        ...
        self.interfaces
            .retain(|i| readings.iter().any(|(name, _, _)| name == &i.name));
    ...
    fn render(
        ...
        let quantize = |history: &VecDeque<f32>| -> Vec<u32> {
            let skip = history
                .len()
                .saturating_sub(nettraffic::NET_TRAFFIC_COLUMNS);
            history
                .iter()
                .skip(skip)
                .map(|&v| nettraffic::quantize_level(v, nettraffic::NET_TRAFFIC_HALF_ROWS))
                .collect()
        };
        ...
        let down = TrafficLane {
            ...
            now: nettraffic::quantize_level(
                iface.rx_bps / iface.peak_bps,
                nettraffic::NET_TRAFFIC_HALF_ROWS,
            ),
            history: &rx_history,
        };
        let up = TrafficLane {
...
mod tests {
    ...
    fn per_interface_totals_keep_each_interface_separate_and_skip_loopback() {
        ...
        assert_eq!(
            readings,
            vec![
                ("eth0".to_string(), 1000, 200),
                ("wlan0".to_string(), 500, 100)
            ]
        );
    }
    ...
    fn update_differences_two_readings_into_a_rate() {
        ...
        assert_eq!(
            widget.interfaces[0].rx_bps, 0.0,
            "the first reading is a baseline, not a rate"
        );
        ...
        assert!(
            (eth.rx_bps / eth.tx_bps - 4.0).abs() < 1e-3,
            "4096 bytes down against 1024 up is 4:1"
        );
    ...
    fn interfaces_that_disappear_are_dropped_and_the_selection_survives_it() {
        ...
        assert_eq!(
            widget.on_input(press(), 56).len(),
            1,
            "a press over two interfaces cycles and repaints"
        );
    ...
    fn a_stale_pass_folds_nothing_and_an_empty_reading_clears_the_tile() {
        ...
        assert!(
            ...
            "a dead screen has no interfaces to cycle"
        );
    }
}
