...
use wm_theme::sysload::{
    render_sysload_tile, SYS_LOAD_COLUMNS, SYS_LOAD_MEM_SEGMENTS, SYS_LOAD_ROWS,
};
use wm_theme::Theme;
...
impl DockWidget for SysLoadWidget {
    ...
    fn update(&mut self, samples: &Samples) -> bool {
        ...
        if samples.fresh(self.meminfo) {
            ...
            let fraction = samples
                .text(self.meminfo)
                .and_then(parse_mem_used_fraction)
                .unwrap_or(0.0);
    ...
    fn render(
        ...
        let levels: Vec<u32> = self
            .cpu_history
            .iter()
            .map(|f| quantize_level(*f, SYS_LOAD_ROWS))
            .collect();
...
mod tests {
    ...
    fn mem_fraction_needs_both_keys_and_a_nonzero_total() {
        ...
        assert_eq!(
            parse_mem_used_fraction("MemTotal: 0 kB\nMemAvailable: 0 kB\n"),
            None
        );
    }
    ...
    fn update_folds_two_proc_readings_into_a_column_and_a_bar() {
        ...
        assert_eq!(
            widget.cpu_history.back().copied(),
            Some(0.0),
            "the first sample has no delta to report"
        );
    ...
    fn a_pass_with_no_fresh_reading_changes_nothing() {
        ...
        assert_eq!(
            widget.cpu_history, history,
            "a stale pass must not shift the graph"
        );
    }
    ...
    fn an_absent_proc_reading_costs_one_column_and_keeps_the_baseline() {
        ...
        widget.update(&bench.samples());
        assert_eq!(
            widget.mem_used_frac, 0.0,
            "no meminfo is an unlit bar, not a guess"
        );
        ...
        assert_eq!(
            widget.prev_cpu,
            Some((1000, 5000)),
            "a garbled read must not become the new baseline"
        );
    }
}
