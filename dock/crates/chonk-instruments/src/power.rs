...
fn summarize(supplies: &[SupplyReading]) -> PowerFace {
    ...
    if batteries.is_empty() {
        ...
        return if supplies.iter().any(|s| s.kind == SupplyKind::Mains) {
            PowerFace::AcOnly
        } else {
            PowerFace::NoInfo
        };
    ...
    let known: Vec<u32> = batteries
        .iter()
        .filter_map(|s| s.capacity.map(u32::from))
        .collect();
    let capacity = if known.is_empty() {
        None
    } else {
    ...
    let ac_online = supplies
        .iter()
        .any(|s| s.kind == SupplyKind::Mains && s.online != Some(false));
...
impl DockWidget for PowerWidget {
    ...
    fn update(&mut self, samples: &Samples) -> bool {
        ...
        let supplies: Vec<SupplyReading> = samples
            .tree(self.supplies)
            .iter()
            .map(reading_from)
            .collect();
...
mod tests {
    ...
    fn summarize_maps_the_ordinary_laptop_states() {
        let chg = summarize(&[
            battery(Some(60), Some(BatteryStatus::Charging)),
            mains(Some(true)),
        ]);
        assert_eq!(
            chg,
        ...
        let bat = summarize(&[
            battery(Some(80), Some(BatteryStatus::Discharging)),
            mains(Some(false)),
        ]);
        assert_eq!(
            bat,
        ...
        let full = summarize(&[
            battery(Some(100), Some(BatteryStatus::Full)),
            mains(Some(true)),
        ]);
        assert_eq!(
            full,
        ...
        let held = summarize(&[
            battery(Some(80), Some(BatteryStatus::NotCharging)),
            mains(Some(true)),
        ]);
        assert_eq!(
            held,
    ...
    fn summarize_falls_back_to_the_ac_line_when_statuses_are_silent() {
        let on_ac = summarize(&[
            battery(Some(50), Some(BatteryStatus::Unknown)),
            mains(Some(true)),
        ]);
        assert_eq!(
            on_ac,
    ...
    fn summarize_gives_desktops_the_ac_face_and_nothing_the_dead_face() {
        ...
        assert_eq!(
            summarize(&[parse_supply(None, None, None, None)]),
            PowerFace::NoInfo
        );
    }
    ...
    fn only_a_changed_face_repaints() {
        let mut bench = SampleBench::new();
        let id = bench.tree(vec![supply(
            "BAT0",
            [Some("Battery"), Some("50"), Some("Discharging"), None],
        )]);
        let mut widget = PowerWidget::new();
        widget.bind(&[id]);
        assert!(widget.update(&bench.samples()));
        ...
        assert!(
            !widget.update(&bench.samples()),
            "a fresh reading that says the same thing is not a change"
        );
    }
