...
fn parse_unicode_escape(bytes: &[u8], at: &mut usize) -> Option<char> {
    ...
    let mut probe = *at + 2;
    let Some(low) = hex4(bytes, &mut probe) else {
        return Some('\u{FFFD}');
    };
...
fn parse_number(bytes: &[u8], at: &mut usize) -> Option<Json> {
    ...
    std::str::from_utf8(bytes.get(start..*at)?)
        .ok()?
        .parse::<f64>()
        .ok()
        .map(Json::Num)
...
mod tests {
    ...
    fn scalars_and_keywords_parse() {
        ...
        assert_eq!(
            Json::parse("\"hi\"").and_then(|v| v.as_str().map(str::to_string)),
            Some("hi".to_string())
        );
    }
    ...
    fn a_busctl_scalar_reply_parses() {
        ...
        assert_eq!(
            value.get("data").and_then(Json::as_str),
            Some("261.2-1-arch")
        );
    }
    ...
    fn a_busctl_property_bag_parses() {
        ...
        assert_eq!(
            bag.entries().len(),
            3,
            "insertion order is preserved and nothing is dropped"
        );
    }
    ...
    fn escapes_including_surrogate_pairs_decode() {
        ...
        assert_eq!(
            Json::parse(r#""tab\there""#).unwrap().as_str(),
            Some("tab\there")
        );
    ...
    fn multibyte_utf8_survives_unescaped() {
        assert_eq!(
            Json::parse("\"Björn's Küche\"").unwrap().as_str(),
            Some("Björn's Küche")
        );
    ...
    fn depth_is_capped_rather_than_overflowing_the_stack() {
        ...
        assert_eq!(
            Json::parse(&deep),
            None,
            "a pathological nest is refused, not a crash"
        );
    }
