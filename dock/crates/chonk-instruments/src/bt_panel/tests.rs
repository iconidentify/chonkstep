...
fn center(panel: &BtPanel, key: &BtRowKey) -> Point {
    let (x, y, w, h) = layout(panel)
        .row_rect(key)
        .unwrap_or_else(|| panic!("no band for {key:?}"));
...
fn a_machine_with_no_radio_is_a_plate_with_no_controls_on_it() {
    ...
    assert!(
        ...
        "no adapter answered, so there is nothing it knows"
    );

    let layout = layout(&panel);
    assert_eq!(
        layout.row_rect(&BtRowKey::Power),
        None,
        "there is no radio to offer to power"
    );
    assert_eq!(
        layout.row_rect(&BtRowKey::PairNew),
        None,
        "and nothing to pair it with"
    );
...
fn the_no_radio_plate_is_panel_sized_not_a_sliver() {
    ...
    assert_eq!(
        spec.width,
        render::panel_width(112),
        "the width is the family's four tiles"
    );
    assert!(
    ...
    assert!(
        spec.height * 3 > spec.width,
        "a panel this wide and this short is the sliver again"
    );
}
...
fn the_three_absences_are_three_different_faces() {
    ...
    assert!(
        layout(&panel).row_rect(&BtRowKey::Power).is_none(),
        "there is no adapter object to power"
    );
    ...
    assert!(
        layout(&panel).row_rect(&BtRowKey::Power).is_some(),
        "this one is a button"
    );

    assert_ne!(
        no_radio.height, no_daemon.height,
        "two absences that lay out identically read identically"
    );
}
...
fn the_devices_are_connected_first_then_merely_known() {
    ...
    assert_eq!(
        names,
        ["WH-1000XM4", "MX Keys"],
        "connected first, then paired-idle; the unpaired ghost is neither"
    );
    ...
    assert!(
        power_y < first_device && first_device < pair_y,
        "power, then the devices, then the way to add one"
    );
}
...
fn the_spec_grows_with_the_devices() {
    ...
    assert_eq!(
        three.spec(TILE).width,
        empty.spec(TILE).width,
        "the width is the family's, not the content's"
    );
}
...
fn a_long_device_list_is_capped_and_says_so() {
    let many: Vec<(&str, bool, bool)> = (0..render::MAX_DEVICE_ROWS + 3)
        .map(|_| ("Thing", false, true))
        .collect();
    let (panel, _) = fixture(true, &many, clear());
    assert_eq!(
        panel.view().devices.len(),
        many.len(),
        "the view carries them all"
    );
    let layout = layout(&panel);
    assert!(layout
        .row_rect(&device_key(render::MAX_DEVICE_ROWS - 1))
        .is_some());
    assert!(
        layout
            .row_rect(&device_key(render::MAX_DEVICE_ROWS))
            .is_none(),
        "past the cap is drawn nowhere"
    );
}
...
fn the_power_row_follows_the_rfkill_order() {
    ...
    assert_eq!(
        ...
        (
            "rfkill",
            &["unblock".to_string(), "bluetooth".to_string()][..]
        )
    );
    ...
    assert!(
        args.contains(&"/org/bluez/hci0".to_string()),
        "the sampled adapter path is the runtime argument"
    );
    ...
    assert_eq!(
        ...
        (
            "rfkill",
            &["block".to_string(), "bluetooth".to_string()][..]
        )
    );
}
...
fn a_hard_block_offers_nothing_because_nothing_would_work() {
    ...
    assert!(
        matches!(click(&mut panel, &BtRowKey::Power), PanelReaction::Repaint),
        "the press highlight still clears"
    );
}
...
fn clicking_a_known_device_connects_it_and_a_connected_one_disconnects() {
    let (mut panel, _) = fixture(
        true,
        &[("WH-1000XM4", true, true), ("MX Keys", false, true)],
        clear(),
    );
...
fn the_only_runtime_argument_is_an_object_path_in_one_argv_element() {
    ...
    assert!(
        args.contains(&device_path(0)),
        "the path rides whole: {args:?}"
    );
    assert!(
        args.iter().all(|arg| !arg.contains(' ')),
        "no argv element is a sentence: {args:?}"
    );
}
...
fn device_rows_are_inert_while_the_radio_is_off() {
    ...
    assert_eq!(
        view.devices.len(),
        1,
        "they are still listed — the panel knows about them"
    );

    let layout = layout(&panel);
    let (x, y, w, h) = layout.row_rect(&device_key(0)).expect("the band exists");
    assert_eq!(
        layout.row_at(x + w as i32 / 2, y + h as i32 / 2),
        None,
        "but the pointer is not offered it"
    );
    assert_eq!(
        layout.row_rect(&forget_key(0)),
        None,
        "and there is no forget key to hit either"
    );
...
fn a_pending_that_outlives_its_deadline_goes_back_to_showing_reality() {
    ...
    assert!(
        !panel.view().devices[0].pending,
        "the deadline expired and the row shows reality"
    );
}
...
fn a_stale_pass_spends_no_deadline() {
    ...
    assert!(
        panel.view().devices[0].pending,
        "no fresh reading arrived, so nothing was spent"
    );
}
...
fn clicking_a_pending_device_again_does_nothing() {
    ...
    click(&mut panel, &device_key(0));
    assert!(
        matches!(click(&mut panel, &device_key(0)), PanelReaction::Repaint),
        "no second Connect mid-negotiation"
    );
}
...
fn forgetting_takes_two_clicks_on_the_same_key() {
    ...
    assert!(
        matches!(click(&mut panel, &forget_key(0)), PanelReaction::Repaint),
        "the first click only arms"
    );
    assert!(
        panel.view().devices[0].armed,
        "and the row says so on its face"
    );
    ...
    assert!(
        args.contains(&device_path(0)),
        "the device is the argument, the adapter is the target"
    );
...
fn an_arming_expires_on_its_own() {
    ...
    assert!(
        panel.tick(now + FORGET_GRACE),
        "the disarm is a visible change"
    );
    assert!(!panel.view().devices[0].armed);
}
...
fn anything_else_disarms_the_confirm() {
    ...
    assert!(
        !panel.view().devices[0].armed,
        "a click elsewhere is not a confirm"
    );
}
...
fn arming_one_key_disarms_another() {
    let (mut panel, _) = fixture(
        true,
        &[("MX Keys", false, true), ("Buds", false, true)],
        clear(),
    );
    panel.tick(Instant::now());
    click(&mut panel, &forget_key(0));
...
fn a_press_that_changes_its_mind_fires_nothing() {
    ...
    assert!(matches!(
        panel.on_event(PanelEvent::LeftRelease { local: elsewhere }, TILE),
        PanelReaction::Repaint
    ));
    assert!(!panel.view().devices[0].armed, "nothing was armed");
    assert_eq!(
        panel.view().status,
        BtStatus::On { connected: 0 },
        "and the power row did not fire either"
    );
}
...
fn the_forget_key_and_the_row_body_are_different_clicks() {
    ...
    assert_eq!(
        bx + bw as i32,
        fx,
        "the body ends exactly where the key begins"
    );
    assert_eq!(
        layout.row_at(bx + bw as i32 - 1, by + bh as i32 / 2),
        Some(device_key(0))
    );
    assert_eq!(
        layout.row_at(fx + fw as i32 / 2, by + bh as i32 / 2),
        Some(forget_key(0))
    );
}
...
fn hover_follows_motion_and_costs_one_repaint_per_change() {
    ...
    assert!(matches!(
        panel.on_event(PanelEvent::Motion { local: at }, TILE),
        PanelReaction::Repaint
    ));
    ...
    assert!(matches!(
        panel.on_event(PanelEvent::Leave, TILE),
        PanelReaction::Repaint
    ));
...
fn the_forget_key_hovers_on_its_own() {
    ...
    panel.on_event(
        PanelEvent::Motion {
            local: center(&panel, &forget_key(0)),
        },
        TILE,
    );
    ...
    panel.on_event(
        PanelEvent::Motion {
            local: center(&panel, &device_key(0)),
        },
        TILE,
    );
...
fn an_idle_panel_asks_for_no_repaints() {
    ...
    for step in 1..8 {
        panel.set_state(Some("hci0"), &bluez, clear(), true);
        assert!(
            !panel.tick(now + Duration::from_millis(step * 16)),
            "an unchanged reading changes no pixels"
        );
    }
}
...
fn a_changed_device_list_asks_for_a_repaint() {
    ...
    panel.set_state(
        Some("hci0"),
        &state(true, &[("MX Keys", false, true), ("Buds", true, true)]),
        clear(),
        true,
    );
...
fn the_panel_renders_into_a_clamped_grant() {
    let (mut panel, _) = fixture(
        true,
        &[("MX Keys", true, true), ("Buds", false, true)],
        clear(),
    );
    let mut fonts = cosmic_text::FontSystem::new();
    let mut swash = cosmic_text::SwashCache::new();
...
fn every_face_renders_at_every_size_in_every_theme() {
    ...
    let themes = wm_theme::default_theme::all_themes().into_iter().chain(
        wm_theme::default_theme::all_themes_in(wm_theme::model::Appearance::Light),
    );
    for theme in themes {
...
fn arming_a_forget_changes_the_row_it_is_asking_about() {
    ...
    assert!(
        row_changed,
        "the first click must look like a confirm being asked for, at row scale"
    );
}
...
fn device_classes_fold_from_bluez_icons() {
    assert_eq!(
        DeviceClass::from_icon(Some("audio-headset")),
        DeviceClass::Headset
    );
    assert_eq!(
        DeviceClass::from_icon(Some("input-keyboard")),
        DeviceClass::Keyboard
    );
    assert_eq!(
        DeviceClass::from_icon(Some("input-mouse")),
        DeviceClass::Mouse
    );
    assert_eq!(DeviceClass::from_icon(Some("phone")), DeviceClass::Phone);
    assert_eq!(
        DeviceClass::from_icon(Some("printer")),
        DeviceClass::Unknown,
        "no mark is better than the wrong mark"
    );
...
fn the_battery_meter_reads_the_whole_range() {
    ...
    assert_eq!(
        render::battery_level(200),
        1.0,
        "a reading past full clamps rather than overflowing the track"
    );
