    ...
    for (path, interfaces) in objects.entries() {
        if let Some(adapter) = interfaces.get(ADAPTER_IFACE) {
            state.adapters.push(Adapter {
                path: path.clone(),
                powered: property(adapter, "Powered")
        ...
        state.devices.push(Device {
            path: path.clone(),
            name,
            connected: property(device, "Connected")
                .and_then(Json::as_bool)
                .unwrap_or(false),
            paired: property(device, "Paired")
                .and_then(Json::as_bool)
                .unwrap_or(false),
            icon: property(device, "Icon")
                .and_then(Json::as_str)
                .map(str::to_string),
            battery: interfaces
                .get(BATTERY_IFACE)
                .and_then(|bag| property(bag, "Percentage"))
                .and_then(Json::as_u8),
        });
    }
    Some(state)
...
mod tests {
    ...
    fn a_full_reply_yields_the_adapter_and_both_devices() {
        ...
        assert_eq!(
            headset.battery,
            Some(80),
            "Battery1 is a separate interface on the same object"
        );
        ...
        assert_eq!(
            ...
            "a device with no Battery1 reports no battery, not zero"
        );
    }
    ...
    fn an_unpowered_adapter_with_no_devices_is_still_an_adapter() {
        ...
        assert_eq!(
            state.primary().map(|a| a.path.as_str()),
            Some("/org/bluez/hci0")
        );
    }
    ...
    fn a_nameless_device_falls_back_to_its_address() {
        ...
        .expect("parses");
        assert_eq!(
            state.devices[0].name, "01:23:45:67:89:AB",
            "a nameless device must still be identifiable"
        );
    }
    ...
    fn unknown_interfaces_and_properties_are_ignored_not_fatal() {
        ...
        assert!(
            ...
            "a missing Paired reads as unpaired, not as an error"
        );
    }
    ...
    fn a_busctl_failure_message_never_parses_into_a_state() {
        for junk in [
            ...
            assert_eq!(
                parse_managed_objects(junk),
                None,
                "{junk:?} must not become a device list"
            );
        }
    }
    ...
    fn device_addresses_come_back_out_of_their_object_paths() {
        assert_eq!(
            address_from_path("/org/bluez/hci0/dev_F8_4E_17_00_11_22").as_deref(),
            Some("F8:4E:17:00:11:22")
        );
        assert_eq!(
            address_from_path("/org/bluez/hci3/dev_aa_bb_cc_dd_ee_ff").as_deref(),
            Some("aa:bb:cc:dd:ee:ff")
        );
        for bad in [
            "/org/bluez/hci0",
            "/org/bluez/hci0/dev_F8_4E",
            "/org/bluez/hci0/dev_ZZ_4E_17_00_11_22",
            "",
            "dev_",
        ] {
    ...
    fn rfkill_reads_only_the_bluetooth_switches() {
        ...
        assert!(
            !state.soft_blocked(),
            "the wifi radio's block is not Bluetooth's"
        );
    }
    ...
    fn a_soft_block_is_the_one_a_click_can_clear() {
        ...
        assert!(
            soft.soft_blocked(),
            "software said no, so software can say yes"
        );

        let hard = rfkill_from(&[switch("bluetooth", "1", "1")]);
        assert!(
            !hard.soft_blocked(),
            "a hardware switch is not ours to flip, so never offer to"
        );
    ...
    fn no_bluetooth_switch_at_all_is_not_a_block() {
        ...
        assert!(
            !rfkill_from(&[]).present,
            "an empty walk is an absent switch, not a blocked one"
        );
    }
}
