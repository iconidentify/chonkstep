...
impl BtLayout {
    ...
    pub fn row_rect(&self, key: &BtRowKey) -> Option<(i32, i32, u32, u32)> {
        ...
        let (_, y, h) = self
            .bands
            .iter()
            .find(|(band, _, _)| matches!(band, Band::Row(k) if *k == want))?;
        match key {
            BtRowKey::Forget(_) => {
                if !self.devices_live {
                    return None;
            ...
            BtRowKey::Device(_) => {
                ...
                let w = if self.devices_live {
                    (seam - self.glass_x).max(0) as u32
                } else {
                    self.glass_w
                };
...
pub fn bt_layout(view: &BtView, tile: u32) -> BtLayout {
    ...
    match &view.status {
        BtStatus::NoRadio => {
            ...
            push(
                &mut bands,
                &mut y,
                Band::Headline("NO BLUETOOTH RADIO".to_string()),
                m.headline_h,
            );
            y += m.step;
            push(
                &mut bands,
                &mut y,
                Band::Note("THIS MACHINE HAS NO CONTROLLER.".to_string()),
                m.note_h,
            );
            push(
                &mut bands,
                &mut y,
                Band::Note("NOTHING TO SWITCH ON OR PAIR WITH.".to_string()),
                m.note_h,
            );
            y += m.step * 2;
            push(
                &mut bands,
                &mut y,
                Band::Note("PLUG IN A USB ADAPTER AND IT APPEARS".to_string()),
                m.note_h,
            );
            push(
                &mut bands,
                &mut y,
                Band::Note("HERE ON THE NEXT SAMPLE.".to_string()),
                m.note_h,
            );
        }
        BtStatus::NoDaemon => {
            ...
            push(
                &mut bands,
                &mut y,
                Band::Headline("BLUETOOTH SERVICE DOWN".to_string()),
                m.headline_h,
            );
            y += m.step;
            let controller = view
                .controller
                .clone()
                .unwrap_or_else(|| "A CONTROLLER".to_string())
                .to_uppercase();
            push(
                &mut bands,
                &mut y,
                Band::Note(format!("{controller} IS PRESENT, BUT BLUEZ IS NOT")),
                m.note_h,
            );
            push(
                &mut bands,
                &mut y,
                Band::Note("ANSWERING ON THE SYSTEM BUS.".to_string()),
                m.note_h,
            );
            y += m.step * 2;
            ...
            push(
                &mut bands,
                &mut y,
                Band::Hint("systemctl start bluetooth".to_string()),
                m.rh,
            );
        }
        BtStatus::Off { block } => {
            ...
            match block {
                Block::Soft => {
                    push(
                        &mut bands,
                        &mut y,
                        Band::Note("RFKILL HAS A SOFT BLOCK ON THIS RADIO.".to_string()),
                        m.note_h,
                    );
                    push(
                        &mut bands,
                        &mut y,
                        Band::Note("THE BLOCK SURVIVES A REBOOT.".to_string()),
                        m.note_h,
                    );
                }
                Block::Hard => {
                    push(
                        &mut bands,
                        &mut y,
                        Band::Note("A HARDWARE KILL SWITCH IS SET.".to_string()),
                        m.note_h,
                    );
                    push(
                        &mut bands,
                        &mut y,
                        Band::Note("NOTHING HERE CAN CLEAR IT.".to_string()),
                        m.note_h,
                    );
                }
                Block::None => {
                    push(
                        &mut bands,
                        &mut y,
                        Band::Note("THE ADAPTER IS POWERED DOWN.".to_string()),
                        m.note_h,
                    );
                }
            }
            if !view.devices.is_empty() {
                ...
                for device in view.devices.iter().take(MAX_DEVICE_ROWS) {
                    push(
                        &mut bands,
                        &mut y,
                        Band::Row(BtRowKey::Device(device.path.clone())),
                        m.rh,
                    );
                }
                y += m.step;
                push(
                    &mut bands,
                    &mut y,
                    Band::Note("TURN THE RADIO ON TO CONNECT THESE.".to_string()),
                    m.note_h,
                );
            }
        }
        BtStatus::On { .. } => {
            ...
            if view.devices.is_empty() {
                push(
                    &mut bands,
                    &mut y,
                    Band::Note("NO DEVICES PAIRED YET.".to_string()),
                    m.note_h,
                );
            }
            for device in view.devices.iter().take(MAX_DEVICE_ROWS) {
            ...
            if view.devices.len() > MAX_DEVICE_ROWS {
                ...
                push(
                    &mut bands,
                    &mut y,
                    Band::Note(format!("+{hidden} MORE PAIRED, NOT SHOWN")),
                    m.note_h,
                );
            }
            y += m.step * 2;
    ...
    BtLayout {
        ...
        glass_w,
        forget_w: m.forget_w,
        devices_live: view.devices_live(),
    }
}
...
pub fn render_bt_panel_into(
    ...
    let layout = bt_layout(view, tile)
        .anchored(ip::ground_inset(theme))
        .fitted(width, height);
...
fn render_layout(
    ...
    let mut pixmap =
        tiny_skia::Pixmap::new(width.max(1), height.max(1)).expect("nonzero bt panel size");
    ...
    for (band, y, h) in &layout.bands {
        ...
        match band {
            ...
            Band::Note(note) => {
                ...
                let (x, w, align) = if centered {
                    (tx, tw, TextAlign::Center)
                } else {
                    (
                        tx + m.cell as i32,
                        tw.saturating_sub(m.cell),
                        TextAlign::Left,
                    )
                };
            ...
            Band::Hint(hint) => {
                draw_hint(&mut pixmap, fonts, swash, &style, &m, hint, tx, y, tw, h)
            }
            Band::Label(label) => {
                ip::draw_section_header(&mut pixmap, fonts, swash, &style, label, tx, y, tw, h)
            }
            Band::Row(key) => draw_row(
                &mut pixmap,
                fonts,
                swash,
                &style,
...
fn draw_header(
    ...
    wm_theme::bluetooth::draw_bt_rune(
        pixmap,
        x,
        y + (h.saturating_sub(mark) / 2) as i32,
        mark,
        mark,
        rune_color,
    );
    ...
    if has_readout {
        ...
        let cells = if connected > 0 {
            wm_theme::bluetooth::count_digits(connected)
        } else {
            [None, None]
        };
        draw_led_digits(
            pixmap,
            digits_x,
            y + (h.saturating_sub(m.rh) / 2) as i32,
            digits_w,
            m.rh,
            &style.pal,
            &cells,
    ...
    match key {
        BtRowKey::Power => {
            ...
            draw_power_row(
                ...
                gx + m.gutter,
                y,
                gw.saturating_sub((m.gutter * 2) as u32),
                h,
            );
        }
        BtRowKey::PairNew => {
        ...
        BtRowKey::Device(path) | BtRowKey::Forget(path) => {
            let Some(device) = view.devices.iter().find(|d| &d.path == path) else {
                return;
            };
...
fn draw_power_row(
    ...
    let name = style
        .typeface(TypeRole::Row, h)
        .colored(style.ink_for(TypeRole::Row, state));
    ip::draw_type(
        pixmap,
        fonts,
        swash,
    ...
    ip::draw_key_cell(
        ...
        key_h,
        style,
        if hard { RowState::Disabled } else { state },
    );
    ip::draw_type(
        pixmap,
        fonts,
...
fn draw_device_row(
    ...
    draw_glyph(
        pixmap,
        glyph_x,
        y + (h.saturating_sub(glyph) / 2) as i32,
        glyph,
        glyph,
        device.class.grid(),
        glyph_color,
    ...
    let name_w = (right - name_x - battery_w as i32 - if battery_w > 0 { m.gutter } else { 0 })
        .max(0) as u32;
    let label = ip::fit_type(fonts, &name_font, &label, name_w);
    ip::draw_type(
        pixmap,
