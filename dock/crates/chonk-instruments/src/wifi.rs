...
use chonk_dock_widget::{
    ...
    Samples, Source, SourceId, TreeEntry, SAMPLE_INTERVAL,
};
...
fn parse_nmcli_wifi(output: &str) -> Option<(String, u8)> {
    for line in output.lines() {
        ...
        let Ok(signal) = fields[fields.len() - 1].trim().parse::<u8>() else {
            continue;
        };
...
impl WifiWidget {
    ...
    pub fn link_header(&self) -> LinkHeader {
        match &self.state {
            ...
            LinkState::Down { interface } => LinkHeader::Down {
                interface: interface.clone(),
            },
            LinkState::Wifi { ssid, signal } => LinkHeader::Wifi {
                ssid: ssid.clone(),
                signal: *signal,
            },
            LinkState::Wired {
...
impl DockWidget for WifiWidget {
    ...
    fn update(&mut self, samples: &Samples) -> bool {
        ...
        if !(samples.fresh(self.interfaces)
            || samples.fresh(self.wifi_list)
            || samples.fresh(self.radio))
        {
            return false;
        }
        ...
        self.radio_enabled = samples
            .text(self.radio)
            .and_then(parse_nmcli_radio)
            .or(self.radio_enabled);
    ...
    fn render(
        ...
        let reading = match &self.state {
            ...
            LinkState::Wifi { ssid, signal } => LinkReading::Wifi {
                ssid,
                signal_pct: *signal,
            },
            LinkState::Wired {
    ...
    fn on_input(&mut self, input: DockInput, _tile: u32) -> Vec<Effect> {
        ...
        if self.nmcli == Some(true) && self.wifi_hw {
            ...
            vec![Effect::Run {
                program: "nmcli",
                args: radio_set_args(enabled),
                then: Some(self.radio),
            }]
...
mod tests {
    ...
    fn iface(
        ...
        TreeEntry {
            name: name.to_string(),
            files: vec![
                Some(operstate.to_string()),
                Some(carrier.to_string()),
                speed.map(str::to_string),
            ],
            dirs: vec![wireless],
        }
    }
    ...
    fn nmcli_escaped_colons_and_backslashes_stay_in_the_ssid() {
        assert_eq!(
            parse_nmcli_wifi("yes:Lab\\:5G:64\n"),
            Some(("Lab:5G".to_string(), 64))
        );
        assert_eq!(
            parse_nmcli_wifi("yes:Back\\\\slash:12\n"),
    ...
    fn nmcli_garbage_lines_are_skipped_not_fatal() {
        assert_eq!(
            parse_nmcli_wifi("yes:Broken:notanumber\nyes:Good:55\n"),
            Some(("Good".to_string(), 55))
        );
        assert_eq!(
            parse_nmcli_wifi("yes:TooHot:250\n"),
            Some(("TooHot".to_string(), 100)),
            "signal clamps to 100"
        );
    }
    ...
    fn best_probe_prefers_the_live_interface_but_settles_for_a_dead_one() {
        let probes = [
            probe("eth0", false, false, None),
            probe("eth1", true, true, Some(100)),
        ];
        assert_eq!(best_probe(&probes), 1);
        let all_dead = [
            probe("eth0", false, false, None),
            probe("eth1", false, false, None),
        ];
    ...
    fn nmcli_radio_reads_only_the_two_words_it_knows() {
        ...
        for unknown in [
            ...
            assert_eq!(
                parse_nmcli_radio(unknown),
                None,
                "{unknown:?} must not become a direction to move the radio"
            );
        }
    }
    ...
    fn probes_from_a_sampled_tree_drops_loopback_and_keeps_the_walk_order() {
        ...
        assert_eq!(
            probes[1].speed_mbps, None,
            "-1 is the kernel's 'unknown', not a speed"
        );
        assert!(
            probes[1].wireless,
            "the wireless/ subdirectory is what gates the radio toggle"
        );
    }
    ...
    fn update_folds_the_three_sources_into_one_link_state() {
        ...
        assert!(
            !widget.wifi_hw,
            "no wireless/ and no association means no radio to toggle"
        );
        ...
        assert!(
            ...
            "a pass with nothing fresh folds nothing"
        );
    }
    ...
    fn a_panel_only_sample_updates_its_rows_without_repainting_the_tile() {
        ...
        assert!(
            !widget.update(&bench.samples()),
            "the tile has no changed sample"
        );
        ...
        assert_eq!(
            ...
            1,
            "the independent connection result must be folded"
        );
        assert_eq!(view.connections[0].uuid, "test-uuid");
        assert!(
            widget.panel_tick(std::time::Instant::now()),
            "only the panel is dirty"
        );
        assert!(
            !widget.panel_tick(std::time::Instant::now()),
            "dirty is consumed once"
        );
        ...
        match &effects[0] {
            Effect::Run {
                ...
                assert_eq!(
                    ...
                    "an enabled radio is turned off"
                );
                assert_eq!(
                    *then,
                    Some(radio),
                    "and the state that decided the direction is what gets resampled"
                );
            }
