...
use chonk_dock_widget::{
    ...
    Samples, Source, SourceId, SAMPLE_INTERVAL,
};
...
impl DockWidget for SoundWidget {
    ...
    fn update(&mut self, samples: &Samples) -> bool {
        ...
        if samples.fresh(self.sink) {
            let reading = samples
                ...
                .map(|(volume, muted)| SinkState { volume, muted });
        ...
        if samples.fresh(self.default_sink) {
            self.fresh_panel |= 2;
            let reading = samples
                .text(self.default_sink)
                .and_then(audio_panel::parse_default_sink);
        ...
        if samples.fresh(self.streams) {
            self.fresh_panel =
                (self.fresh_panel & !4) | (u8::from(samples.text(self.streams).is_some()) * 4);
            let streams = samples
                .text(self.streams)
                .map(audio_panel::parse_sink_inputs)
                .unwrap_or_default();
    ...
    fn on_input(&mut self, input: DockInput, tile: u32) -> Vec<Effect> {
        ...
        // it.
        match input {
            DockInput::Press { local, .. } => {
                vec![self.zone_effect(soundctl::zone_at(local, tile))]
            }
    ...
    fn render_panel(&mut self, frame: &mut PanelFrame, ctx: &mut PanelCtx<'_>) {
        ...
        self.metrics = PanelMetrics::granted(
            ctx.tile,
            frame.width(),
            frame.height(),
            self.panel.sinks().len(),
        );
        let buffer =
            render_audio_panel(ctx.theme, ctx.fonts, ctx.swash, &self.metrics, &self.panel);
    ...
    fn panel_input(&mut self, event: PanelEvent, _tile: u32) -> PanelReaction {
        ...
        match event {
            ...
            PanelEvent::Scroll { local, delta } => {
                match self.panel.on_scroll(local, delta, &metrics) {
...
mod tests {
    ...
    fn parses_the_documented_wpctl_formats() {
        ...
        assert_eq!(
            parse_wpctl_volume("Volume: 1.20"),
            Some((1.2, false)),
            "overdrive passes through"
        );
    }
    ...
    fn zone_commands_cap_louder_and_only_louder() {
        assert!(zone_command(SoundZone::Louder)
            .windows(2)
            .any(|w| w == ["-l", "1.0"]));
        ...
        assert_eq!(
            zone_command(SoundZone::MuteToggle),
    ...
    fn update_folds_a_wpctl_line_into_the_sink_state() {
        ...
        assert!(
            !widget.update(&bench.samples()),
            "the same reading again is not a repaint"
        );
    }
    ...
    fn without_a_sink_the_face_is_dead_and_the_zones_are_inert() {
        ...
        let press = DockInput::Press {
            local: Point::new(28, 8),
        ...
        match &effects[0] {
            Effect::Run {
                ...
                assert_eq!(
                    *then,
                    Some(id),
                    "the set must nudge the sampler that will confirm it"
                );
            }
    ...
    fn a_scroll_notch_maps_to_the_matching_zone_command() {
        ...
        for (delta, zone) in [(1, SoundZone::Louder), (-1, SoundZone::Softer)] {
            let effects = widget.on_input(
                DockInput::Scroll {
                    local: Point::new(28, 50),
                    delta,
                },
                56,
            );
    ...
    fn scroll_ignores_the_click_zone_map() {
        ...
        for y in [0, 28, 55] {
            ...
            match &effects[0] {
                Effect::Run { args, .. } => assert!(
                    args.contains(&"5%+".to_string()),
                    "y {y}: up is louder everywhere"
                ),
    ...
    fn scroll_bursts_are_clamped_and_zero_is_ignored() {
        ...
        assert_eq!(
            burst.len(),
            MAX_SCROLL_NOTCHES as usize,
            "i32::MIN must clamp, not overflow"
        );
    }
    ...
    fn row(i: i32) -> Point {
        let m = panel_metrics();
        Point::new(
            m.glass_x() + m.row_h as i32,
            m.row_top(i as usize) + m.row_h as i32 / 2,
        )
    }
    ...
    fn the_panel_is_offered_only_once_pactl_has_answered() {
        let mut widget = SoundWidget::new();
        assert!(
            widget.panel_spec(56).is_none(),
            "nothing sampled, nothing to show"
        );
        ...
        widget.update(&bench.samples());
        assert!(
            widget.state.is_some(),
            "the tile still works on wpctl alone"
        );
        assert!(widget.panel_spec(56).is_none(), "no pactl, no panel");

        let (widget, _) = panel_widget();
        assert_eq!(
            widget.panel_spec(56).map(|s| (s.width, s.height)),
            Some(PanelMetrics::request(56, 2))
        );
    }
    ...
    fn a_mixer_with_no_outputs_still_opens() {
        ...
        assert_eq!(
            widget.panel_spec(56).map(|s| (s.width, s.height)),
            Some(PanelMetrics::request(56, 0))
        );
    }
    ...
    fn the_pactl_readings_fold_into_the_panel_and_tick_once_per_change() {
        ...
        assert!(
            ...
            "the first fold is a repaint"
        );
        assert!(
            !widget.panel_tick(std::time::Instant::now()),
            "and it is taken, not left set"
        );
    ...
    fn a_row_click_switches_and_migrates_the_playing_streams() {
        ...
        assert_eq!(
            then,
            Some(b.default_sink),
            "the default reading is what proves the switch"
        );
        assert_eq!(
            widget.panel.shown_default(),
            Some("hdmi"),
            "the lamp jumps on the click, optimistically"
        );
        ...
        assert!(matches!(
            widget.panel_input(PanelEvent::Motion { local: row(0) }, 56),
            PanelReaction::Repaint
        ));
    }
    ...
    fn closed_sound_discovers_once_and_keeps_only_pending_confirmation_active() {
        ...
        assert_eq!(
            (0..4)
                .map(|i| widget.source_active(i, false))
                .collect::<Vec<_>>(),
        ...
        widget.panel_input(PanelEvent::LeftPress { local: row(0) }, 56);
        let effects = widget
            .panel_input(PanelEvent::LeftRelease { local: row(0) }, 56)
            .effects();
        assert_eq!(effects.len(), 2);
        widget.panel_visibility_changed(false);
        ...
        widget.update(&b.bench.samples());
        assert!(
            !widget.source_active(2, false),
            "confirmation retires closed-panel work"
        );
        ...
        assert!(
            !widget.source_active(2, false),
            "a stopped confirming worker cannot leave a permanent prediction"
        );
    }
    ...
    fn reopening_waits_for_new_sink_and_stream_data_before_switching() {
        ...
        assert_eq!(
            widget.panel.shown_default(),
            Some("usb"),
            "no false prediction while refreshing"
        );
        ...
        b.bench.set_text(
            b.streams,
            r#"[{"index":42,"properties":{"application.name":"New player"}}]"#,
        );
        widget.update(&b.bench.samples());
        widget.panel_input(PanelEvent::LeftPress { local: row(0) }, 56);
        let effects = widget
            ...
            .effects();
        assert!(
            matches!(effects.as_slice(), [Effect::Run { .. }, Effect::Run { args, .. }]
            if args == &["move-sink-input", "42", "hdmi"])
        );
        widget.panel_visibility_changed(false);
        assert!(
            widget
                .panel_input(PanelEvent::Motion { local: row(1) }, 56)
                .effects()
                .is_empty(),
    ...
    fn the_panel_wheel_is_the_hovered_devices_knob() {
        ...
        let reaction = widget.panel_input(
            PanelEvent::Scroll {
                local: row(0),
                delta: 1,
            },
        ...
        assert_eq!(
            args,
            ["set-sink-volume", "hdmi", "45%"],
            "40% plus one 5% notch, absolutely"
        );
        assert_eq!(then, Some(b.sinks));
    ...
    fn crossings_track_the_pointer_without_acting() {
        ...
        assert!(matches!(
            widget.panel_input(PanelEvent::Enter, 56),
            PanelReaction::None
        ));
        widget.panel_input(PanelEvent::LeftPress { local: row(0) }, 56);
        assert!(matches!(
            widget.panel_input(PanelEvent::Leave, 56),
            PanelReaction::Repaint
        ));
        assert!(matches!(
            widget.panel_input(PanelEvent::LeftRelease { local: row(0) }, 56),
            PanelReaction::None
        ));
    }
}
