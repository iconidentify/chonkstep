...
pub fn speed_digits(speed_mbps: u32) -> ([Option<u8>; 4], &'static str) {
    let (value, mark) = if speed_mbps >= 10_000 {
        (speed_mbps / 1000, "G")
    } else {
        (speed_mbps, "M")
    };
...
pub fn render_wifi_tile(
    ...
    let (gx, gy, gw, gh) =
        panel::draw_panel_glass(&mut pixmap, margin, margin, well_w, well_h, theme);
...
fn draw_link_row(
    ...
    let font = FontSpec {
        family: family.to_string(),
        size: (h as f32 * 0.52).max(6.0),
        weight: FontWeight::Bold,
        style: FontStyle::Normal,
    };
    let text_w =
        paint::text_width(font_system, &font, label).min(w.saturating_sub(lamp + gap as u32));
...
mod tests {
    ...
    fn count_exact(buffer: &DecorationBuffer, color: Color) -> usize {
        buffer
            ...
            .0
            .iter()
            .filter(|p| (p[0], p[1], p[2]) == (color.r, color.g, color.b))
            .count()
    }
    ...
    fn the_four_states_are_pairwise_distinct_at_default_scale() {
        ...
        for a in 0..faces.len() {
            for b in (a + 1)..faces.len() {
                assert_ne!(
                    faces[a], faces[b],
                    "states {a} and {b} rendered identically"
                );
            }
        }
    }
    ...
    fn a_down_link_lights_no_ink_at_all() {
        ...
        let pal = panel::panel_palette(&theme);
        assert!(
            pal.ink.r != pal.ink.g || pal.ink.g != pal.ink.b,
            "test premise: flagship LED ink is saturated"
        );
        for size in [56u32, 112] {
            let down = render_wifi_tile(
                &theme,
                &mut fs,
                &mut sc,
                size,
            ...
            assert_eq!(
                count_exact(&down, pal.ink),
                0,
                "size {size}: down face must not light full ink"
            );
            let up = render_wifi_tile(
                &theme,
                &mut fs,
                &mut sc,
                size,
            ...
            assert!(
                count_exact(&up, pal.ink) > 0,
                "size {size}: a live link must light the lamp and digits"
            );
        }
    }
    ...
    fn different_ssids_render_differently_and_long_ones_stay_inside() {
        ...
        let long = render_wifi_tile(
            ...
            56,
            &LinkReading::Wifi {
                ssid: "AN ABSURDLY LONG NETWORK NAME THAT CANNOT FIT",
                signal_pct: 50,
            },
        );
    ...
    fn signal_bars_map_with_a_ceiling() {
        ...
        assert_eq!(
            signal_bars(255),
            5,
            "out-of-range input clamps, never overflows the meter"
        );
    }
    ...
    fn speed_digits_cover_the_real_ladder() {
        ...
        assert_eq!(
            speed_digits(1000),
            ([Some(1), Some(0), Some(0), Some(0)], "M")
        );
        assert_eq!(
            speed_digits(2500),
            ([Some(2), Some(5), Some(0), Some(0)], "M")
        );
        assert_eq!(speed_digits(10_000), ([None, None, Some(1), Some(0)], "G"));
        assert_eq!(
            speed_digits(100_000),
            ([None, Some(1), Some(0), Some(0)], "G")
        );
        assert_eq!(
            speed_digits(0),
            ([None, None, None, Some(0)], "M"),
            "an honest zero still shows a digit"
        );
    }
}
