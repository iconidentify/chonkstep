...
pub fn render_bluetooth_tile(
    ...
    draw_bt_rune(
        ...
        rune_w,
        gh,
        if powered { pal.ink } else { pal.ghost },
    );
...
fn draw_panel_row(
    ...
    let font = FontSpec {
        family: family.to_string(),
        size: (row_h as f32 * 0.50).max(6.0),
        weight: FontWeight::Bold,
        style: FontStyle::Normal,
    };
    ...
    match row {
        BtPanelRow::Power { on } => {
            ...
            paint::draw_text(
                ...
                text_x,
                y,
                width.saturating_sub(text_x as u32),
                row_h,
                TextAlign::Left,
            );
            let state_w = row_h * 2;
        ...
        BtPanelRow::Device {
            ...
            paint::fill_rect(
                ...
                lamp,
                lamp,
                if *connected { pal.ink } else { pal.ghost },
            );
...
mod tests {
    ...
    fn the_off_face_lights_no_ink_at_all() {
        ...
        for size in [56u32, 112] {
            ...
            assert_eq!(
                count_exact(&off, pal.ink),
                0,
                "size {size}: off face must not light full ink"
            );
            let idle = render_bluetooth_tile(&theme, &mut fs, &mut sc, size, &BtReading::Idle);
            assert!(
                count_exact(&idle, pal.ink) > 0,
                "size {size}: a powered radio must light its rune"
            );
        }
    }
    ...
    fn a_connection_lights_more_ink_than_idle() {
        ...
        let connected = render_bluetooth_tile(
            &theme,
            &mut fs,
            &mut sc,
            56,
        ...
        assert!(
            count_exact(&connected, ink) > count_exact(&idle, ink),
            "the count digits must add lit ink over the bare rune"
        );
    }
    ...
    fn the_rune_cell_stays_legible_at_the_stock_tile() {
        ...
        assert!(
            cell >= 3.0,
            "a {cell:.1}px rune cell reads as noise, not as the Bluetooth mark"
        );
    }
    ...
    fn long_device_names_truncate_rather_than_overflow() {
        ...
        let long = render_bluetooth_tile(
            ...
            &BtReading::Connected {
                count: 1,
                name: "AN ABSURDLY LONG DEVICE NAME THAT CANNOT FIT",
            },
        );
    ...
    fn an_armed_forget_cell_is_visibly_different() {
        ...
        let (mut fs, mut sc) = ctx();
        let (w, row_h) = (panel_content_width(56), panel_row_height(56));
        let calm = [BtPanelRow::Device {
            name: "BUDS",
            connected: true,
            pending: false,
            armed: false,
        }];
        let armed = [BtPanelRow::Device {
            name: "BUDS",
            connected: true,
            pending: false,
            armed: true,
    ...
    fn pending_and_settled_devices_render_differently() {
        ...
        let (mut fs, mut sc) = ctx();
        let (w, row_h) = (panel_content_width(56), panel_row_height(56));
        let settled = [BtPanelRow::Device {
            name: "BUDS",
            connected: false,
            pending: false,
            armed: false,
        }];
        let pending = [BtPanelRow::Device {
            name: "BUDS",
            connected: false,
            pending: true,
            armed: false,
