...
pub fn render_nettraffic_tile(
    ...
    let (gx, gy, gw, gh) =
        panel::draw_panel_glass(&mut pixmap, f.well_x, f.well_y, f.well_w, f.well_h, theme);
    debug_assert_eq!(
        (gx, gy, gw, gh),
        (f.glass_x, f.glass_y, f.glass_w, f.glass_h),
        "frame() must mirror draw_panel_glass's inset"
    );
...
fn draw_graph(
    ...
    panel::draw_led_matrix(
        ...
        band.h as u32,
        pal,
        NET_TRAFFIC_HALF_ROWS,
        down.history,
        up.history,
    );
    let meter = Band {
        x: band.x + pad + hist_w + now_gap,
        y: band.y,
        w: now_w,
        h: band.h,
    };
...
mod tests {
    ...
    fn buffers_come_out_at_the_requested_size() {
        ...
        for size in [16u32, 56, 112] {
            let buffer = render(
                ...
                size,
                "eth0",
                &lane(84.0 * 1024.0 * 1024.0, 4, &WAVE),
                &lane(38.0 * 1024.0, 1, &IDLE),
            );
        ...
        for theme in all_themes() {
            for size in [56u32, 112] {
                let buffer = render(
                    ...
                    size,
                    "wlan0",
                    &lane(12.5 * 1024.0 * 1024.0, 3, &WAVE),
                    &lane(420.0 * 1024.0, 2, &WAVE),
                );
                assert_eq!(
    ...
    fn the_representative_states_all_render_distinctly() {
        ...
        let sat = render(
            ...
            56,
            "eth0",
            &lane(118.0 * 1024.0 * 1024.0, 4, &FULL),
            &lane(97.0 * 1024.0 * 1024.0, 4, &FULL),
        );
        let states = [
            ("idle", &idle),
            ("download", &dl),
        ...
        for (i, (name_a, a)) in states.iter().enumerate() {
            for (name_b, b) in states.iter().skip(i + 1) {
                assert_ne!(
                    a.pixels, b.pixels,
                    "{name_a} and {name_b} must not render identically"
                );
            }
        }
    }
    ...
    fn download_lights_only_the_top_half_and_upload_only_the_bottom() {
        ...
        let size = 112u32;
        let pal = panel::panel_palette(&theme);
        let blank = RateReadout {
            digits: [None; 3],
            unit: RateUnit::Kilo,
        };
        let busy = TrafficLane {
            readout: blank,
            now: NET_TRAFFIC_HALF_ROWS,
            history: &FULL,
        };
        let quiet = TrafficLane {
            readout: blank,
            now: 0,
            history: &IDLE,
        };
        ...
        let ink_in = |buffer: &DecorationBuffer, y0: i32, y1: i32| -> usize {
            ...
            for y in y0..y1 {
                for x in f.glass_x..f.glass_x + f.glass_w as i32 {
                    ...
                    if (buffer.pixels[i], buffer.pixels[i + 1], buffer.pixels[i + 2])
                        == (pal.ink.r, pal.ink.g, pal.ink.b)
                    {
                        count += 1;
                    }
                }
        ...
        assert!(
            ink_in(&dl, f.glass_y, mid) > 50,
            "download load must light the top half"
        );
        assert_eq!(
            ink_in(&dl, mid, f.glass_y + f.glass_h as i32),
            0,
            "download load must leave the bottom half dark"
        );

        let ul = render(&theme, size, "eth0", &quiet, &busy);
        assert_eq!(
            ink_in(&ul, f.glass_y, mid),
            0,
            "upload load must leave the top half dark"
        );
        assert!(
            ink_in(&ul, mid, f.glass_y + f.glass_h as i32) > 50,
            "upload load must light the bottom half"
        );
    }
    ...
    fn long_modern_interface_names_stay_inside_the_strip() {
        ...
        for size in [56u32, 112] {
            ...
            assert!(
                touched > 0,
                "size {size}: the long name must actually be drawn"
            );
        }
    }
    ...
    fn each_lane_shows_its_own_unit_letter() {
        let theme = nextstep_classic();
        let kilo = TrafficLane {
            readout: RateReadout {
                digits: [None, Some(4), Some(2)],
                unit: RateUnit::Kilo,
            },
            now: 1,
            history: &IDLE,
        };
        let giga = TrafficLane {
            readout: RateReadout {
                digits: [None, Some(4), Some(2)],
                unit: RateUnit::Giga,
            },
    ...
    fn format_rate_folds_rates_into_blank_padded_digits_and_escalating_units() {
        ...
        assert_eq!(
            format_rate(1023.0 * k),
            RateReadout {
                digits: [None, None, Some(1)],
                unit: RateUnit::Mega
            }
        );
        assert_eq!(
            format_rate(5.0 * k * k),
            RateReadout {
                digits: [None, None, Some(5)],
                unit: RateUnit::Mega
            }
        );
        assert_eq!(
            format_rate(3.0 * k * k * k),
            RateReadout {
                digits: [None, None, Some(3)],
                unit: RateUnit::Giga
            }
        );
    ...
    fn quantize_level_keeps_trickles_visible_and_zero_dark() {
        ...
        assert_eq!(
            quantize_level(1e-4, 4),
            1,
            "any traffic at all must light one cell"
        );
        assert_eq!(quantize_level(0.25, 4), 1);
        assert_eq!(quantize_level(0.26, 4), 2);
        assert_eq!(quantize_level(1.0, 4), 4);
        assert_eq!(
            quantize_level(5.0, 4),
            4,
            "over-unity input clamps to the top cell"
        );
