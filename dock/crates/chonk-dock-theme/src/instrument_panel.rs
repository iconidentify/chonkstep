...
//!
//! ```no_run
//! # use chonk_dock_theme::instrument_panel as ip;
//! # use chonk_dock_theme::model::TextAlign;
//! # fn draw(pixmap: &mut tiny_skia::Pixmap, theme: &chonk_dock_theme::Theme,
...
pub fn draw_key_cell(
    ...
    match state {
        ...
        state => {
            let lift = if state == RowState::Hover {
                HOVER_LIFT
            } else {
                14
            };
...
pub fn draw_lamp(
    ...
    let (fill, halo) = match state {
        LampState::On => (
            style.pal.ink,
            Some(mix(style.pal.glass, style.pal.ink, 0.45)),
        ),
    ...
    if state == LampState::On {
        ...
        if core > 0 {
            paint::fill_rect(
                ...
                core as u32,
                core as u32,
                mix(fill, Color::rgb(0xFF, 0xFF, 0xFF), 0.5),
            );
        }
    }
...
pub fn draw_meter(
    ...
    panel::draw_led_bar(
        pixmap, inner_x, inner_y, inner_w, inner_h, &pal, segments, lit, false,
    );
}
...
pub fn draw_type(
    ...
    if font.tracking <= 0.0 {
        paint::draw_text(
            pixmap, fonts, swash, text, &font.spec, font.color, x, y, w, h, align,
        );
        return;
    }
...
pub fn fit_type(
    ...
    paint::fit_measured_prefix(text, max_w, "…", |candidate| {
        type_width(fonts, font, candidate)
    })
}
...
mod tests {
    ...
    fn the_type_ramp_keeps_its_contrast_ordering_in_every_theme() {
        for theme in every_theme() {
            ...
            assert!(
                section.tracking > 0.0,
                "theme {}: the section label is tracked",
                theme.id
            );
            assert_eq!(
                row.tracking, 0.0,
                "theme {}: body type is not tracked",
                theme.id
            );
            ...
            assert!(
                s + 90 < r,
                "theme {}: section ({s}) must be clearly dimmer than a row label ({r})",
                theme.id
            );
            assert!(
                r + 40 < o,
                "theme {}: a readout ({o}) must clearly out-glow a row label ({r})",
                theme.id
            );
            assert!(
    ...
    fn every_ramp_step_carries_the_glass_in_every_theme() {
        for theme in every_theme() {
            ...
            for role in [TypeRole::Section, TypeRole::Row, TypeRole::Readout] {
                ...
                assert!(
                    ink - glass > 150,
                    "theme {}: {role:?} ink {ink} is lost on glass {glass}",
                    theme.id
                );
            }
        }
    ...
    fn a_meter_draws_zero_one_percent_and_full_distinctly() {
        ...
        assert_ne!(
            full, idle,
            "a playing meter and an idle one burn at different currents"
        );
        assert_ne!(idle, silent, "a silenced meter lights nothing at all");
        assert_eq!(
            zero,
            draw(0.0, MeterGlow::Active),
            "and the drawing is a pure function of its inputs"
        );
        ...
        assert!(
            low < mid && mid < high,
            "lit area must rise with the level: {low} < {mid} < {high}"
        );
    ...
    fn row_and_key_states_all_read_differently() {
        for theme in every_theme() {
            ...
            for draw in [&row as &dyn Fn(RowState) -> Vec<u8>, &key] {
                ...
                assert_ne!(
                    hover, pressed,
                    "theme {}: press must not look like hover",
                    theme.id
                );
                assert_ne!(idle, pressed, "theme {}: press must show", theme.id);
                assert_ne!(
                    hover, disabled,
                    "theme {}: a disabled control must not look hovered",
                    theme.id
                );
            }
        }
    ...
    fn the_ground_is_glass_inside_a_gasket_in_every_theme() {
        for theme in every_theme() {
            ...
            assert_eq!(
                (gx, gy),
                (inset, inset),
                "theme {}: the glass sits one gasket in",
                theme.id
            );
            assert_eq!(
                (gw, gh),
                ((200 - inset * 2) as u32, (100 - inset * 2) as u32),
                "theme {}",
                theme.id
            );
            ...
            assert!(
                p.pixels().iter().all(|px| px.alpha() == 255),
                "theme {}: the ground is opaque",
                theme.id
            );
        }
    }
    ...
    fn an_engraved_rule_is_a_groove_not_a_hairline() {
        ...
        assert_ne!(
            style.engrave_dark(),
            style.engrave_light(),
            "the two walls are different colors"
        );

        let mut seam = pixmap(20, 60, &theme);
        draw_engraved_seam(&mut seam, 10, 6, 48, &style);
        assert_eq!(
            count_of(&seam, style.engrave_dark()),
            48,
            "the standing groove has both walls too"
        );
    ...
    fn tracked_type_is_wider_and_headers_carry_their_rule() {
        ...
        assert!(
            tracked_w > solid_w,
            "tracking widens the label: {tracked_w} vs {solid_w}"
        );

        let mut p = pixmap(200, 30, &theme);
        let before = p.data().to_vec();
        draw_section_header(
            &mut p, &mut fonts, &mut swash, &style, "OUTPUTS", 6, 6, 188, 14,
        );
        assert_ne!(before, p.data().to_vec(), "the header draws something");
        assert!(
            count_of(&p, style.engrave_dark()) > 40,
            "the header's rule runs to the right edge"
        );
    }
    ...
    fn fitting_trims_characters_not_the_box() {
        ...
        assert!(
            type_width(&mut fonts, &row, &fitted) <= 90,
            "the result fits"
        );
        assert_eq!(
            fit_type(&mut fonts, &row, "OK", 400),
            "OK",
