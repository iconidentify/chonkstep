...
const PANEL_SHADOW: Color = Color {
    r: 0x58,
    g: 0x64,
    b: 0x58,
    a: 0xff,
...
fn draw_dot_matrix(
    ...
    let mut lit_paint = Paint {
        anti_alias: false,
        ..Default::default()
    };
    lit_paint.set_color(paint::sk_color(INK));
    let mut unlit_paint = Paint {
        anti_alias: false,
        ..Default::default()
    };
    ...
    for col in 0..NET_LOAD_COLUMNS {
        let rx_lit = rx_levels
            .get(col)
            .copied()
            .unwrap_or(0)
            .min(NET_LOAD_HALF_ROWS);
        let tx_lit = tx_levels
            .get(col)
            .copied()
            .unwrap_or(0)
            .min(NET_LOAD_HALF_ROWS);
        for row in 0..rows {
            ...
            if let Some(r) = SkRect::from_xywh(cx, cy, dot, dot) {
                pixmap.fill_rect(
                    r,
                    if lit { &lit_paint } else { &unlit_paint },
                    Transform::identity(),
                    None,
                );
            }
...
mod tests {
    ...
    fn different_units_light_a_different_label() {
        ...
        assert_ne!(
            kilo.pixels, giga.pixels,
            "the K/M/G column should visibly change which label is lit"
        );
    }
    ...
    fn tile_face_shows_through_and_the_lcd_glass_survives() {
        ...
        assert_ne!(
            px(2, 2),
            px(size - 3, size - 3),
            "tile gradient should show on the face"
        );
        ...
        assert!(
            glass > 200,
            "expected a substantial LCD glass area, found {glass} PANEL pixels"
        );
    }
}
