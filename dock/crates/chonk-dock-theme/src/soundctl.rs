...
pub fn render_soundctl_tile(
    ...
    let (gx, gy, gw, gh) = panel::draw_panel_glass(
        &mut pixmap,
        r.margin,
        r.margin,
        well_w,
...
mod tests {
    ...
    fn distinct_states_render_distinctly_at_both_scales() {
        ...
        for size in [56u32, 112] {
            ...
            assert_ne!(
                mid, muted,
                "size {size}: muted must not look like its own level"
            );
            assert_ne!(
                muted, silent,
                "size {size}: muted and volume-zero are different states"
            );
        }
    }
    ...
    fn digits_alone_distinguish_levels_within_one_slat() {
        ...
        let (mut fs, mut sc) = ctx();
        assert_eq!(
            lit_segments(0.55, SOUND_BAR_SEGMENTS),
            lit_segments(0.56, SOUND_BAR_SEGMENTS)
        );
    ...
    fn muted_glass_is_all_ghost_and_unmuted_glass_glows() {
        ...
        let ink_on_glass = |buf: &DecorationBuffer| {
            ...
            for y in r.glass_y..(r.glass_y + r.glass_h) {
                for x in glass_x..(glass_x + glass_w) {
                    ...
                    if (buf.pixels[i], buf.pixels[i + 1], buf.pixels[i + 2])
        ...
        assert_eq!(
            ink_on_glass(&muted),
            0,
            "muted glass must not light a single LED"
        );
        assert!(
            ink_on_glass(&live) > 50,
            "an unmuted 55% should light digits and slats"
        );
    }
    ...
    fn louder_volume_lights_more_of_the_glass() {
        ...
        assert!(
            full > low,
            "100% ({full} ink px) should out-light 15% ({low} ink px)"
        );
    }
    ...
    fn every_theme_renders_with_its_own_glass() {
        ...
        for theme in all_themes() {
            ...
            assert!(
                glass > 200,
                "theme {}: expected a substantial glass area, found {glass} px",
                theme.id
            );
        }
    }
    ...
    fn percent_digits_blank_pad_like_a_real_lcd() {
        ...
        assert_eq!(
            percent_digits(1.5),
            [Some(1), Some(5), Some(0)],
            "overdrive shows honestly"
        );
        assert_eq!(
            percent_digits(-0.2),
            [None, None, Some(0)],
    ...
    fn zones_band_the_tile_top_to_bottom() {
        for tile in [56u32, 112] {
            ...
            assert_eq!(
                at(0),
                SoundZone::Louder,
                "tile {tile}: the top edge raises volume"
            );
            assert_eq!(at(tile as i32 / 4), SoundZone::Louder);
            assert_eq!(
                at(tile as i32 - 1),
                SoundZone::MuteToggle,
                "tile {tile}: the base toggles mute"
            );
        }
    }
