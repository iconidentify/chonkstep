...
fn frame_layout(size: u32, theme: &Theme) -> (RectI, RectI) {
    ...
    (
        (margin, margin, well_w as u32, well_h as u32),
        (margin, margin + well_h, well_w as u32, strip_h as u32),
    )
}
...
fn regions(gx: i32, gy: i32, gw: u32, gh: u32) -> Regions {
    ...
    Regions {
        ...
        zone: (
            gx + gw.saturating_sub(zone_w) as i32,
            gy,
            zone_w.min(gw),
            digit_h,
        ),
...
fn draw_battery_glass(
    ...
    match (state, low) {
        ...
        (ChargeState::Discharging, false) => {
            draw_mark(pixmap, bx, by, bw, bh, pal.ghost, &BOLT_MARK)
        }
    }
...
pub fn render_power_tile(
    ...
    let (gx, gy, gw, gh) =
        panel::draw_panel_glass(&mut pixmap, well.0, well.1, well.2, well.3, theme);
    ...
    let (word, word_color) = match face {
        PowerFace::Battery { capacity, state } => {
            ...
            match state {
                ...
                ChargeState::Discharging if is_low(capacity, state) => {
                    ("LOW", panel::panel_accent(theme))
                }
...
mod tests {
    ...
    fn all_six_states_render_pairwise_distinctly() {
        ...
        let rendered: Vec<Vec<u8>> = ALL_FACES
            .iter()
            .map(|f| render(&theme, 56, *f).pixels)
            .collect();
        for a in 0..rendered.len() {
            for b in a + 1..rendered.len() {
                assert_ne!(
                    rendered[a], rendered[b],
                    "faces {:?} and {:?} render identically",
                    ALL_FACES[a], ALL_FACES[b]
                );
            }
        }
    }
    ...
    fn low_battery_kills_the_ghosts_and_frames_the_meter() {
        ...
        let mid = |i: u32| {
            (
                mx + (i as f32 * cell + cell / 2.0) as i32,
                my + mh as i32 / 2,
            )
        };
        ...
        assert_eq!(
            px(&low, x0, y0),
            rgb(pal.ink),
            "the remaining cell still glows"
        );
        let (x5, y5) = mid(5);
        assert_eq!(
            px(&low, x5, y5),
            rgb(pal.glass),
            "ghost cells go dark, not ghost"
        );
        assert_eq!(
            px(&low, mx - 2, my - 2),
            rgb(pal.ink),
            "the alarm frame corner is inked"
        );
    ...
    fn the_indicator_zone_tells_the_states_apart() {
        ...
        assert!(
            count_in_rect(&charging, zone, pal.ink) > 0,
            "charging bolt is lit"
        );
        assert_eq!(
            count_in_rect(&discharging, zone, pal.ink),
            0,
            "discharging shows no lit mark"
        );
        assert!(
            count_in_rect(&discharging, zone, pal.ghost) > 0,
            "discharging shows the ghost bolt die"
        );
        assert!(
            count_in_rect(&full, zone, pal.ink) > 0,
            "full-on-ac plug is lit"
        );
        assert!(
            count_in_rect(&low, zone, pal.ink) > 0,
            "low alarm mark is lit"
        );
    ...
    fn unknown_capacity_degrades_to_ghost_digits_and_an_empty_scale() {
        ...
        assert_eq!(
            count_in_rect(&buf, r.digits, pal.ink),
            0,
            "no digit segment may claim a reading"
        );
        assert_eq!(
            count_in_rect(&buf, r.meter, pal.ink),
            0,
            "no meter cell may claim a level"
        );
        assert!(
            count_in_rect(&buf, r.digits, pal.ghost) > 0,
            "the ghost 8s show the instrument is alive"
        );
    }
    ...
    fn ac_only_face_is_live_and_railed_not_dead() {
        ...
        assert!(
            count_in_rect(&buf, (gx, gy, gw, gh), pal.ink) > 400,
            "the AC face should read across the room"
        );
    }
    ...
    fn capacity_digits_blank_leading_positions() {
        ...
        assert_eq!(
            capacity_digits(Some(255)),
            [Some(1), Some(0), Some(0)],
            "garbage clamps to 100"
        );
    }
