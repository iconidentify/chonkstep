    ...
    if saturation(&cursor) < 48 {
        ...
        best = if saturation(&cursor) >= 8 {
            ...
            theme
                ...
                .min_by(|a, b| {
                    hue_distance(hue_degrees(a), target)
                        .total_cmp(&hue_distance(hue_degrees(b), target))
                })
                .copied()
                .unwrap_or(loudest)
        } else {
...
/// defect, not noise.
pub(crate) fn mirrored_row_edges(
    y: i32,
    h: u32,
    half_rows: u32,
    k: u32,
    ...
    for col in 0..cols {
        ...
        for k in 0..half_rows {
            ...
            frect(
                ...
                dot,
                dot_h as f32,
                if k < bottom_lit { &ink } else { &ghost },
            );
        }
    }
}
...
mod tests {
    ...
    fn every_theme_gets_a_glowing_ink_on_a_dark_glass() {
        for theme in all_themes() {
            ...
            assert!(
                lum(pal.ghost) > lum(pal.glass),
                "theme {}: ghosts sit just above the glass",
                theme.id
            );
            assert!(
                lum(pal.ink) > lum(pal.ink_dim),
                "theme {}: dim ink is dimmer than ink",
                theme.id
            );
        }
    }
    ...
    fn the_accent_follows_the_theme() {
        // System 7 variants deliberately share one terminal palette.
        let themes: Vec<_> = all_themes()
            .into_iter()
            .filter(|theme| !theme.id.starts_with("system-7"))
            .collect();
        let accents: Vec<Color> = themes.iter().map(panel_accent).collect();
    ...
    fn teal_blueprint_leds_are_actually_teal() {
        ...
        assert_eq!(
            (ink.r, ink.g, ink.b),
            (0x8F, 0xE3, 0xD2),
            "the teal cursor is the accent, verbatim"
        );
    }
        ...
        for (w, h) in [(38u32, 17u32), (79, 38), (64, 40)] {
            ...
            for dy in 0..seam {
                assert_eq!(
                    row(seam - 1 - dy),
                    row(seam + dy),
                    "{w}x{h}: rows {dy} out from the seam must mirror"
                );
            }
        }
    }
    ...
    fn dead_tile_renders_at_size_with_the_glass_present() {
        ...
        assert!(
            glass > 500,
            "expected a substantial glass area, found {glass} pixels"
        );
    }
}
