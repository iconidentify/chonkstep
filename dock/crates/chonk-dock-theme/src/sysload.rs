...
pub fn render_sysload_tile(
    ...
    let (gx, gy, gw, gh) =
        panel::draw_panel_glass(&mut pixmap, well_x, well_y, well_w, well_h, theme);
...
mod tests {
    ...
    fn count_color(buffer: &DecorationBuffer, c: Color) -> usize {
        buffer
            ...
            .0
            .iter()
            .filter(|p| (p[0], p[1], p[2]) == (c.r, c.g, c.b))
            .count()
    }
    ...
    fn the_glass_is_present_and_theme_derived() {
        for theme in all_themes() {
            ...
            assert!(
                count_color(&buffer, pal.glass) > 300,
                "theme {}: expected a substantial glass area",
                theme.id
            );
            assert!(
                count_color(&buffer, pal.ghost) > 50,
                "theme {}: expected visible ghost dots",
                theme.id
            );
        }
    }
    ...
    fn busy_lights_more_ink_than_idle() {
        ...
        let pegged = render(
            &theme,
            56,
            &[SYS_LOAD_ROWS; SYS_LOAD_COLUMNS],
            SYS_LOAD_MEM_SEGMENTS,
            false,
        );
        assert!(
    ...
    fn cpu_and_memory_read_independently() {
        ...
        assert_ne!(
            cpu_only.pixels, mem_only.pixels,
            "the two readings occupy different glass regions"
        );
        ...
        assert_eq!(
            left_half(&mem_only),
            left_half(&idle),
            "memory bar must stay out of the CPU half"
        );
    }
    ...
    fn memory_levels_fill_the_bar_bottom_up() {
        ...
        let top_ink_row = |b: &DecorationBuffer| {
            for y in 0..112u32 {
                for x in 74..112u32 {
                    ...
                    if (b.pixels[i], b.pixels[i + 1], b.pixels[i + 2])
        ...
        assert!(
            half_top > full_top,
            "a fuller bar must be lit higher up (half {half_top} vs full {full_top})"
        );
    }
    ...
    fn the_alert_frame_is_its_own_lamp() {
        ...
        assert_ne!(
            calm.pixels, alarmed.pixels,
            "the alert flag must be visible even with the bar already full"
        );
        let pal = panel::panel_palette(&theme);
        assert!(
    ...
    fn all_five_preview_states_render_distinctly() {
        ...
        for size in [56u32, 112] {
            let rendered: Vec<Vec<u8>> = states
                .iter()
                .map(|(cpu, mem, alert)| render(&theme, size, cpu, *mem, *alert).pixels)
                .collect();
            for i in 0..rendered.len() {
                for j in (i + 1)..rendered.len() {
                    assert_ne!(
                        rendered[i], rendered[j],
                        "states {i} and {j} must render distinctly at {size}px"
                    );
                }
            }
        }
