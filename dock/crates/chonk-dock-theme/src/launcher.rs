...
pub fn render_launcher_tile(
    ...
    let label_font = FontSpec {
        size: (label_h as f32 * 0.60).max(6.0),
        ..theme.titlebar.font.clone()
    };
    let label = elide_to_width(
        font_system,
        &label_font,
        name,
        face_w.saturating_sub(2 * t as u32).max(1),
    );
    paint::draw_text(
        &mut pixmap,
        font_system,
    ...
    if running {
        ...
        let rim = Color::rgb(
            (accent.r as u16 * 2 / 5) as u8,
            (accent.g as u16 * 2 / 5) as u8,
            (accent.b as u16 * 2 / 5) as u8,
        );
        paint::fill_rect(
            &mut pixmap,
            lamp_x,
...
mod tests {
    ...
    fn running_lights_the_lamp() {
        ...
        assert_ne!(
            idle.pixels, running.pixels,
            "the running lamp must visibly change the tile"
        );
    }
    ...
    fn monogram_takes_the_first_two_words_initials() {
        ...
        assert_eq!(
            monogram("GNU Image Manipulation Program"),
            "GI",
            "only the first two words vote"
        );
    }
    ...
    fn long_names_elide_into_the_label_strip_leaving_the_face_untouched() {
        ...
        assert_ne!(
            short.pixels, long.pixels,
            "the label strip itself should show different (elided) text"
        );
