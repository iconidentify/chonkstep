...
impl<B: Backend> LaunchDock<B> {
    pub fn reload(&mut self, backend: &mut B, theme: &Theme, apps: &[AppEntry]) {
        self.pins = self
            .state_path
            .as_deref()
            .map(|path| load_pins(path, apps))
            .unwrap_or_default();
        self.lit = vec![false; self.pins.len()];
    ...
    fn sync_window(&mut self, backend: &mut B, theme: &Theme) {
        ...
        let geometry = Rect {
            pos: strip_origin(self.primary, self.tile),
            size: Size::new(self.tile, self.strip_height()),
        };
        let window = match self.window {
            ...
            None => {
                match backend.create_shell_surface(geometry, crate::desktop::DESKTOP_BG, true) {
                    Some(window) => {
                        backend.set_role(window, crate::surface::Role::Launcher);
    ...
    fn repaint(&mut self, backend: &mut B, theme: &Theme) {
        ...
        backend.paint_shell_surface(
            ...
            &DecorationBuffer {
                width: self.tile,
                height: self.pins.len() as u32 * self.tile,
                pixels,
            },
        );
    }
...
fn running_lamps<W>(pins: &[AppEntry], running: &[(String, W)]) -> Vec<bool> {
    pins.iter()
        .map(|pin| {
            running
                .iter()
                .any(|(class, _)| match_window_class(std::slice::from_ref(pin), class).is_some())
        })
        .collect()
}
...
fn ghost_slot(pixels: &mut [u8], tile: u32, slot: usize) {
    ...
    let Some(region) = pixels.get_mut(start..start + slot_bytes) else {
        return;
    };
...
fn load_pins(path: &Path, apps: &[AppEntry]) -> Vec<AppEntry> {
    ...
    for id in text.lines().map(str::trim).filter(|line| !line.is_empty()) {
        match apps.iter().find(|app| app.id == id) {
            ...
            None => tracing::warn!(
                id,
                "dropping stale launcher pin with no matching .desktop entry"
            ),
        }
    }
    pins
...
mod tests {
    ...
    fn temp_state_file(tag: &str) -> PathBuf {
        ...
        std::env::temp_dir()
            .join(format!(
                "chonkstep-launchdock-{}-{tag}-{unique}",
                std::process::id()
            ))
            .join("dock")
    }
    ...
    fn persistence_format_is_one_desktop_file_id_per_line() {
        ...
        save_pins(
            &path,
            &[entry("org.mozilla.firefox"), entry("org.gnome.Calculator")],
        )
        .unwrap();
        assert_eq!(
    ...
    fn unpinning_the_last_pin_releases_the_strips_pixels_but_keeps_its_surface() {
        ...
        let primary = Rect {
            pos: Point::new(0, 0),
            size: Size::new(1920, 1200),
        };
        let mut backend = FakeBackend::new();
        let mut dock: LaunchDock<FakeBackend> = LaunchDock::new(
            &mut backend,
            &theme,
            primary,
            56,
        ...
        assert_eq!(
            dock.window,
            Some(window),
            "the surface must survive the empty spell"
        );
        assert!(
            backend.shell_geometries.contains_key(&window),
    ...
    fn a_press_becomes_a_drag_only_past_the_threshold() {
        ...
        assert!(
            !crossed_threshold(press, Point::new(22, 82), 4),
            "sub-threshold wiggle is still a click"
        );
        assert!(
            crossed_threshold(press, Point::new(24, 80), 4),
            "the threshold itself crosses, matching desktop.rs"
        );
        assert!(
            crossed_threshold(press, Point::new(20, 70), 4),
    ...
    fn slots_resolve_over_the_strip_extent_and_the_empty_strips_first_slot() {
        ...
        assert_eq!(
            slot_at(origin, 56, 3, Point::new(10, 168)),
            None,
            "below the last tile"
        );
        assert_eq!(
            slot_at(origin, 56, 3, Point::new(56, 4)),
            None,
            "right of the strip"
        );
        assert_eq!(
            slot_at(origin, 56, 3, Point::new(10, -1)),
            None,
            "above the strip is not the strip"
        );
    ...
    fn restyling_to_a_new_tile_matches_a_strip_built_at_that_tile() {
        ...
        let mut restyled: LaunchDock<FakeBackend> = LaunchDock::new(
            &mut backend,
            &theme,
            primary,
            56,
        ...
        // silently dropped.
        assert_eq!(
            restyled.primary, primary,
            "a restyle must not depend on the monitor having moved"
        );
    }
    ...
    fn the_strip_anchors_to_the_primary_monitor_not_the_desktop_origin() {
        ...
        assert_eq!(
            origin,
            Point::new(1600, 0),
            "the primary's own corner, which the Clip has vacated"
        );

        assert_eq!(
            slot_at(origin, 56, 3, Point::new(1610, 4)),
            Some(0),
            "hit-testing follows the strip"
        );
        assert_eq!(
            slot_at(origin, 56, 3, Point::new(10, 4)),
            None,
            "the other monitor is not the strip"
        );
    }
    ...
    fn move_pin_shifts_neighbors_rather_than_swapping() {
        ...
        assert_eq!(
            items,
            ["d", "c", "a", "b"],
            "an overshot target clamps to the end"
        );
    }
    ...
    fn clicks_focus_a_matching_running_window_and_launch_otherwise() {
        ...
        let running = [
            ("xterm".to_string(), 11u32),
            ("Navigator".to_string(), 42u32),
        ];
        assert!(matches!(
            resolve_click(&pin, &running),
            LaunchDockAction::Focus(42)
        ));

        // Name match, case-insensitive.
        let by_name = [("firefox".to_string(), 7u32)];
        assert!(matches!(
            resolve_click(&pin, &by_name),
            LaunchDockAction::Focus(7)
        ));

        // Nothing running that matches: launch a clone of the entry.
        let unrelated = [("xterm".to_string(), 11u32)];
        assert!(
            matches!(resolve_click(&pin, &unrelated), LaunchDockAction::Launch(app) if app == pin)
        );
    }
    ...
    fn a_pin_change_invalidates_lamps_even_when_the_client_set_did_not_change() {
        ...
        assert!(
            ...
            "startup must reconcile persisted pins once"
        );
        dock.update_running(&mut backend, &theme, &[]);
        assert!(!dock.running_refresh_needed());

        assert!(dock.try_pin_at(
            &mut backend,
            &theme,
            Point::new(10, 10),
            &entry("org.example.App")
        ));
        assert!(
            dock.running_refresh_needed(),
