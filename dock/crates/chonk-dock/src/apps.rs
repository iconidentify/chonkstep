...
pub fn scan_applications() -> Vec<AppEntry> {
    collate_scanned(
        read_desktop_sources(&xdg_application_dirs()),
        &program_on_path,
    )
}
...
pub fn match_window_class(entries: &[AppEntry], wm_class: &str) -> Option<usize> {
    entries
        ...
        .or_else(|| {
            entries
                .iter()
                .position(|e| e.name.eq_ignore_ascii_case(wm_class))
        })
        .or_else(|| {
            entries.iter().position(|e| {
                e.exec
                    .first()
                    .is_some_and(|argv0| basename(argv0).eq_ignore_ascii_case(wm_class))
            })
        })
}
    ...
    for (rank, dir) in dirs.iter().enumerate() {
        ...
        for path in paths {
            if path.is_dir() {
                ...
                let Ok(sub_reader) = fs::read_dir(&path) else {
                    continue;
                };
...
fn parse_with_lookup(
    ...
    let exec: Vec<String> = words
        .iter()
        .filter_map(|word| strip_field_codes(word))
        .collect();
    if exec.is_empty() {
...
fn program_on_path(program: &str) -> bool {
    ...
    path_var
        ...
        .any(|dir| is_executable_file(&Path::new(dir).join(program)))
}
...
mod tests {
    ...
    fn duplicate_keys_take_the_first_occurrence() {
        let text = "[Desktop Entry]\nType=Application\nName=First\nName=Second\nExec=x\n";
        assert_eq!(
            parse_desktop_entry("t", text).expect("should parse").name,
            "First"
        );
    }
    ...
    fn missing_type_is_skipped() {
        assert_eq!(
            parse_desktop_entry("t", "[Desktop Entry]\nName=X\nExec=x\n"),
            None
        );
    }
    ...
    fn nodisplay_true_is_skipped_but_false_is_not() {
        assert_eq!(
            parse_desktop_entry("t", &app_fixture("NoDisplay=true")),
            None
        );
    ...
    fn any_onlyshowin_value_is_skipped() {
        ...
        assert_eq!(
            parse_desktop_entry("t", &app_fixture("OnlyShowIn=GNOME;")),
            None
        );
    ...
    fn every_spec_field_code_is_stripped() {
        assert_eq!(
            exec_of("Exec=app %f %F %u %U %d %D %n %N %i %c %k %v %m end"),
            argv(&["app", "end"])
        );
    }
    ...
    fn mid_word_field_codes_are_stripped_in_place() {
        assert_eq!(
            exec_of("Exec=app --file=%f --url=%uX"),
            argv(&["app", "--file=", "--url=X"])
        );
    }
    ...
    fn a_missing_categories_key_maps_to_other() {
        assert_eq!(
            parse_desktop_entry("t", &app_fixture(""))
                .expect("should parse")
                .category,
            AppCategory::Other
        );
    ...
    fn entries_sort_by_name_case_insensitively() {
        ...
        let names: Vec<String> = collate_scanned(sources, &|_| true)
            .into_iter()
            .map(|e| e.name)
            .collect();
        assert_eq!(
    ...
    fn the_walk_reads_one_subdirectory_level_and_joins_ids_with_dashes() {
        ...
        fs::write(
            dir.join("extras").join("deeper").join("gamma.desktop"),
            "gamma text",
        )
    ...
    fn startup_wm_class_matches_first_and_case_insensitively() {
        let entries = [
            entry("Alpha", "alpha-bin", None),
            entry("Beta", "/usr/bin/beta", Some("BetaWindow")),
        ];
    ...
    fn a_later_startup_wm_class_beats_an_earlier_name_coincidence() {
        ...
        let entries = [
            entry("firefox", "other-bin", None),
            entry("Mozilla Firefox", "firefox-bin", Some("firefox")),
        ];
    ...
    fn name_matches_when_no_startup_wm_class_does() {
        let entries = [
            entry("Alpha", "alpha-bin", None),
            entry("Beta", "beta-bin", Some("Unrelated")),
        ];
