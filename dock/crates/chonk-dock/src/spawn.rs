...
mod tests {
    ...
    fn qt_scale_factor_carries_the_exact_requested_scale() {
        for scale in [1.0f32, 1.5, 2.0, 3.0, 2.25] {
            ...
            let qt_scale: f32 = env
                .iter()
                .find(|(k, _)| k == "QT_SCALE_FACTOR")
                .unwrap()
                .1
                .parse()
                .unwrap();
            assert!(
                (qt_scale - scale).abs() < 0.01,
                "scale {scale}: QT_SCALE_FACTOR should reconstruct it exactly"
            );
        }
    }
    ...
    fn a_dockapps_launch_environment_has_no_display_server_in_it() {
        ...
        apply_env(
            &mut command,
            &[(
                "CHONKSTEP_DOCK_SOCKET".to_string(),
                "/run/user/1000/chonkstep/dock-1.sock".to_string(),
            )],
            &DISPLAY_SERVER_ENV,
        );
        ...
        for variable in DISPLAY_SERVER_ENV {
            ...
            assert_eq!(
                ...
                Some(None),
                "{variable} must be removed, not merely left unset"
            );
        }
        assert!(
            envs.iter().any(
                |(key, value)| *key == std::ffi::OsStr::new("CHONKSTEP_DOCK_SOCKET")
                    && value.is_some()
            ),
    ...
    fn a_declared_control_socket_reaches_every_launch_but_a_dockapps() {
        ...
        declare_control_socket(std::path::PathBuf::from(
            "/run/user/1000/chonkstep/control-test.sock",
        ));
        let mut launch = Command::new("/bin/true");
        apply_env(&mut launch, &[], &[]);
        let exported = launch
            .get_envs()
            .find(|(key, _)| *key == std::ffi::OsStr::new(CONTROL_SOCKET_ENV));
        assert_eq!(
        ...
        let withheld = dockapp
            .get_envs()
            .find(|(key, _)| *key == std::ffi::OsStr::new(CONTROL_SOCKET_ENV));
        assert_eq!(
            withheld.map(|(_, value)| value),
            Some(None),
            "a dockapp has it removed, not merely unset"
        );
    }
    ...
    fn ordinary_launches_keep_public_environment_but_drop_internal_controls() {
        ...
        for variable in ["QT_SCALE_FACTOR", "QT_AUTO_SCREEN_SCALE_FACTOR"] {
            ...
            assert!(
                ...
                "{variable} must still be supplied"
            );
        }
        for variable in INTERNAL_ENV {
            ...
            assert_eq!(
                entry.map(|(_, value)| *value),
                Some(None),
                "{variable} must be removed from every child"
            );
        }
    }
    ...
    fn the_compositors_own_image_launches_browsers_as_native_wayland_clients() {
        ...
        assert_eq!(
            resolve_display_stack(Some("chonkstep-wayland"), Some("wayland-1")),
            DisplayStack::Wayland
        );
        assert_eq!(
            chromium_platform_args(DisplayStack::Wayland),
            vec!["--ozone-platform=wayland".to_string()]
        );
    }
    ...
    fn a_rebuilt_binary_is_still_the_compositor() {
        ...
        assert_eq!(
            resolve_display_stack(Some("chonkstep (deleted)"), Some("wayland-1")),
            DisplayStack::X11
        );
        // And the suffix is only ever stripped from the end, so it
        // cannot smuggle an unrelated name into a match.
        assert_eq!(
            resolve_display_stack(Some("chonkstep-wayland (deleted) x"), Some("wayland-1")),
            DisplayStack::X11
        );
    }
    ...
    fn a_declared_stack_beats_the_deduction() {
        ...
        assert_eq!(
            stack_with_declaration(
                Some(DisplayStack::Wayland),
                Some("chonkstep-wayland (deleted)"),
                None
            ),
            DisplayStack::Wayland,
        ...
        assert_eq!(
            stack_with_declaration(
                Some(DisplayStack::X11),
                Some("chonkstep-wayland"),
                Some("wayland-1")
            ),
            DisplayStack::X11,
        ...
        assert_eq!(
            stack_with_declaration(None, Some("chonkstep"), Some("wayland-1")),
            DisplayStack::X11
        );
    }
    ...
    fn the_x11_window_manager_still_pins_x11_even_from_inside_a_wayland_desktop() {
        ...
        assert_eq!(
            resolve_display_stack(Some("chonkstep"), Some("wayland-1")),
            DisplayStack::X11
        );
        assert_eq!(
            resolve_display_stack(Some("chonkstep"), None),
            DisplayStack::X11
        );
        assert_eq!(
            chromium_platform_args(DisplayStack::X11),
    ...
    fn a_compositor_with_no_socket_in_its_environment_declines_to_send_a_browser_at_one() {
        ...
        assert_eq!(
            resolve_display_stack(Some("chonkstep-wayland"), None),
            DisplayStack::X11
        );
        assert_eq!(
            resolve_display_stack(Some("chonkstep-wayland"), Some("")),
            DisplayStack::X11
        );
    }
    ...
    fn an_unrecognized_image_gets_the_answer_that_used_to_ship_unconditionally() {
        ...
        assert_eq!(
            resolve_display_stack(None, Some("wayland-1")),
            DisplayStack::X11
        );
        assert_eq!(
            resolve_display_stack(Some("spawn-8f3c1d2e"), Some("wayland-1")),
            DisplayStack::X11
        );
    }
}
