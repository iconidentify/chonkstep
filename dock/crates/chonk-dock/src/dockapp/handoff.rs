...
pub(crate) fn write(path: &Path, tokens: &[(String, [u8; TOKEN_BYTES])]) {
    ...
    match written {
        Ok(()) => tracing::info!(
            count = tokens.len(),
            "handed dockapp tokens to the incoming shell; those tiles keep running"
        ),
        Err(error) => {
...
pub(crate) fn take(path: &Path) -> HashMap<String, [u8; TOKEN_BYTES]> {
    ...
    for line in text.lines() {
        ...
        let Some(token) = transport::token_from_hex(hex) else {
            continue;
        };
        if id.is_empty() {
            continue;
...
mod tests {
    ...
    impl Scratch {
        fn new() -> Self {
            ...
            let dir =
                std::env::temp_dir().join(format!("chonk-handoff-{}-{unique}", std::process::id()));
    ...
    fn a_token_survives_the_restart_and_the_file_does_not() {
        ...
        write(
            &scratch.file(),
            &[("clock".into(), token(1)), ("net".into(), token(2))],
        );
        ...
        assert!(
            !scratch.file().exists(),
            "consumed on read: a file that survived would be re-read at the next restart"
        );
        assert!(
            take(&scratch.file()).is_empty(),
            "and a second read finds nothing"
        );
    }
    ...
    fn the_file_is_private_to_this_user() {
        ...
        assert_eq!(
            mode, 0o600,
            "a credential file must not be readable by group or other"
        );
    }
    ...
    fn nothing_to_hand_off_removes_a_stale_file_rather_than_leaving_it() {
        ...
        assert!(
            take(&scratch.file()).is_empty(),
            "a shell with no dockapps must not resurrect the last shell's list"
        );
    }
    ...
    fn a_torn_line_costs_one_tile_its_adoption_and_not_the_file() {
        ...
        std::fs::write(
            ...
            format!(
                "clock {}\nnet not-hex\nno-space-here\n\nsound {}\n",
                transport::token_to_hex(&token(1)),
                transport::token_to_hex(&token(2))
            ),
        )
        .unwrap();
        let taken = take(&scratch.file());
    ...
    fn the_handoff_lives_beside_the_socket_it_authenticates() {
        ...
        assert_eq!(
            beside(Path::new("")),
            None,
            "a session that could not bind has nothing to hand over"
        );
    }
}
