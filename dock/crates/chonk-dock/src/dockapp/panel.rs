...
pub(crate) fn place(
    ...
    Rect {
        pos: Point::new(x.max(workarea.pos.x), y),
        size,
    }
}
...
impl<B: Backend> InstrumentPanel<B> {
    ...
    pub(crate) fn content_point(&self, theme: &Theme, local: Point) -> Option<Point> {
        ...
        (x >= 0 && y >= 0 && (x as u32) < self.content.0 && (y as u32) < self.content.1)
    ...
    pub(crate) fn show(
        ...
        if self.window.is_none() {
            self.window = backend.create_shell_surface(
                geometry,
                wm_theme::switcher::panel_background(theme),
                true,
            );
            if self.window.is_none() {
...
mod tests {
    ...
    fn a_panel_sits_flush_against_the_dock_and_level_with_its_tile() {
        ...
        assert_eq!(
            rect.pos.x + rect.size.w as i32,
            dock.pos.x,
            "flush against the dock strip"
        );
    ...
    fn closing_a_panel_releases_its_raster_and_reopening_reuses_the_surface() {
        ...
        assert_eq!(
            panel.window,
            Some(window),
            "reopening must reuse the surface"
        );
        assert!(
            backend
                .shell_buffer_bytes
    ...
    fn a_panel_for_a_low_tile_is_clamped_into_the_workarea() {
        ...
        assert_eq!(
            rect.pos.y + rect.size.h as i32,
            1080,
            "pulled up so it stays fully on screen"
        );
    ...
    fn the_chrome_is_a_border_around_untouched_content_pixels() {
        ...
        assert_eq!(
            &buffer.pixels[at..at + 4],
            &[0x00, 0xFF, 0x00, 0xFF],
            "streamed pixels are blitted, not restyled"
        );
        ...
        assert_ne!(
            &buffer.pixels[0..3],
            &[0x00, 0xFF, 0x00],
            "the border is the shell's, not the client's"
        );
    }
    ...
    fn a_wrong_sized_frame_is_never_blitted_into_the_chrome() {
        ...
        let theme = theme();
        let inset = chrome_inset(&theme);
        let wrong = DecorationBuffer {
            width: 10,
            height: 10,
            pixels: [0xFF, 0x00, 0x00, 0xFF].repeat(100),
        };
        ...
        assert_ne!(
            &buffer.pixels[at..at + 4],
            &[0xFF, 0x00, 0x00, 0xFF],
            "a stale-grant frame is refused, not drawn"
        );
    }
}
