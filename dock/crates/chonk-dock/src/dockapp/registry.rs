...
impl std::fmt::Display for RejectReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RejectReason::MalformedId => {
                write!(f, "`id` must be 1-64 characters of [A-Za-z0-9._-]")
            }
            RejectReason::ReservedId => write!(
                f,
                "`id` may not begin with the reserved `{BUILTIN_PREFIX}` prefix"
            ),
            RejectReason::EmptyExec => write!(
                f,
                "`exec` is empty; it must be an argv array whose first element is the program"
            ),
            RejectReason::BadTileUnits(units) => {
                write!(f, "`tile_units` is {units}, outside 1..={MAX_TILE_UNITS}")
            }
        }
    }
}
...
pub(crate) fn parse(text: &str, source: &Path) -> Result<DockappEntry, RejectReason> {
    let raw: RawDockapp =
        toml::from_str(text).map_err(|error| RejectReason::Unparsable(error.to_string()))?;
    ...
    let name = raw
        .name
        .unwrap_or_else(|| raw.id.to_uppercase().chars().take(5).collect());
    Ok(DockappEntry {
        id: raw.id,
        name,
        exec: raw.exec,
...
mod tests {
    ...
    fn a_minimal_registration_gets_the_documented_defaults() {
        ...
        assert_eq!(
            entry.name, "CLOCK",
            "the label falls back to the id, clipped to what the tile face can draw"
        );
    }
    ...
    fn a_malformed_id_is_rejected_here_rather_than_at_the_handshake() {
        ...
        // forever for one.
        assert_eq!(
            parsed("id = \"has spaces\"\nexec = [\"x\"]\n"),
            Err(RejectReason::MalformedId)
        );
        assert_eq!(
            parsed("id = \"../../etc/passwd\"\nexec = [\"x\"]\n"),
            Err(RejectReason::MalformedId)
        );
        assert_eq!(
            parsed("id = \"\"\nexec = [\"x\"]\n"),
    ...
    fn a_file_that_is_not_toml_says_so_rather_than_panicking() {
        assert!(matches!(
            parsed("this is not toml at all ]["),
            Err(RejectReason::Unparsable(_))
        ));
        assert!(
            matches!(parsed(""), Err(RejectReason::Unparsable(_))),
            "a file with no id and no exec is unparsable, not empty-id"
        );
    }
