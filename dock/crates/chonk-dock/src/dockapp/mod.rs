...
//! [`Desktop::extra_poll_fds`](crate::desktop::Desktop::extend_extra_poll_fds)
...
fn usable_scale(scale: f32) -> f32 {
    ...
    tracing::error!(
        scale,
        "the dock's scale is not a number a tile can be drawn at; telling dockapps 1.0 instead"
    );
    1.0
}
    ...
    pub(crate) fn new(display: &str) -> Self {
        let socket_path = match chonk_dock_proto::transport::socket_path(display) {
            ...
            Err(error) => {
                tracing::warn!(
                    ?error,
                    "no dockapp socket path; dockapps are unavailable this session"
                );
            ...
            match listener.accept() {
                Ok(Some(socket)) => {
                    ...
                    match socket.peer_is_this_user() {
                        ...
                        Ok(false) => {
                            tracing::warn!("refused a dockapp connection from another user")
                        }
                        Err(error) => tracing::warn!(
                            ?error,
                            "could not read a dockapp connection's peer credentials; refusing it"
                        ),
                    }
                }
                Ok(None) => return,
        ...
        for pending in std::mem::take(&mut self.pending) {
            match pending.socket.recv(&mut self.scratch) {
                ...
                Ok(n) => match ClientMessage::decode(&self.scratch[..n]) {
                    Ok(hello @ ClientMessage::Hello { .. }) => admissions.push(Admission {
...
mod tests {
    ...
    fn theme(id: &str) -> Theme {
        let mut theme = wm_theme::default_theme::all_themes()
            .into_iter()
            .next()
    ...
    fn the_serialized_theme_is_produced_once_and_reused() {
        ...
        assert!(
            !first.theme_toml.is_empty(),
            "the correctness path carries a real palette, not an empty string"
        );

        let before = broadcast.state().theme_toml.as_ptr();
        broadcast.refresh(56, 1.0, &theme("nextstep-classic"));
        assert_eq!(
            broadcast.state().theme_toml.as_ptr(),
            before,
            "an unchanged theme must not be re-serialized"
        );
    ...
    fn every_input_that_a_dockapp_can_see_change_triggers_a_refresh() {
        ...
        assert!(
            !broadcast.state().same_as(&start),
            "a relayout changes the tile edge"
        );
        ...
        assert!(
            !broadcast.state().same_as(&start),
            "...different palette, and the dockapp has to be told"
        );
    }
    ...
    fn a_scale_no_tile_can_be_drawn_at_never_reaches_the_wire() {
        ...
        for bad in [
            ...
            -1.0,
            1e30,
            chonk_dock_proto::MAX_SCALE + 0.1,
        ] {
            let mut broadcast = ThemeBroadcast::new();
            broadcast.refresh(56, bad, &theme("nextstep-classic"));
            let bytes = ServerMessage::Welcome(broadcast.state().clone())
                .encode()
                .expect("encodable");
            assert!(
                ServerMessage::decode(&bytes).is_ok(),
                "the shell must not send a scale of {bad}, which its own peer would refuse"
            );
        ...
        for good in [0.5f32, 1.0, 1.5, 2.0, chonk_dock_proto::MAX_SCALE] {
            ...
            assert_eq!(
                broadcast.state().scale,
                good,
                "a real session's scale is passed through untouched"
            );
        }
    }
}
...
mod panel_wire_tests {
    ...
    impl Scratch {
        fn new() -> Self {
            ...
            let dir = std::env::temp_dir()
                .join(format!("chonk-panel-wire-{}-{unique}", std::process::id()));
    ...
    impl Harness {
        fn start(socket: &Path, now: Instant) -> Self {
            ...
            Self {
                host,
                tile: RemoteTile::new(entry, 56, now),
                scratch: Vec::new(),
                theme,
            }
        }
    ...
    fn a_scripted_dockapp_opens_streams_receives_input_and_is_dismissed() {
        ...
        peer.send(&hello("gauge", 1, token, InputMask::all()).encode().unwrap())
            .unwrap();
        shell.pass(base);
        let welcome = drain(&peer)
            ...
            .expect("welcomed");
        assert!(
            welcome.panels_supported(),
            "the Welcome advertises protocol {} — the probe a client reads before OpenPanel",
            welcome.proto
        );
        ...
        let press = InputEvent {
            ...
            button: Some(chonk_dock_proto::wire::Button::Left),
            x: 700,
            y: 90,
            delta: 0,
        };
        ...
        assert_eq!(
            drain(&peer),
            vec![
                ServerMessage::PanelInput(press),
                ServerMessage::PanelInput(motion)
            ],
        ...
        assert!(
            shell.tile.poll_fd().is_some(),
            "the dismissal cost the panel, not the tile"
        );
        assert_eq!(
            shell.tile.state(),
            TileState::Live,
            "which is still drawing"
        );

        // -- and a late band for the closed panel is quietly nothing ----
        let late = ClientMessage::PanelFrame {
            generation: 2,
            y: 0,
            band_height: 1,
            width: 800,
        ...
        assert!(
            shell.tile.poll_fd().is_some(),
            "streaming against an unseen PanelClosed is a race, not a crime"
        );
...
mod v1_compat_wire_tests {
    ...
    impl Scratch {
        fn new() -> Self {
            let unique = COUNTER.fetch_add(1, Ordering::Relaxed);
            let dir = std::env::temp_dir()
                .join(format!("chonk-v1-compat-{}-{unique}", std::process::id()));
            std::fs::create_dir_all(&dir).unwrap();
            std::fs::set_permissions(&dir, std::os::unix::fs::PermissionsExt::from_mode(0o700))
                .unwrap();
            Self(dir)
    ...
    fn strict_v1_decode(buf: &[u8]) -> Result<V1Message, String> {
        ...
        match buf[0] {
            kind @ (0x81 | 0x82) => {
                ...
                if reserved != 0 {
                    return Err(format!(
                        "reserved field at body offset 10 was not zero (got {reserved})"
                    ));
                }
                ...
                let theme_id = std::str::from_utf8(&buf[20..20 + theme_id_len])
                    .map_err(|_| "bad utf-8")?
                    .to_string();
                Ok(if kind == 0x81 {
                    V1Message::Welcome {
                        proto_field: reserved,
                        theme_id,
                    }
                } else {
                    V1Message::ThemeChanged {
                        proto_field: reserved,
                        theme_id,
                    }
                })
    ...
    fn drain_strict(peer: &Seqpacket) -> (Vec<V1Message>, bool) {
        ...
        loop {
            match peer.recv(&mut buffer) {
                ...
                Ok(n) => messages.push(strict_v1_decode(&buffer[..n]).unwrap_or_else(|error| {
                    panic!("the strict v1 decoder rejected a shell message: {error}")
                })),
        ...
        let welcome = messages
            ...
            .find_map(|m| match m {
                V1Message::Welcome {
                    proto_field,
                    theme_id,
                } => Some((*proto_field, theme_id.clone())),
                _ => None,
            })
            .expect("a Welcome the strict decoder accepted");
        assert_eq!(
            welcome,
            (0, "nextstep-classic".to_string()),
            "reserved zero on the wire, exactly the v1 layout"
        );
        assert!(
            !messages.iter().any(|m| matches!(m, V1Message::Goodbye(_))),
            "and no refusal"
        );
    ...
    fn a_version_2_hello_is_told_the_shells_real_version() {
        ...
        let peer = Seqpacket::connect(&scratch.socket()).expect("connect");
        peer.send(
            &chonk_dock_proto::handshake::hello("wmnet", 1, token, InputMask::all())
                .encode()
                .unwrap(),
        )
        .unwrap();
        ...
        assert!(
            welcome.panels_supported(),
            "which is the panel probe a v2 client reads"
        );
    }
...
mod restart_tests {
    ...
    impl FakeShell {
        ...
        fn start(socket: &Path, ids: &[&str], now: Instant) -> Self {
            let host = DockHost::bind_at(socket.to_path_buf());
            let mut inherited = host
                .handoff_path()
                .map(|path| handoff::take(&path))
                .unwrap_or_default();
            let tiles = ids
                .iter()
                .map(|id| {
    ...
    fn dockapp_connects(socket: &Path, id: &str, token: [u8; TOKEN_BYTES]) -> Seqpacket {
        ...
        peer.send(&hello(id, 1, token, InputMask::all()).encode().unwrap())
            .expect("Hello");
        peer
    }
    ...
    fn a_dockapp_survives_the_shells_own_restart_and_is_readopted() {
        ...
        assert!(
            welcomed_with(&peer).is_some(),
            "the dockapp is admitted and welcomed"
        );
        ...
        assert_eq!(
            goodbye_reason(&parting),
            None,
            "a bare EOF means \"try again\"; a Goodbye would have told it the opposite"
        );
        drop(peer);
        ...
        assert!(
            matches!(second.tiles[0].state(), TileState::Rejoining { .. }),
            "and nothing is launched while it waits"
        );
        ...
        assert!(
            second.tiles[0].poll_fd().is_some(),
            "readopted into the existing tile"
        );
        assert!(
            second.tiles[0].pid().is_none(),
            "and no second process was ever spawned"
        );
        assert_eq!(
        ...
        assert_eq!(
            second.tiles[0].state(),
            TileState::Live,
            "the same process, drawing into the same slot"
        );
    }
    ...
    fn a_session_that_ends_stops_its_dockapps_and_leaves_no_tokens_behind() {
        ...
        assert_eq!(
            goodbye_reason(&drain(&peer).0),
            Some(GoodbyeReason::Shutdown)
        );

        let handoff = shell.host.handoff_path().unwrap();
        assert!(
            !handoff.exists(),
            "a logout must not leave credentials for a session that is over"
        );
        drop(shell);

        let next = FakeShell::start(&socket, &["clock"], base);
        assert!(
            matches!(next.tiles[0].state(), TileState::Waiting { .. }),
            "the next login launches its own, immediately"
        );
    }
    ...
    fn a_valid_token_does_not_displace_a_dockapp_that_is_already_connected() {
        ...
        assert_eq!(
            shell.tiles[0].poll_fd(),
            held,
            "and the tile that was working keeps working"
        );
    ...
    fn a_held_open_slot_still_refuses_a_token_it_did_not_inherit() {
        ...
        assert_eq!(
            goodbye_reason(&drain(&impostor).0),
            Some(GoodbyeReason::Unauthorized)
        );
        assert!(
            shell.tiles[0].poll_fd().is_none(),
            "the slot was not given away"
        );
        assert!(
            matches!(shell.tiles[0].state(), TileState::Rejoining { .. }),
            "and is still being held for the one that can prove it"
        );
    ...
    fn a_hung_dockapp_is_stopped_by_a_restart_rather_than_handed_forward() {
        ...
        assert!(
            !handoff.exists(),
            "nothing was handed forward, so the next shell launches a live tile instead"
        );
    }
    ...
    fn a_handoff_is_spent_by_the_shell_that_reads_it() {
        ...
        assert!(
            matches!(second.tiles[0].state(), TileState::Waiting { .. }),
            "the token was spent; this tile launches its own"
        );
    }
    ...
    fn a_handoff_for_an_id_that_is_no_longer_registered_is_ignored() {
        ...
        shell.pass(base, &at_56);
        assert_eq!(
            goodbye_reason(&drain(&survivor).0),
            Some(GoodbyeReason::Unauthorized)
        );
    }
}
