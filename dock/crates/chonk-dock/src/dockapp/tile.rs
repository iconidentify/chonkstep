...
use chonk_dock_proto::wire::{
    frame_matches_tile, Button, InputEvent, InputKind, InputMask, LogLevel, PanelCloseReason,
    ThemeState,
};
use chonk_dock_proto::{
    transport, ClientMessage, FrameLimiter, SendOutcome, SendQueue, ServerMessage, TOKEN_BYTES,
};
...
impl LaunchBudget {
    ...
    pub(crate) fn record_failure(&mut self, now: Instant, stable: bool) -> Fate {
        while self
            .failures
            .front()
            .is_some_and(|at| now.saturating_duration_since(*at) >= CRASH_LOOP_WINDOW)
        {
        ...
        Fate::Retry {
            at: now + Duration::from_secs(Self::BACKOFF_SECONDS[step]),
        }
    }
...
impl RemoteTile {
    ...
    pub(crate) fn next_service_deadline(&self, now: Instant) -> Option<Instant> {
        ...
        if let Some(panel) = &self.panel {
            if panel.dirty {
                consider(
                    panel
                        .last_present
                        .map_or(now, |presented| presented + PANEL_PRESENT_INTERVAL),
                );
            }
        }
        ...
        match self.state {
            ...
            TileState::Starting { since } if self.connection.is_none() => {
                consider(since + HANDSHAKE_GRACE)
            }
            TileState::Starting { .. }
            | TileState::Live
            | TileState::Hung { .. }
    ...
    pub(crate) fn awaiting_hello(&self) -> bool {
        matches!(
            self.state,
            TileState::Starting { .. } | TileState::Rejoining { .. }
    ...
    pub(crate) fn rejoin(&mut self, token: [u8; TOKEN_BYTES], now: Instant) {
        ...
        self.token = token;
        self.state = TileState::Rejoining {
            until: now + REJOIN_WINDOW,
        };
        self.dirty = true;
    }
    ...
    pub(crate) fn hand_off(&mut self) -> Option<(String, [u8; TOKEN_BYTES])> {
        let worth_keeping = matches!(
            self.state,
            TileState::Starting { .. } | TileState::Live | TileState::Rejoining { .. }
        );
        if !worth_keeping {
            return None;
        }
    ...
    pub(crate) fn adopt(
        ...
        for message in [
            ServerMessage::Welcome(welcome.for_client(client_proto)),
    ...
    fn handle(&mut self, message: ClientMessage, ctx: &ServiceContext) -> bool {
        ...
        let panel_family = matches!(
            message,
            ClientMessage::OpenPanel { .. }
                | ClientMessage::PanelFrame { .. }
        ...
        if panel_family
            && self
                .connection
                .as_ref()
                .is_some_and(|connection| connection.proto < 2)
        {
        ...
        match message {
            ClientMessage::Frame {
                ...
                height,
                pixels,
            } => self.on_frame(generation, width, height, pixels, ctx),
    ...
    fn on_open_panel(&mut self, width: u32, height: u32, ctx: &ServiceContext) {
        match clamp_panel_grant(width, height, ctx.panel_bounds) {
            ...
            None => {
                ...
                self.enqueue(
                    ServerMessage::PanelClosed {
                        reason: PanelCloseReason::Refused,
                    },
                    ctx.now,
                );
                self.flush(ctx.now);
    ...
    fn check_liveness(&mut self, now: Instant) {
        ...
        // nothing else at all.
        if unanswered >= UNANSWERED_PINGS_BEFORE_HUNG
            && !matches!(self.state, TileState::Hung { .. })
        {
            tracing::warn!(
                id = %self.entry.id,
                pid = ?self.pid(),
    ...
    fn push_theme(&mut self, ctx: &ServiceContext) {
        ...
        // like.
        let client_proto = self
            .connection
            .as_ref()
            .map_or(0, |connection| connection.proto);
        self.enqueue(
            ServerMessage::ThemeChanged(next.for_client(client_proto)),
            ctx.now,
        );
    }
    ...
    fn flush(&mut self, now: Instant) {
        ...
        let result = connection
            .send
            .flush(|message| connection.socket.send(message));
    ...
    fn disconnected(&mut self, now: Instant, why: &str) {
        let stable = self.connection.as_ref().is_some_and(|connection| {
            now.saturating_duration_since(connection.since) >= CRASH_LOOP_WINDOW
        });
        self.connection = None;
        ...
        match self.budget.record_failure(now, stable) {
            ...
            Fate::CrashLooped => {
                ...
                self.state = TileState::Stopped {
                    reason: StopReason::CrashLooped,
                };
            }
        }
    }
    ...
    fn maybe_launch(&mut self, ctx: &ServiceContext) {
        match self.state {
            ...
            TileState::Starting { since }
                if self.connection.is_none()
                    && ctx.now.saturating_duration_since(since) >= HANDSHAKE_GRACE =>
            {
    ...
    fn launch(&mut self, ctx: &ServiceContext) {
        ...
        let mut env = vec![
            ...
            (
                "CHONKSTEP_SCALE".to_string(),
                format!("{:.4}", ctx.theme.scale),
            ),
    ...
    pub(crate) fn panel_frame(&self) -> Option<&DecorationBuffer> {
        self.panel
            .as_ref()
            .filter(|panel| panel.streamed)
            .map(|panel| &panel.buffer)
    }
    ...
    pub(crate) fn take_panel_just_opened(&mut self) -> bool {
        self.panel
            .as_mut()
            .map(|panel| std::mem::take(&mut panel.just_opened))
            .unwrap_or(false)
    }
    ...
    pub(crate) fn take_panel_ready(&mut self, now: Instant) -> bool {
        ...
        if panel
            .last_present
            .is_some_and(|at| now.saturating_duration_since(at) < PANEL_PRESENT_INTERVAL)
        {
            return false;
        }
        panel.dirty = false;
...
fn wash_toward(buffer: &mut DecorationBuffer, toward: Color, mix: u16) {
    let blend = |channel: u8, target: u8| -> u8 {
        ((channel as u16 * (255 - mix) + target as u16 * mix) / 255) as u8
    };
...
fn dead_face(
    ...
    let Some(mut pixmap) = tiny_skia::Pixmap::new(tile.max(1), height.max(1)) else {
        return DecorationBuffer {
            width: tile,
            height,
            pixels: vec![0; (tile * height * 4) as usize],
        };
    };
...
fn mark_stopped(buffer: &mut DecorationBuffer) {
    let Some(mut pixmap) = tiny_skia::Pixmap::new(buffer.width.max(1), buffer.height.max(1)) else {
        return;
    };
...
impl DockWidget for RemoteTile {
    ...
    fn render(
        ...
        let frame = self
            .last_frame
            .as_ref()
            .filter(|frame| (frame.width, frame.height) == expected);
    ...
    fn on_input(&mut self, input: DockInput, tile: u32) -> Vec<Effect> {
        ...
        match self.state {
            ...
            TileState::Waiting { .. }
            | TileState::Starting { .. }
            | TileState::Rejoining { .. } => return Vec::new(),
        }

        let Some(event) = wire_event(&input) else {
            return Vec::new();
        };
        let wants = self
            .connection
            .as_ref()
            .map(|connection| connection.wants)
            .unwrap_or_else(InputMask::none);
    ...
    match *input {
        DockInput::Press { local, button } => {
            ...
            Some(InputEvent {
                ...
                button: Some(reserved_filter(button)?),
                x,
                y,
                delta: 0,
            })
        ...
        DockInput::Enter => Some(InputEvent {
        ...
        DockInput::Leave => Some(InputEvent {
...
mod tests {
    ...
    fn theme() -> Theme {
        wm_theme::default_theme::all_themes()
            .into_iter()
            .next()
            .expect("the theme set is never empty")
    }
    ...
    fn the_backoff_caps_at_thirty_seconds() {
        ...
        assert_eq!(
            last, 30,
            "a tile still failing after ten tries retries every 30s, not every 17 minutes"
        );
    }
    ...
    fn five_failures_in_a_minute_stop_the_tile_permanently() {
        ...
        for step in 0..LaunchBudget::MAX_FAILURES - 1 {
            ...
            assert!(
                matches!(budget.record_failure(now, false), Fate::Retry { .. }),
                "failure {step} is still a retry"
            );
        }
    ...
    fn a_long_lived_connection_resets_the_backoff_but_not_the_window() {
        ...
        assert_eq!(
            budget.record_failure(now, true),
            Fate::Retry {
                at: now + Duration::from_secs(1)
            }
        );
        assert_eq!(
            budget.recent_failures(),
            4,
            "a stable run forgives the backoff, never the window"
        );
    }
    ...
    fn failures_spread_beyond_the_window_never_trip_the_cutoff() {
        ...
        for step in 0..50 {
            ...
            assert!(
                matches!(budget.record_failure(now, false), Fate::Retry { .. }),
                "a tile that fails once an hour is not a crash loop"
            );
        }
    }
    ...
    fn the_user_asking_again_clears_everything() {
        ...
        assert_eq!(
            budget.record_failure(now, false),
            Fate::Retry {
                at: now + Duration::from_secs(1)
            },
            "and the backoff starts over at one second"
        );
    }
    ...
    fn time_driven_tile_states_publish_the_deadline_that_services_them() {
        ...
        assert_eq!(
            tile.next_service_deadline(now),
            Some(now),
            "the initial launch is due immediately"
        );
        ...
        tile.state = TileState::Stopped {
            reason: StopReason::PolicyNever,
        };
        assert_eq!(
            tile.next_service_deadline(now),
            None,
            "a stopped tile cannot change merely because time passed"
        );
    }
    ...
    fn a_parked_frame_shortens_idle_sleep_to_the_limiter_refill() {
        ...
        assert!(
            tile.limiter.offer(frame(), now).is_none(),
            "the next frame is parked, not dropped"
        );
        ...
        let deadline = tile
            ...
            .expect("a parked frame owns a wakeup");
        assert!(deadline > now);
        assert!(
            deadline <= now + Duration::from_millis(34),
            "30 Hz refill is due after about 33.3 ms"
        );
    }
    ...
    fn the_hung_wash_moves_a_frame_halfway_toward_the_tile_fill_and_leaves_alpha_alone() {
        let mut buffer = DecorationBuffer {
            width: 1,
            height: 1,
    ...
    fn a_hung_tile_does_not_draw_the_same_pixels_as_a_live_one() {
        ...
        tile.last_frame = Some(DecorationBuffer {
            width: 56,
            height: 56,
            pixels: vec![128; 56 * 56 * 4],
        });
        ...
        assert_ne!(
            hung.pixels, live.pixels,
            "a hung tile must be visibly distinguishable from a live one"
        );
    }
    ...
    fn a_frame_of_the_wrong_size_is_never_drawn() {
        ...
        assert_eq!(
            ...
            (56, 56),
            "the dock's slot is 56px whatever the dockapp sent"
        );
    ...
    fn a_relayout_drops_a_frame_that_no_longer_fits() {
        ...
        tile.last_frame = Some(DecorationBuffer {
            width: 56,
            height: 56,
            pixels: vec![9; 56 * 56 * 4],
        });
        tile.set_tile_px(112);
        assert!(
            tile.last_frame.is_none(),
            "the stored frame is the wrong size now too"
        );
    ...
    fn a_stopped_tile_is_marked_differently_from_a_starting_one() {
        ...
        assert_ne!(
            starting.pixels, stopped.pixels,
            "'warming up' and 'this is not coming back' must not look the same"
        );
    }
    ...
    fn middle_and_right_never_reach_a_dockapp() {
        ...
        for button in [wm_core::MouseButton::Middle, wm_core::MouseButton::Right] {
            assert!(
                wire_event(&DockInput::Press { local: at, button }).is_none(),
                "{button:?} press"
            );
            assert!(
                wire_event(&DockInput::Release { local: at, button }).is_none(),
                "{button:?} release"
            );
        }
        let left = wire_event(&DockInput::Press {
            local: at,
            button: wm_core::MouseButton::Left,
        })
        .unwrap();
    ...
    fn scroll_and_crossing_carry_what_the_wire_expects() {
        let scroll = wire_event(&DockInput::Scroll {
            local: Point::new(1, 2),
            delta: -1,
        })
        .unwrap();
        ...
        for (input, kind) in [
            (DockInput::Enter, InputKind::Enter),
            (DockInput::Leave, InputKind::Leave),
        ] {
            ...
            assert_eq!(
                (event.x, event.y),
                (0, 0),
                "a crossing is about the tile, not a position in it"
            );
        }
    }
    ...
    fn a_press_on_a_stopped_tile_restarts_it_and_clears_the_budget() {
        ...
        for step in 0..LaunchBudget::MAX_FAILURES - 1 {
            tile.budget
                .record_failure(now + Duration::from_secs(step as u64), false);
        }
        tile.state = TileState::Stopped {
            reason: StopReason::CrashLooped,
        };

        let effects = tile.on_input(
            DockInput::Press {
                local: Point::new(1, 1),
                button: wm_core::MouseButton::Left,
            },
        ...
        assert_eq!(
            effects.len(),
            1,
            "the tile's face changed, so the dock repaints"
        );
        assert!(
            matches!(tile.state, TileState::Waiting { .. }),
            "and it is queued to launch again"
        );
    ...
    fn seqpacket_pair() -> (Seqpacket, Seqpacket) {
        ...
        unsafe {
            (
                Seqpacket::from_fd(std::os::fd::OwnedFd::from_raw_fd(fds[0])),
                Seqpacket::from_fd(std::os::fd::OwnedFd::from_raw_fd(fds[1])),
            )
        }
    }
    ...
    fn a_dockapp_that_never_reads_costs_the_servicing_pass_nothing() {
        ...
        tile.adopt(
            ours,
            InputMask::all(),
            chonk_dock_proto::PROTOCOL_VERSION,
            welcome(),
            base,
        );
    ...
    fn a_dockapp_whose_process_vanished_disconnects_rather_than_signalling() {
        ...
        assert_eq!(
            ...
            "and `restart = never` means it stays gone"
        );
    }
    ...
    fn a_tile_that_stops_answering_is_dimmed_and_recovers_when_it_answers_again() {
        ...
        for step in 1..UNANSWERED_PINGS_BEFORE_HUNG {
            ...
            assert_eq!(
                tile.state,
                TileState::Live,
                "{step} unanswered pings is not yet a verdict"
            );
        }
        pass(&mut tile, UNANSWERED_PINGS_BEFORE_HUNG);
        assert!(
            matches!(tile.state, TileState::Hung { .. }),
            "{UNANSWERED_PINGS_BEFORE_HUNG} unanswered pings is"
        );
    ...
    fn hiding_the_dock_tells_a_dockapp_to_stop_and_showing_it_tells_it_to_start() {
        ...
        assert_eq!(
            visibility_pushed(&drain(&peer)),
            vec![true],
            "the handshake's, and no repeat of it"
        );
        ...
        assert_eq!(
            visibility_pushed(&drain(&peer)),
            vec![false],
            "told once that it is off screen"
        );
        assert_eq!(
            tile.state,
            TileState::Live,
            "and not disturbed otherwise — a hidden tile is not a dead one"
        );
    ...
    fn a_theme_change_restyles_a_running_dockapp_without_relaunching_it() {
        ...
        let next = ThemeState {
            ...
            scale: 2.0,
            proto: chonk_dock_proto::SHELL_PROTOCOL_VERSION,
            theme_id: "amber-phosphor".into(),
            theme_toml: "id = \"amber-phosphor\"".into(),
        };
        let mut ctx = ServiceContext {
            now: base,
            theme: &next,
        ...
        assert_eq!(
            themes_pushed(&drain(&peer)),
            vec![next.clone()],
            "the dockapp is told, over its existing connection"
        );
        assert_eq!(
            tile.state,
            TileState::Live,
            "and is not restarted, or even disturbed"
        );
        assert!(tile.poll_fd().is_some(), "the same socket it had before");
    ...
    fn a_relayout_rejects_the_old_size_and_theme_changed_is_how_the_dockapp_catches_up() {
        ...
        assert!(
            tile.on_frame(1, 56, 56, vec![9; 56 * 56 * 4], &ctx),
            "the dock is laid out at 56"
        );
        ...
        assert!(
            tile.last_frame.is_none(),
            "the stored 56px frame is dropped rather than drawn into a 112px slot"
        );
        assert!(
            tile.on_frame(2, 56, 56, vec![9; 56 * 56 * 4], &ctx),
            "an in-flight old-size frame is refused, not fatal"
        );
        assert!(
            tile.last_frame.is_none(),
            "and above all not blitted at the wrong size"
        );
        assert!(
            tile.poll_fd().is_some(),
            "refusing a frame does not cost the connection"
        );
        ...
        assert_eq!(
            pushed[0].tile_px, 112,
            "carrying the new geometry, which is what ends the rejections"
        );

        assert!(
            tile.on_frame(3, 112, 112, vec![7; 112 * 112 * 4], &ctx),
            "the dockapp redraws at the size it was told"
        );
    ...
    fn an_unchanged_theme_is_never_pushed_again() {
        ...
        for step in 0..1_000u32 {
            let mut ctx = ServiceContext {
                now: base + Duration::from_millis(16 * step as u64),
                theme: &steady,
                socket_path: &socket_path,
                scratch: &mut scratch,
                panel_bounds: (1024, 1024),
        ...
        assert!(
            themes_pushed(&drain(&peer)).is_empty(),
            "a dock that did not change told the dockapp nothing"
        );
    }
    ...
    fn a_theme_state_the_shell_cannot_compare_is_pushed_once_not_forever() {
        ...
        assert_ne!(
            nan, nan,
            "the premise: derived equality is not reflexive here"
        );
        for step in 0..1_000u32 {
            let mut ctx = ServiceContext {
                now: base + Duration::from_millis(16 * step as u64),
        ...
        // the far side either.
        let sent = drain(&peer);
        let refused: Vec<_> = sent
            .iter()
            .filter_map(|bytes| ServerMessage::decode(bytes).err())
            .collect();
        assert_eq!(
            refused,
        ...
        assert!(
            themes_pushed(&sent).is_empty(),
            "nothing usable was claimed to be a theme"
        );
        assert!(
            tile.poll_fd().is_some(),
            "and the tile was never disconnected for overflow"
        );
    }
    ...
    fn a_frame_is_taken_only_at_exactly_the_registered_geometry() {
        ...
        assert!(
            tile.on_frame(1, 56, 112, vec![0; 56 * 112 * 4], &ctx),
            "two units of 56px is exactly this tile"
        );
        assert!(tile.last_frame.is_some());
        assert_eq!(
            tile.state,
        ...
        assert!(
            tile.on_frame(2, 56, 56, vec![1; 56 * 56 * 4], &ctx),
            "a one-unit frame is refused, not fatal"
        );
        assert_eq!(
            tile.last_frame.as_ref().map(|frame| frame.pixels[0]),
            Some(0),
            "and the good frame is still what is drawn"
        );
        assert!(
            tile.poll_fd().is_some(),
            "a wrong-sized frame does not cost the connection"
        );
    }
    ...
    fn giving_up_on_a_rejoin_launches_without_spending_the_crash_budget() {
        ...
        assert_eq!(
            tile.state,
            TileState::Rejoining {
                until: base + REJOIN_WINDOW
            }
        );
        assert_eq!(
            tile.token(),
            &[7u8; TOKEN_BYTES],
            "the inherited token is what a survivor has to present"
        );
        ...
        tile.service(&mut ctx);
        assert_eq!(
            tile.budget.recent_failures(),
            1,
            "the failed launch, and only the failed launch"
        );
        assert!(
            matches!(tile.state, TileState::Waiting { .. }),
            "backing off to try the program again"
        );
    }
    ...
    fn the_shell_waits_exactly_as_long_as_a_dockapp_keeps_knocking() {
        assert_eq!(
            ...
            "chonk_ui::dockapp::RECONNECT_WINDOW is the same ten seconds"
        );
    }
    ...
    fn a_tile_starts_due_immediately_rather_than_after_a_backoff() {
        ...
        assert_eq!(
            tile.state,
            TileState::Waiting { until: now },
            "the first launch is not a retry"
        );
    ...
    fn a_tile_reports_the_height_it_registered_for() {
        let now = Instant::now();
        assert_eq!(
            RemoteTile::new(entry(RestartPolicy::Never, 3), 56, now).tile_height(),
            3
        );
    }
    ...
    fn a_remote_tile_declares_no_sources() {
        let now = Instant::now();
        assert!(RemoteTile::new(entry(RestartPolicy::Always, 1), 56, now)
            .sources()
            .is_empty());
    }
    ...
    fn server_messages(datagrams: &[Vec<u8>]) -> Vec<ServerMessage> {
        datagrams
            .iter()
            .filter_map(|bytes| ServerMessage::decode(bytes).ok())
            .collect()
    }
    ...
    fn a_panel_request_is_clamped_to_the_caps_and_the_workarea() {
        assert_eq!(
            clamp_panel_grant(300, 200, (1024, 1024)),
            Some((300, 200)),
            "a fitting request is granted verbatim"
        );
        assert_eq!(
            clamp_panel_grant(4000, 4000, (1024, 1024)),
            Some((1024, 1024)),
            "the protocol caps clamp"
        );
        assert_eq!(
            clamp_panel_grant(600, 400, (500, 300)),
            Some((500, 300)),
            "the workarea clamps"
        );
        assert_eq!(
            clamp_panel_grant(0, 200, (1024, 1024)),
            None,
            "a zero edge cannot be clamped into a size"
        );
        assert_eq!(
            clamp_panel_grant(300, 200, (0, 300)),
            None,
            "a degenerate workarea refuses"
        );
    }
    ...
    fn an_open_panel_request_is_answered_with_a_clamped_grant() {
        ...
        assert!(
            tile.panel_frame().is_none(),
            "nothing streamed yet: the desktop shows the well, not a fake frame"
        );
    }
    ...
    fn a_panel_request_nothing_can_satisfy_is_refused_not_granted() {
        ...
        assert_eq!(
            server_messages(&drain(&peer)),
            vec![ServerMessage::PanelClosed {
                reason: PanelCloseReason::Refused
            }]
        );
        assert!(
            tile.poll_fd().is_some(),
            "a refusal is an answer, not a disconnection"
        );
    }
    ...
    fn panel_bands_assemble_into_the_granted_buffer() {
        ...
        let top = ClientMessage::PanelFrame {
            generation: 1,
            y: 0,
            band_height: 2,
            width: 4,
        ...
        let bottom = ClientMessage::PanelFrame {
            generation: 1,
            y: 2,
            band_height: 2,
            width: 4,
        ...
        assert!(
            frame.pixels[..32].iter().all(|&b| b == 0x11),
            "rows 0..2 are the first band"
        );
        assert!(
            frame.pixels[32..].iter().all(|&b| b == 0x22),
            "rows 2..4 are the second"
        );
        assert!(
            tile.take_panel_ready(base + Duration::from_millis(16)),
            "new pixels are ready to present"
        );
    }
    ...
    fn a_band_outside_the_grant_is_refused_without_costing_the_connection() {
        ...
        let wrong_width = ClientMessage::PanelFrame {
            generation: 2,
            y: 0,
            band_height: 1,
            width: 2,
        ...
        let off_bottom = ClientMessage::PanelFrame {
            generation: 2,
            y: 3,
            band_height: 2,
            width: 4,
        ...
        assert!(
            tile.poll_fd().is_some(),
            "a wrong-sized band does not cost the connection"
        );
        let frame = tile.panel_frame().unwrap();
        assert!(
            frame.pixels.iter().all(|&b| b == 0x33),
            "and above all is never blitted"
        );
    }
    ...
    fn a_stale_generation_band_is_dropped() {
        ...
        let newer = ClientMessage::PanelFrame {
            generation: 7,
            y: 0,
            band_height: 2,
            width: 4,
        ...
        let stale = ClientMessage::PanelFrame {
            generation: 6,
            y: 0,
            band_height: 2,
            width: 4,
        ...
        assert!(
            tile.panel_frame()
                ...
                .iter()
                .all(|&b| b == 0x77),
            "the newest repaint's rows stand"
        );
    }
    ...
    fn generation_newness_wraps_like_a_sequence_number() {
        ...
        assert!(
            generation_newer(0, u32::MAX),
            "the counter rolling over is not a stuck panel"
        );
        assert!(!generation_newer(u32::MAX, 0));
        assert!(
            !generation_newer(5, 5),
            "equal is not older: several bands of one repaint all land"
        );
    }
    ...
    fn a_panel_close_is_acknowledged_and_a_dismissal_is_attributed() {
        ...
        peer.send(&ClientMessage::ClosePanel.encode().unwrap())
            .unwrap();
        service_once(&mut tile, base, (1024, 1024));
        assert!(!tile.panel_open());
        assert_eq!(
            server_messages(&drain(&peer)),
            vec![ServerMessage::PanelClosed {
                reason: PanelCloseReason::ClientRequest
            }]
        );
        ...
        assert_eq!(
            server_messages(&drain(&peer)),
            vec![ServerMessage::PanelClosed {
                reason: PanelCloseReason::Dismissed
            }]
        );
        assert!(
            tile.poll_fd().is_some(),
            "a dismissal costs the panel, never the tile"
        );
    }
    ...
    fn a_second_open_renegotiates_in_place() {
        ...
        assert!(
            tile.panel_frame().is_some(),
            "a size-identical renegotiation keeps the streamed pixels"
        );
        ...
        assert!(
            tile.panel_frame().is_none(),
            "pixels for the old grant are exactly the wrong-sized blit the checks refuse"
        );
    }
    ...
    fn a_hung_dockapp_loses_its_panel_by_the_ping_machinery() {
        ...
        assert!(
            !tile.panel_open(),
            "a frozen detail view is a stale reading at ten times the size"
        );
        ...
        assert_eq!(
            closes,
            vec![ServerMessage::PanelClosed {
                reason: PanelCloseReason::Shutdown
            }]
        );
    }
    ...
    fn a_dead_dockapps_panel_is_torn_down_with_it() {
        ...
        for step in 0..500 {
            service_once(
                &mut tile,
                base + Duration::from_millis(16 + step),
                (1024, 1024),
            );
    ...
    fn panel_input_reaches_the_wire_as_panel_input() {
        ...
        assert_eq!(
            server_messages(&drain(&peer)),
            vec![ServerMessage::PanelInput(event)]
        );
    ...
    fn panel_presents_are_metered_not_per_band() {
        ...
        assert!(
            presents <= 2,
            "{presents} presents for a 250Hz stream is not a meter"
        );
        assert!(
            presents >= 1,
            "a streaming panel must reach the screen at all"
        );
    }
}
