...
use chonk_dock_widget::{
    DockWidget, PanelCtx, PanelEvent, PanelFrame, PanelReaction, PanelSpec, Samples,
};
use wm_theme::Theme;
...
mod tests {
    ...
    fn the_probe_paints_green_until_clicked_and_red_after() {
        ...
        let theme = wm_theme::default_theme::all_themes()
            .into_iter()
            .next()
        ...
        let reaction = probe.panel_input(
            PanelEvent::LeftPress {
                local: wm_theme_api::Point::new(1, 1),
            },
            56,
        );
        assert!(
            matches!(reaction, PanelReaction::Repaint),
            "a click must ask for a repaint"
        );
        probe.render_panel(&mut frame, &mut ctx);
        ...
        assert!(
            matches!(
                probe.panel_input(PanelEvent::Enter, 56),
                PanelReaction::None
            ),
            "crossings mean nothing to the probe"
        );
    }
}
