...
pub use chonk_dock_widget::{
    DockInput, DockWidget, Effect, PanelCtx, PanelEvent, PanelFrame, PanelReaction, PanelSpec,
    Samples, Source, SourceId,
};
pub use chonk_instruments::{
    BluetoothWidget, ClockWidget, NetTrafficWidget, PowerWidget, SoundWidget, SysLoadWidget,
    WifiWidget,
};
...
impl DockItem {
    ...
    pub(crate) fn builtin(id: &'static str, widget: Box<dyn DockWidget>) -> Self {
        ...
        debug_assert!(
            ...
            "a built-in dock item's id must begin with `{BUILTIN_PREFIX}`, got {id:?}"
        );
...
impl Limits {
    const STOCK: Limits = Limits {
        budget: BUDGET,
        severe: SEVERE_BUDGET,
        hard: HARD_BUDGET,
...
fn verdict(elapsed: Duration, offences_before: u32, warmed: bool, limits: Limits) -> Verdict {
    ...
    if elapsed >= limits.severe {
        return if offences_before + 1 >= limits.offences {
            Verdict::Evict
        } else {
            Verdict::Offence
        };
...
impl SupervisedWidget {
    ...
    pub(crate) fn bind(&mut self, registry: &mut SamplerRegistry) {
        ...
        self.item.bind(&ids);
        self.source_requests = (0..ids.len())
            .map(|index| self.item.source_active(index, false))
            .collect();
        self.source_ids = ids;
    ...
    pub(crate) fn panel_input(
        &mut self,
        event: chonk_dock_widget::PanelEvent,
        tile: u32,
    ) -> chonk_dock_widget::PanelReaction {
        if self.evicted {
        ...
        match verdict(elapsed, self.offences, warmed, self.limits) {
            ...
            Verdict::Slow => {
                ...
                tracing::error!(
                    ...
                    ?elapsed,
                    budget = ?self.limits.budget,
                    "dock widget overran its frame budget on the dock's UI thread, \
                     where the dock draws nothing and reads no input until it returns. Not \
            ...
            Verdict::Offence => {
                ...
                tracing::error!(
                    ...
                    "dock widget stopped the dock's UI thread outright; the dock \
            ...
            Verdict::Evict => {
                ...
                tracing::error!(
                    ...
                    "evicting dock widget: it has blocked the dock's UI thread often \
...
mod tests {
    ...
    fn a_call_inside_the_budget_is_never_charged() {
        assert_eq!(
            verdict(Duration::from_micros(60), 0, true, Limits::STOCK),
            Verdict::Fine
        );
        ...
        assert_eq!(
            verdict(Duration::from_micros(3_100), 0, true, Limits::STOCK),
            Verdict::Fine
        );
    }
    ...
    fn the_first_call_of_each_kind_is_forgiven_once_but_only_softly() {
        assert_eq!(
            verdict(Duration::from_millis(9), 0, false, Limits::STOCK),
            Verdict::WarmUp
        );
        assert_eq!(
            verdict(Duration::from_millis(9), 0, true, Limits::STOCK),
            Verdict::Slow
        );
        ...
        assert_eq!(
            verdict(SEVERE_BUDGET, 0, false, Limits::STOCK),
            Verdict::Offence
        );
        assert_eq!(
            verdict(HARD_BUDGET, 0, false, Limits::STOCK),
            Verdict::Evict
        );
    }
    ...
    fn a_merely_over_budget_call_is_reported_but_never_counted() {
        for offences in 0..100 {
            assert_eq!(
                verdict(Duration::from_millis(9), offences, true, Limits::STOCK),
                Verdict::Slow
            );
        }
    }
    ...
    fn one_nmcli_shaped_stall_evicts_immediately() {
        assert_eq!(
            verdict(Duration::from_millis(3_600), 0, true, Limits::STOCK),
            Verdict::Evict
        );
        assert!(
    ...
    fn an_evicted_widget_is_never_called_again() {
        ...
        let mut supervised = SupervisedWidget::with_limits(
            DockItem::builtin("builtin:test", Box::new(widget)),
            TEST_LIMITS,
        );
        ...
        assert!(
            supervised.evicted,
            "three severe updates should have evicted it"
        );
        ...
        assert_eq!(
            ticks.get(),
            before,
            "an evicted widget must not be updated again"
        );
        assert!(
            supervised
                .render(&theme(), 56, &mut fonts, &mut swash)
                .is_none(),
            "an evicted widget renders no pixels of its own"
        );
        assert_eq!(renders.get(), 0);
        let effects = supervised.on_input(
            DockInput::Press {
                local: Point::new(1, 1),
    ...
    fn eviction_collapses_the_slot_to_one_tile_and_asks_for_exactly_one_relayout() {
        ...
        let mut supervised = SupervisedWidget::with_limits(
            DockItem::builtin(
                "builtin:test",
                Box::new(SlowWidget::new(TEST_LIMITS.severe * 2)),
            ),
            TEST_LIMITS,
        );
        assert_eq!(
            supervised.tile_height(),
            3,
            "a healthy widget keeps whatever height it asked for"
        );
        ...
        assert_eq!(
            supervised.tile_height(),
            1,
            "a tombstone is one square dead screen"
        );
        assert!(
            supervised.update(&bench.samples()),
            "the first update after an eviction must ask the dock to lay out again"
        );
        assert!(
            !supervised.update(&bench.samples()),
            "and exactly once — a tombstone never changes after that"
        );
    }
    ...
    fn offences_accumulate_across_entry_points_and_across_healthy_frames_in_between() {
        ...
        assert!(
            !supervised.evicted,
            "two offences is not three, however far apart they fell"
        );
        supervised.charge(CallKind::Input, TEST_LIMITS.severe);
        assert!(
            supervised.evicted,
            "the third offence evicts wherever it came from"
        );
    }
    ...
    fn a_healthy_widget_is_never_evicted_however_long_it_runs() {
        ...
        let bench = no_samples();
        let mut supervised =
            SupervisedWidget::new(DockItem::builtin("builtin:cheap", Box::new(Cheap)));
        for _ in 0..10_000 {
            supervised.update(&bench.samples());
        }
        assert!(!supervised.evicted);
    ...
    fn panel_calls_are_budgeted_and_an_evicted_widget_has_no_panel() {
        ...
        assert!(
            supervised.panel_spec(TEST_TILE).is_some(),
            "a healthy panel-capable widget advertises its panel"
        );

        let press = || PanelEvent::LeftPress {
            local: Point::new(1, 1),
        };
        assert!(matches!(
            supervised.panel_input(press(), TEST_TILE),
            PanelReaction::Repaint
        ));
        assert!(matches!(
            supervised.panel_input(press(), TEST_TILE),
        ...
        assert!(
            supervised.panel_spec(TEST_TILE).is_none(),
            "a tombstone has no detail view"
        );
        assert!(
            matches!(
                supervised.panel_input(press(), TEST_TILE),
            ...
            "and no panel controls"
        );
        assert!(
            !supervised.panel_tick(Instant::now()),
            "and nothing to keep live"
        );
    }
    ...
    fn the_tombstone_face_renders_for_every_built_in_name() {
        ...
        for name in ["NET", "LOAD", "SND", "LNK", "PWR", "CLK"] {
            let tile = wm_theme::panel::render_dead_tile(
                &theme,
                &mut font_system,
                &mut swash_cache,
                56,
            ...
            assert_eq!(
                ...
                (56, 56),
                "{name}'s tombstone must fill its slot"
            );
    ...
    fn every_built_in_widget_names_itself_shortly_and_distinctly() {
        ...
        for name in &names {
            assert!(
                !name.is_empty(),
                "a nameless widget is an unactionable log line"
            );
            assert!(
                name.len() <= 5,
                "{name:?} is too long for the tombstone tile's face"
            );
            assert_eq!(
                *name,
                name.to_uppercase(),
                "{name:?} should match the SDK's NET/SND/LNK empty-state labels"
            );
        }
        ...
        assert_eq!(
            unique.len(),
            names.len(),
            "two widgets sharing a name make the log ambiguous: {names:?}"
        );
    }
}
