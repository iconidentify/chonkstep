...
use chonk_dock_proto::wire::{
    is_valid_id, sanitize_text, Button, ClientMessage, DecodeError, GoodbyeReason, InputEvent,
    InputKind, InputMask, LogLevel, PanelCloseReason, ServerMessage, ThemeState,
};
use chonk_dock_proto::{
    MAX_FRAME_BYTES, MAX_ID_BYTES, MAX_LOG_BYTES, MAX_MESSAGE_BYTES, MAX_PANEL_PX, TOKEN_BYTES,
};
...
fn iterations(base: usize) -> usize {
    let scale: usize = std::env::var("CHONK_FUZZ_ITERS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(1);
...
fn same_server(
    a: &Result<ServerMessage, DecodeError>,
    b: &Result<ServerMessage, DecodeError>,
) -> bool {
    match (a, b) {
...
fn bounded_client(message: &ClientMessage, bytes: &[u8], origin: &str) {
    match message {
        ClientMessage::Hello { id, .. } => {
            assert!(
                id.len() <= MAX_ID_BYTES,
                "id over cap from {origin}: {}",
                id.len()
            );
            assert!(
                is_valid_id(id),
                "an id outside the allowlist decoded from {origin}: {id:?}"
            );
        }
        ClientMessage::Frame {
            width,
        ...
        ClientMessage::Log { text, .. } => {
            assert!(
                text.len() <= MAX_LOG_BYTES,
                "log text over cap from {origin}"
            );
            assert!(
        ...
        ClientMessage::PanelFrame {
            ...
            assert!(
                *width <= MAX_PANEL_PX,
                "a panel band wider than the cap decoded from {origin}"
            );
            assert!(
            ...
            assert!(
                pixels.len() <= MAX_FRAME_BYTES,
                "a band over the datagram frame budget decoded from {origin}"
            );
        }
        ClientMessage::OpenPanel { width, height } => {
            ...
            assert!(
                *width > 0 && *height > 0,
                "a zero-sized panel request decoded from {origin}"
            );
        }
    ...
    assert!(
        ...
        "a message over the transport cap decoded ({origin})"
    );
}
...
fn bounded_server(message: &ServerMessage, bytes: &[u8], origin: &str) {
    if let ServerMessage::Welcome(state) | ServerMessage::ThemeChanged(state) = message {
        assert!(
            state.theme_id.len() <= 64,
            "theme_id over cap from {origin}"
        );
        assert!(
            state.theme_toml.len() <= 128 * 1024,
            "theme_toml over cap from {origin}"
        );
    }
    assert!(
        bytes.len() <= MAX_MESSAGE_BYTES,
...
fn canonical_server(message: &ServerMessage, bytes: &[u8], origin: &str) {
    ...
    assert_eq!(
        reencoded, bytes,
        "two distinct byte strings decode to the same ServerMessage ({origin})"
    );
}
...
fn client_corpus() -> Vec<(&'static str, Vec<u8>)> {
    let mut corpus: Vec<(&'static str, ClientMessage)> = vec![
        ...
        (
            ...
            ClientMessage::Log {
                ...
                text: "battery sampler timed out".into(),
            },
        ),
        (
            "log/max",
        ...
        (
            ...
            ClientMessage::Log {
                level: LogLevel::Info,
                text: "CPU 42% · 3.4 GHz ünïcödé".into(),
            },
        ),
        (
            "open-panel",
    ...
    corpus
        ...
        .map(|(name, message)| {
            let bytes = message
                .encode()
                .expect("corpus entries are valid by construction");
            (name, bytes)
        })
        .collect()
}
...
fn server_corpus() -> Vec<(&'static str, Vec<u8>)> {
    ...
    let mut corpus: Vec<(&'static str, ServerMessage)> = vec![
        (
            "welcome",
            ServerMessage::Welcome(state("id = \"nextstep-classic\"\n")),
        ),
        ("welcome/empty-toml", ServerMessage::Welcome(state(""))),
        (
            "theme-changed",
            ServerMessage::ThemeChanged(state("[tile]\n")),
        ),
        (
            "input/press",
...
fn structured(rng: &mut Rng) -> Vec<u8> {
    const KINDS: [u8; 16] = [
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88,
        0x89,
    ];
...
fn every_single_byte_mutation_of_every_valid_message_is_handled() {
    ...
    for (name, bytes) in client_corpus().into_iter().chain(server_corpus()) {
        assert!(
            bytes.len() <= 512,
            "{name} is too large for the exhaustive pass; shrink it"
        );
    ...
    assert!(
        cases > 100_000,
        "the exhaustive pass covered only {cases} mutations; the corpus shrank"
    );
}
    ...
    for _ in 0..iterations(20_000) {
        ...
        match ServerMessage::decode(&bytes) {
            Err(DecodeError::BadFloat { field, bits }) => {
                let scale = unusable_scale(&message)
                    .unwrap_or_else(|| panic!("BadFloat for {message:?} which has no float"));
                assert_eq!(
                    (field, bits),
                    ("scale", scale.to_bits()),
                    "rejected the wrong float (seed {seed:#x})"
                );
            }
            other => assert!(
...
fn sanitized(message: ClientMessage) -> ClientMessage {
    match message {
        ClientMessage::Log { level, text } => ClientMessage::Log {
            level,
...
fn random_server_message(rng: &mut Rng) -> ServerMessage {
    match rng.below(6) {
        ...
        3 => ServerMessage::Visibility {
            visible: rng.bool(),
        },
        4 => ServerMessage::Ping {
            seq: rng.next_u32(),
        },
        _ => {
...
fn a_frame_the_encoder_could_not_produce_is_not_accepted_by_the_decoder() {
    ...
    assert_eq!(
        bytes.len(),
        MAX_MESSAGE_BYTES,
        "the reproducer sits exactly on the transport ceiling"
    );
    assert!(
