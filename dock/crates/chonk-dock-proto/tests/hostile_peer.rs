...
fn wedge(shell_side: &Seqpacket) -> u32 {
    ...
    loop {
        ...
        assert!(
            sent < 10_000,
            "the peer's socket buffer never filled; this test is not testing anything"
        );
    }
}
...
fn a_flooding_dockapp_costs_one_bounded_pass_not_an_unbounded_one() {
    ...
    assert!(
        read > 0,
        "the flooder sent {sent} frames and the shell read none of them"
    );
...
fn a_dockapp_that_dies_mid_send_does_not_signal_the_shell_to_death() {
    ...
    assert!(
        errors > 0,
        "sending to a closed peer must report an error rather than silently succeeding"
    );
    assert!(
    ...
    for size in [
        ...
        match dockapp.send(&huge) {
            ...
            Ok(_) => {
                ...
                let n = shell_side
                    .recv(&mut buffer)
                    .expect("a datagram the kernel accepted should arrive");
                assert!(
