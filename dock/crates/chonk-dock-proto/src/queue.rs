...
mod tests {
    ...
    fn a_full_queue_asks_for_a_disconnect_only_after_it_stays_full() {
        ...
        assert_eq!(
            queue.push(
                message(2),
                start + SUSTAINED_OVERFLOW - Duration::from_millis(1)
            ),
            SendOutcome::DroppedOldest,
    ...
    fn a_peer_that_catches_up_resets_the_overflow_clock() {
        ...
        assert_eq!(
            queue.push(message(3), start + Duration::from_secs(10)),
            SendOutcome::DroppedOldest
        );
    }
    ...
    fn a_real_send_error_is_propagated_so_the_caller_can_close() {
        ...
        let err = queue
            .flush(|_| Err(io::Error::from(io::ErrorKind::BrokenPipe)))
            .unwrap_err();
    ...
    fn the_first_frame_after_a_handshake_is_never_rate_limited() {
        ...
        assert_eq!(
            limiter.offer(1u32, now),
            Some(1),
            "a dead tile until 33ms have passed is worse than the limit"
        );
    }
    ...
    fn a_flood_is_coalesced_to_the_newest_frame_not_queued() {
        ...
        for tag in 100..1000u32 {
            assert_eq!(
                ...
                None,
                "no budget, so nothing is processed"
            );
        }
        assert_eq!(
            limiter.coalesced(),
            899,
            "899 frames were superseded before anyone looked at them"
        );
    ...
    fn the_long_run_rate_is_the_configured_rate() {
        ...
        for ms in 0..1000u64 {
            if limiter
                .offer(ms as u32, start + Duration::from_millis(ms))
                .is_some()
            {
                delivered += 1;
            }
        ...
        assert!(
            (29..=31).contains(&delivered),
            "about 30 frames in a second, got {delivered}"
        );
    }
    ...
    fn next_ready_in_tells_the_event_loop_when_to_come_back() {
        ...
        assert!(
            wait > Duration::ZERO && wait <= Duration::from_millis(34),
            "waited {wait:?}"
        );
        ...
        assert_eq!(
            limiter.next_ready_in(start + wait + Duration::from_millis(1)),
            Some(Duration::ZERO)
        );
    }
