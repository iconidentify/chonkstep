...
use crate::{
    MAX_FRAME_BYTES, MAX_ID_BYTES, MAX_LOG_BYTES, MAX_MESSAGE_BYTES, MAX_PANEL_PX, MAX_SCALE,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooLarge { len } => write!(
                f,
                "message of {len} bytes exceeds the {MAX_MESSAGE_BYTES}-byte cap"
            ),
            ...
            Self::StringLength { field, len, max } => {
            ...
            Self::BadEnum { field, value } => {
                write!(f, "field `{field}` has undefined value {value}")
            }
            Self::FrameGeometry { width, height } => {
                write!(f, "frame geometry {width}x{height} is out of range")
            }
            Self::PanelGeometry { width, height } => {
                write!(f, "panel geometry {width}x{height} is out of range")
            }
            Self::FrameLengthMismatch { expected, actual } => {
                write!(
                    f,
                    "frame declared {expected} pixel bytes but carried {actual}"
                )
            }
            Self::BadFloat { field, bits } => {
                write!(
                    f,
                    "field `{field}` is {} (bits {bits:#010x}), which is not a usable value",
                    f32::from_bits(*bits)
                )
            }
        }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::FrameLengthMismatch { expected, actual } => {
                write!(
                    f,
                    "frame geometry needs {expected} pixel bytes but was given {actual}"
                )
            }
            Self::TooLarge { len } => write!(
                f,
            ...
            Self::MotionOutsidePanel => {
                write!(f, "InputKind::Motion is defined only for PanelInput")
            }
        }
    }
}
...
fn check_string(field: &'static str, s: &str, max: usize) -> Result<(), EncodeError> {
    if s.len() > max {
        return Err(EncodeError::StringLength {
            field,
            len: s.len(),
            max,
        });
...
fn input_event_decode(r: &mut Reader<'_>, allow_motion: bool) -> Result<InputEvent, DecodeError> {
    ...
    let input_kind = InputKind::from_code(kind_code).ok_or(DecodeError::BadEnum {
        field: "input.kind",
        value: kind_code,
    })?;
    ...
    let button = Button::from_code(button_code).ok_or(DecodeError::BadEnum {
        field: "input.button",
        value: button_code,
    })?;
        ...
        match self {
            ...
            Self::Frame {
                ...
                if *width == 0
                    || *height == 0
                    || *width > MAX_TILE_PX
                    || *height > MAX_TILE_PX * u32::from(MAX_TILE_UNITS)
                {
                    return Err(EncodeError::FrameGeometry {
                        width: *width,
                        height: *height,
                    });
                }
                ...
                if pixels.len() != expected {
                    return Err(EncodeError::FrameLengthMismatch {
                        expected,
                        actual: pixels.len(),
                    });
                }
    ...
    pub fn decode(buf: &[u8]) -> Result<Self, DecodeError> {
        ...
        let message = match kind {
            KIND_HELLO => {
                ...
                let wants = InputMask::new(wants_bits).ok_or(DecodeError::BadEnum {
                    field: "wants",
                    value: wants_bits,
                })?;
                Self::Hello {
            ...
            KIND_LOG => {
                ...
                let level = LogLevel::from_code(level_code).ok_or(DecodeError::BadEnum {
                    field: "level",
                    value: level_code,
                })?;
                ...
                Self::Log {
                    level,
                    text: sanitize_text(&text, MAX_LOG_BYTES),
                }
            }
            KIND_OPEN_PANEL => {
            ...
            KIND_PANEL_FRAME => {
                ...
                if !panel_band_ok(y, band_height, width) {
                    return Err(DecodeError::PanelGeometry {
                        width,
                        height: band_height,
                    });
                }
    ...
    pub fn decode(buf: &[u8]) -> Result<Self, DecodeError> {
        ...
        let message = match kind {
            ...
            KIND_GOODBYE => {
                ...
                let reason = GoodbyeReason::from_code(code).ok_or(DecodeError::BadEnum {
                    field: "reason",
                    value: code,
                })?;
            ...
            KIND_PANEL_CLOSED => {
                ...
                let reason = PanelCloseReason::from_code(code).ok_or(DecodeError::BadEnum {
                    field: "panel.reason",
                    value: code,
                })?;
...
mod tests {
    ...
    fn every_client_message_survives_a_round_trip() {
        let messages = [
            ...
            ClientMessage::Log {
                level: LogLevel::Warn,
                text: "battery sampler timed out".to_string(),
            },
    ...
    fn every_server_message_survives_a_round_trip() {
        let messages = [
            ...
            ServerMessage::PanelInput(InputEvent {
                kind: InputKind::Press,
                button: Some(Button::Left),
                x: 40,
                y: 12,
    ...
    fn the_scale_float_survives_bit_exactly() {
        ...
        for scale in [1.0f32, 1.5, 2.0, 2.25, 3.0] {
            ...
            let ServerMessage::Welcome(decoded) = ServerMessage::decode(&bytes).unwrap() else {
                panic!("kind")
            };
    ...
    fn client_and_server_kinds_live_in_separate_number_spaces() {
        ...
        for bytes in [
            hello().encode().unwrap(),
            ClientMessage::Pong { seq: 1 }.encode().unwrap(),
        ] {
            assert!(matches!(
                ServerMessage::decode(&bytes),
                Err(DecodeError::UnknownKind { .. })
    ...
    fn panel_messages_have_the_documented_byte_layouts() {
        ...
        let bytes = ClientMessage::OpenPanel {
            width: 448,
            height: 168,
        }
        .encode()
        ...
        let bytes = ClientMessage::PanelFrame {
            generation: 9,
            y: 100,
            band_height: 1,
            width: 2,
        ...
        let bytes = ServerMessage::PanelClosed {
            reason: PanelCloseReason::Refused,
        }
        .encode()
        .unwrap();
        assert_eq!(
            bytes,
            vec![0x88, 0, 0, 0, 3, 0, 0, 0],
            "reason padded to four bytes, the Goodbye convention"
        );
    }
    ...
    fn the_version_u16_lives_at_body_offset_ten_of_welcome() {
        ...
        assert_eq!(
            &bytes[14..16],
            &crate::SHELL_PROTOCOL_VERSION.to_le_bytes(),
            "proto at body offset 10"
        );

        let mut v1 = bytes.clone();
        v1[14] = 0;
        v1[15] = 0;
        let ServerMessage::Welcome(decoded) = ServerMessage::decode(&v1).unwrap() else {
            panic!("kind")
        };
        assert_eq!(decoded.proto, 0, "kept raw for canonical re-encoding");
        assert!(
            !decoded.panels_supported(),
            "and zero reads as a protocol-1, tiles-only shell"
        );
    }
    ...
    fn a_version_1_client_is_sent_the_byte_exact_v1_welcome() {
        ...
        let sent = ServerMessage::Welcome(state.for_client(1))
            .encode()
            .unwrap();
        assert_eq!(
            sent, captured_v1_welcome,
            "a strict v1 decoder must see exactly the wire it was built against"
        );
        ...
        for v2_client in [2u32, 3, u32::MAX] {
            assert!(
                state.for_client(v2_client).same_as(&state),
                "a v2 client sees the unmasked state"
            );
        }
    }
    ...
    fn motion_is_a_panel_only_input_kind() {
        ...
        assert_eq!(
            ServerMessage::decode(&bytes),
            Ok(ServerMessage::PanelInput(motion))
        );

        assert_eq!(
            ServerMessage::Input(motion).encode(),
            Err(EncodeError::MotionOutsidePanel)
        );
        let mut forged = bytes.clone();
        forged[0] = 0x83;
        assert_eq!(
            ServerMessage::decode(&forged),
    ...
    fn panel_close_reasons_cover_exactly_the_published_codes() {
        for (reason, code) in [
            ...
            assert_eq!(
                ServerMessage::decode(&bytes),
                Ok(ServerMessage::PanelClosed { reason })
            );
        }
        ...
        assert!(
            ServerMessage::decode(&unpadded).is_err(),
            "the reserved padding is part of the layout"
        );
    }
    ...
    fn a_zero_sized_panel_request_is_rejected_but_an_oversized_one_is_not() {
        ...
        .concat();
        assert_eq!(
            ClientMessage::decode(&zero),
            Err(DecodeError::PanelGeometry {
                width: 0,
                height: 10
            })
        );
        let big = ClientMessage::OpenPanel {
            width: MAX_PANEL_PX * 4,
            height: 2,
        };
        assert!(
            ClientMessage::decode(&big.encode().unwrap()).is_ok(),
            "an over-cap request decodes; the grant clamps it"
        );
    }
    ...
    fn a_band_the_transport_cannot_carry_fails_at_the_encoder() {
        ...
        let message = ClientMessage::PanelFrame {
            generation: 1,
            y: 0,
            band_height: 512,
            width: 512,
        ...
        let band_ok = ClientMessage::PanelFrame {
            generation: 1,
            y: 385,
            band_height: 127,
            width: 512,
            pixels: vec![0; 512 * 127 * 4],
        };
    ...
    fn every_truncation_of_every_message_is_an_error_and_never_a_panic() {
        ...
        for bytes in &client {
            for len in 0..bytes.len() {
                assert!(
                    ClientMessage::decode(&bytes[..len]).is_err(),
                    "prefix {len} of {bytes:?} decoded"
                );
            }
        ...
        for bytes in &server {
            for len in 0..bytes.len() {
                assert!(
                    ServerMessage::decode(&bytes[..len]).is_err(),
                    "prefix {len} decoded"
                );
            }
    ...
    fn a_frame_over_the_payload_ceiling_is_refused_even_when_both_its_edges_are_legal() {
        ...
        assert!(
            payload > MAX_FRAME_BYTES && payload + HEADER_BYTES + 12 == MAX_MESSAGE_BYTES,
            "the reproducer still sits on the ceiling"
        );
        ...
        // produced.
        assert!(
            frame(w, h).encode().is_err(),
            "the encoder always refused this one"
        );
    }
    ...
    fn every_decodable_frame_fits_the_payload_budget_the_shell_allocates_against() {
        ...
        for width in 1..=MAX_TILE_PX {
            for height in 1..=MAX_TILE_PX * u32::from(MAX_TILE_UNITS) {
                ...
                assert_eq!(
                    decodable, encodable,
                    "the two ends disagree about {width}x{height}"
                );
            }
        }
    }
    ...
    fn an_id_can_look_like_a_relative_path_so_the_shell_must_never_join_one_to_a_path() {
        ...
        assert!(
            !is_valid_id("../etc"),
            "a separator is still refused, which is what stops traversal proper"
        );
    }
    ...
    fn every_message_that_decodes_is_equal_to_itself() {
        ...
        for scale in interesting {
            ...
            let Ok(decoded) = ServerMessage::decode(&bytes) else {
                continue;
            };
            assert_eq!(
                decoded, decoded,
                "scale {scale} decoded to something not equal to itself"
            );
        }
    }
    ...
    fn same_as_is_reflexive_where_derived_equality_is_not() {
        ...
        assert_ne!(
            nan, nan,
            "the derive is not reflexive, which is exactly the trap"
        );
        assert!(nan.same_as(&nan), "`same_as` is");

        let ordinary = ThemeState {
            scale: 2.0,
            ..nan.clone()
        };
        assert!(ordinary.same_as(&ordinary));
        assert!(
            !ordinary.same_as(&nan),
            "and it still separates two different states"
        );
        assert!(!ordinary.same_as(&ThemeState {
            tile_px: 112,
            ..ordinary.clone()
        }));
        assert!(!ordinary.same_as(&ThemeState {
    ...
    fn an_id_at_exactly_the_cap_is_accepted() {
        ...
        let ClientMessage::Hello { id: decoded, .. } = ClientMessage::decode(&bytes).unwrap()
        else {
            panic!()
        };
    ...
    fn ids_outside_the_allowlist_are_rejected_at_both_ends() {
        for hostile in [
            ...
            assert!(
                !is_valid_id(hostile),
                "{hostile:?} should not be a valid id"
            );
            let message = hello_with_id(hostile);
            assert!(message.encode().is_err(), "{hostile:?} should not encode");
        }
        assert!(
            is_valid_id("builtin:clock"),
            "reserved built-in ids share the namespace"
        );
    ...
    fn invalid_utf8_in_a_string_field_is_rejected() {
        ...
        assert_eq!(
            ClientMessage::decode(&bytes),
            Err(DecodeError::BadUtf8 { field: "text" })
        );
    }
    ...
    fn an_over_long_log_line_is_rejected_on_decode_and_truncated_on_encode() {
        ...
        let encoded = ClientMessage::Log {
            level: LogLevel::Info,
            text: long,
        }
        .encode()
        .unwrap();
        let ClientMessage::Log { text, .. } = ClientMessage::decode(&encoded).unwrap() else {
            panic!()
        };
    ...
    fn control_characters_never_survive_the_decoder() {
        ...
        assert!(
            ...
            "no C0/C1 controls survive: {text:?}"
        );
    ...
    fn unicode_line_separators_are_dropped_along_with_the_ascii_one() {
        ...
        assert!(
            !clean.contains('\u{2028}') && !clean.contains('\u{2029}'),
            "{clean:?}"
        );
        assert_eq!(
            clean, "battery okERROR: disk failingsecond forged line",
            "the text closes up behind them"
        );
    ...
    fn undefined_enum_discriminants_are_rejected() {
        let mut bytes = ServerMessage::Input(InputEvent {
            kind: InputKind::Press,
            button: Some(Button::Left),
            x: 0,
            y: 0,
        ...
        let mut bytes = ServerMessage::Goodbye {
            reason: GoodbyeReason::Shutdown,
        }
        .encode()
        .unwrap();
        ...
        let mut bytes = ClientMessage::Log {
            level: LogLevel::Info,
            text: String::new(),
        }
        .encode()
    ...
    fn a_bool_on_the_wire_is_zero_or_one_and_nothing_else() {
        let mut bytes = ServerMessage::Visibility { visible: true }
            .encode()
            .unwrap();
        bytes[4] = 2;
        assert_eq!(
    ...
    fn an_input_mask_gates_exactly_the_events_it_names() {
        ...
        assert!(
            crossing_only.accepts(InputKind::Leave),
            "wanting Enter without Leave latches hover forever"
        );
    ...
    fn a_frame_from_before_a_monitor_change_does_not_match_the_new_tile() {
        ...
        assert!(
            !frame_matches_tile(56, 56, 112, 1),
            "the dock rescaled under it"
        );
        assert!(
            !frame_matches_tile(56, 56, 56, 2),
            "one tile's worth of pixels for a two-tile slot"
        );
