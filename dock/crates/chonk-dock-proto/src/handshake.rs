...
mod tests {
    ...
    fn a_wrong_token_is_unauthorized() {
        ...
        assert_eq!(
            validate_hello(&message, &token, 56),
            Err(GoodbyeReason::Unauthorized)
        );
    }
    ...
    fn a_tile_the_transport_cannot_carry_is_refused_at_connect_time() {
        ...
        assert_eq!(
            validate_hello(&message, &token, 168),
            Err(GoodbyeReason::TileTooLarge)
        );
        assert_eq!(
    ...
    fn a_dockapp_cannot_skip_the_handshake_by_drawing() {
        ...
        assert_eq!(
            validate_hello(&frame, &token, 56),
            Err(GoodbyeReason::ProtocolError)
        );
        assert_eq!(
            validate_hello(&ClientMessage::Pong { seq: 0 }, &token, 56),
            Err(GoodbyeReason::ProtocolError)
        );
    }
    ...
    fn a_zero_tile_unit_hello_is_refused() {
        ...
        let token = mint_token().unwrap();
        assert_eq!(
            validate_hello(&hello("z", 0, token, InputMask::none()), &token, 56),
            Err(GoodbyeReason::TileTooLarge)
        );
    }
    ...
    fn the_whole_handshake_works_over_a_real_socket() {
        ...
        let state = client
            .join()
            .unwrap()
            .expect("the client should be welcomed");
    ...
    fn a_refused_dockapp_learns_why_instead_of_seeing_a_bare_eof() {
        ...
        let mut buffer = vec![0u8; MAX_MESSAGE_BYTES];
        let n = peer
            .recv_until(&mut buffer, Instant::now() + Duration::from_secs(5))
            .unwrap()
            .unwrap();
    ...
    fn a_shell_that_never_answers_times_out_rather_than_hanging_forever() {
        ...
        let err = client_handshake(&socket, "clock", 1, [0u8; TOKEN_BYTES], InputMask::none())
            .expect_err("no Welcome");
        assert_eq!(err.kind(), io::ErrorKind::TimedOut);
        assert!(started.elapsed() >= HANDSHAKE_TIMEOUT);
        assert!(
            started.elapsed() < HANDSHAKE_TIMEOUT * 3,
            "the wait should be bounded by the deadline, not by luck"
        );
    }
}
