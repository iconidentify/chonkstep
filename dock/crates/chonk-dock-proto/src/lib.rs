...
pub fn panel_band_fits(width: u32, band_height: u32) -> bool {
    width != 0
        && band_height != 0
        && (width as u64) * (band_height as u64) * 4 <= MAX_FRAME_BYTES as u64
}
...
mod tests {
    ...
    fn the_tile_sizes_this_desktop_actually_uses_fit_inline() {
        ...
        for scale in [1.0f32, 1.5, 2.0, 3.0] {
            ...
            assert!(
                frame_fits(tile, 1),
                "a single {tile}px tile must fit inline"
            );
            assert!(
                frame_fits(tile, 2),
                "a two-unit {tile}px tile must fit inline"
            );
        }
        // The tallest stack at the scale most HiDPI sessions use.
        assert!(
            frame_fits(112, MAX_TILE_UNITS),
            "a four-tile stack at scale 2"
        );
    }
    ...
    fn every_grantable_panel_can_be_repainted_in_bounded_bands() {
        ...
        assert!(!panel_band_fits(
            MAX_PANEL_PX,
            tallest_band_at(MAX_PANEL_PX) + 1
        ));
