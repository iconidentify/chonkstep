...
fn sockaddr_un(path: &Path) -> io::Result<(libc::sockaddr_un, libc::socklen_t)> {
    ...
    if bytes.len() >= addr.sun_path.len() {
        return Err(io::Error::new(
            ...
            format!(
                "socket path is {} bytes, the kernel's limit is {}: {}",
                bytes.len(),
                addr.sun_path.len() - 1,
                path.display()
            ),
...
impl SeqpacketListener {
    ...
    pub fn accept(&self) -> io::Result<Option<Seqpacket>> {
        let Some(fd) = accept_nonblocking(self.fd.as_raw_fd())? else {
            return Ok(None);
        };
        widen_socket_buffers(fd.as_raw_fd());
...
impl Stream {
    ...
    pub fn connect(path: &Path) -> io::Result<Self> {
        Ok(Self {
            fd: connect_nonblocking(path, libc::SOCK_STREAM)?,
        })
    }
...
mod tests {
    ...
    impl Scratch {
        fn new() -> Self {
            ...
            let dir = std::env::temp_dir()
                .join(format!("chonk-dock-test-{}-{unique}", std::process::id()));
    ...
    fn connected_pair(scratch: &Scratch) -> (SeqpacketListener, Seqpacket, Seqpacket) {
        ...
        let server = listener
            .accept()
            .expect("accept")
            .expect("a connection was pending");
    ...
    fn accept_returns_a_socket_that_cannot_block() {
        ...
        assert!(
            server.is_nonblocking().unwrap(),
            "an accepted dockapp socket must be O_NONBLOCK"
        );
        assert!(
            client.is_nonblocking().unwrap(),
            "the SDK's own socket too — a dockapp should not park either"
        );
    }
    ...
    fn a_send_to_a_peer_that_stopped_reading_returns_wouldblock_instead_of_parking_the_caller() {
        ...
        assert!(
            ...
            "filling the buffer took {elapsed:?}; that is not a non-blocking send"
        );
    }
    ...
    fn a_send_to_a_dead_peer_is_an_error_not_a_fatal_signal() {
        ...
        assert!(
            last.is_err(),
            "sending to a closed peer must report an error"
        );
    }
    ...
    fn the_listener_never_blocks_when_nothing_is_connecting() {
        ...
        assert!(
            ...
            "accept() blocked on an empty backlog"
        );
    }
    ...
    fn a_message_at_the_protocol_ceiling_survives_one_datagram() {
        ...
        assert_eq!(
            client.send(&biggest).unwrap(),
            biggest.len(),
            "the kernel must accept a ceiling-sized datagram"
        );
        let mut buffer = vec![0u8; MAX_MESSAGE_BYTES];
    ...
    fn the_socket_is_private_to_this_user() {
        ...
        let listener = SeqpacketListener::bind(&scratch.socket()).expect("bind");
        let mode =
            std::os::unix::fs::MetadataExt::mode(&std::fs::metadata(listener.path()).unwrap());
        assert_eq!(
            mode & 0o777,
            0o600,
            "the dock socket must not be reachable by group or other"
        );
    }
    ...
    fn the_socket_directory_is_created_private_and_verified_on_reuse() {
        ...
        std::fs::set_permissions(&dir, std::os::unix::fs::PermissionsExt::from_mode(0o755))
            .unwrap();
        assert!(
            ensure_socket_dir(&dir).is_err(),
            "a world-traversable socket directory must be refused"
        );
    }
    ...
    fn the_listener_removes_its_socket_when_dropped() {
        ...
        assert!(
            !path.exists(),
            "a stray socket makes the next session guess whether a shell is running"
        );
    }
    ...
    fn a_live_session_is_not_evicted_by_a_second_bind() {
        ...
        assert!(
            Seqpacket::connect(first.path()).is_ok(),
    ...
    fn an_over_long_socket_path_is_a_clear_error_not_a_truncated_address() {
        ...
        assert!(
            err.to_string().contains("107"),
            "the message should name the usable limit: {err}"
        );
    }
    ...
    fn display_names_cannot_escape_the_socket_directory() {
        ...
        assert_eq!(
            sanitize_display("../../etc/passwd"),
            "______etc_passwd",
            "no dot and no separator survives"
        );
        assert_eq!(sanitize_display(""), "default");
        assert_eq!(sanitize_display("::::"), "default");
        assert!(!sanitize_display(&"a/b".repeat(100)).contains('/'));
        assert!(
            sanitize_display(&"a".repeat(1000)).len() <= 32,
            "a display name cannot blow the 108-byte sun_path"
        );
    }
    ...
    fn the_control_socket_lives_beside_the_dock_socket_under_the_same_display_key() {
        ...
        match (socket_path("wayland-1"), control_socket_path("wayland-1")) {
            (Ok(dock), Ok(control)) => {
                ...
                assert_eq!(
                    control_socket_path(":1").unwrap().file_name().unwrap(),
                    "control-1.sock"
                );
            }
            (Err(_), Err(_)) => {}
            (dock, control) => panic!(
                "the two paths must agree on whether a runtime dir exists: {dock:?} vs {control:?}"
            ),
        }
    }
    ...
    fn tokens_survive_the_environment_round_trip() {
        ...
        assert_eq!(
            token_from_hex(&format!("  {hex}\n")),
            Some(token),
            "trailing newline from a shell pipeline"
        );
    }
    ...
    fn a_malformed_token_string_is_rejected_rather_than_padded() {
        for bad in [
            ...
            &"0".repeat(31),
            &"0".repeat(33),
            "gg00000000000000000000000000000000",
        ] {
    ...
    fn the_urandom_fallback_reads_sixteen_bytes_and_returns() {
        ...
        assert_ne!(
            ...
            "sixteen zero bytes is not a credential"
        );
    }
    ...
    fn probing_a_socket_whose_backlog_is_full_does_not_hang_and_does_not_delete_it() {
        ...
        assert!(
            pending.len() >= BACKLOG as usize,
            "the backlog should have accepted at least {BACKLOG} connections"
        );
        ...
        assert!(
            path.exists(),
            "the probe must not unlink a socket that is still being served"
        );
    ...
    fn connect_reports_a_full_backlog_instead_of_waiting_for_it() {
        ...
        assert_eq!(
            ...
            "a full backlog is EAGAIN, not a wait"
        );
    }
    ...
    fn minted_tokens_are_not_all_the_same() {
        ...
        assert_ne!(
            a, [0u8; TOKEN_BYTES],
            "getrandom silently failing would look exactly like this"
        );
    }
    ...
    fn token_comparison_accepts_only_an_exact_match() {
        ...
        assert!(
            !tokens_match(&token, &off_by_one_bit),
            "the last byte must matter as much as the first"
        );
    ...
    fn an_accepted_stream_is_nonblocking_on_both_ends() {
        ...
        assert!(
            server.is_nonblocking().unwrap(),
            "an accepted control client must be O_NONBLOCK"
        );
    ...
    fn a_stale_stream_socket_is_replaced_and_a_live_one_is_kept() {
        ...
        std::fs::write(&path, b"").unwrap();
        let first =
            StreamListener::bind(&path).expect("debris left by a killed session is cleared");
        ...
        assert_eq!(err.kind(), io::ErrorKind::AddrInUse);
        assert!(
            Stream::connect(first.path()).is_ok(),
            "the refused bind must not have unlinked the live socket"
        );
    }
    ...
    fn a_stream_listener_removes_its_socket_when_dropped() {
        ...
        assert!(
            !path.exists(),
            "a clean shutdown must not leave debris for the next session to probe"
        );
    }
