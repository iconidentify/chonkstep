...
use chonk_dock_client::dockapp::{self, Handlers, LogLevel, Options, Pixmap};
use chonk_dock_proto::handshake::validate_hello;
use chonk_dock_proto::transport::{
    mint_token, token_to_hex, Seqpacket, SeqpacketListener, ENV_SOCKET, ENV_TOKEN,
};
use chonk_dock_proto::wire::{
    frame_matches_tile, ClientMessage, GoodbyeReason, ServerMessage, ThemeState,
};
...
fn recv_until<T>(peer: &Seqpacket, mut want: impl FnMut(ClientMessage) -> Option<T>) -> T {
    ...
    loop {
        ...
        assert!(
            ...
            "timed out waiting for the expected message"
        );
    }
}
...
fn a_dockapp_handshakes_draws_answers_pings_and_rethemes_without_restarting() {
    ...
    assert!(
        peer.is_nonblocking().expect("flags"),
        "an accepted dockapp socket must never be able to block the shell"
    );
    assert!(peer.peer_is_this_user().expect("peer credentials"));

    // 1. Hello, validated exactly as the shell will validate it.
    let hello = recv_until(&peer, |m| {
        matches!(m, ClientMessage::Hello { .. }).then_some(m)
    });
    ...
    peer.send(
        &ServerMessage::Welcome(theme_state(56, 1.0, "nextstep-classic"))
            .encode()
            .unwrap(),
    )
    .unwrap();
    ...
    assert!(
        ...
        "control characters must not survive the wire: {text:?}"
    );
    ...
    assert!(
        frame_matches_tile(width, height, 56, 1),
        "{width}x{height} is not the tile the shell allocated"
    );
    assert_eq!(pixels.len(), 56 * 56 * 4);
    assert_eq!(
        &pixels[..4],
        &[56, 56, 56, 0xFF],
        "the dockapp's own pixels, premultiplied RGBA, top row first"
    );

    // 5. Liveness. The Pong may be preceded by more frames; a shell has
    //    to cope with that, so this does too.
    peer.send(&ServerMessage::Ping { seq: 0xABCD }.encode().unwrap())
        .unwrap();
    ...
    peer.send(
        &ServerMessage::ThemeChanged(theme_state(112, 2.0, "amber-phosphor"))
            .encode()
            .unwrap(),
    )
    .unwrap();
    let (width, height, pixels) = recv_until(&peer, |m| match m {
        ClientMessage::Frame {
            ...
            pixels,
            ..
        } if width == 112 => Some((width, height, pixels)),
        _ => None,
    });
    ...
    assert_eq!(
        &pixels[..4],
        &[112, 112, 112, 0xFF],
        "the resized tile, drawn fresh rather than scaled up"
    );
    ...
    assert_eq!(
        ...
        quiet_at,
        "a hidden dockapp should stop drawing, not just stop being seen"
    );
    peer.send(
    ...
    .unwrap();
    recv_until(&peer, |m| {
        matches!(m, ClientMessage::Frame { .. }).then_some(())
    });
    ...
    let peer = loop {
        ...
        assert!(
            reconnect_started.elapsed() < DEADLINE,
            "the dockapp gave up instead of reconnecting"
        );
    };
    let hello = recv_until(&peer, |m| {
        matches!(m, ClientMessage::Hello { .. }).then_some(m)
    ...
    let readopted =
        validate_hello(&hello, &token, 56).expect("the reconnect re-presents its id and token");
    assert_eq!(
        readopted.id, "conformance-tile",
        "the same id, so a fresh shell can match it to the same slot"
    );
    peer.send(
        &ServerMessage::Welcome(theme_state(56, 1.0, "nextstep-classic"))
            .encode()
    ...
    assert!(
        frame_matches_tile(width, height, 56, 1),
        "the readopted tile draws at the new shell's geometry"
    );
    ...
    dockapp
        .join()
        .expect("the dockapp thread should not panic")
