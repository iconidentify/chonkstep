...
//!
//! ```no_run
//! use chonk_dock_client::dockapp::{self, Handlers};
//!
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Refused(reason) => {
                write!(f, "the shell closed this dockapp's connection: {reason:?}")
            }
            Self::Geometry { width, height } => {
                write!(f, "could not allocate a {width}x{height} tile")
            }
        }
    }
}
    ...
    I: FnMut(&Ctx, InputEvent) -> bool,
{
    ...
    let mut state =
        handshake::client_handshake(&socket, id, options.tile_units, token, options.wants)?;
    ...
    loop {
        match serve(&socket, &state, &options, &mut handlers, visible)? {
            ...
            Outcome::Disconnected => {
                ...
                socket = reconnected;
                state = handshake::client_handshake(
                    &socket,
                    id,
                    options.tile_units,
                    token,
...
fn connection_details() -> Result<(std::path::PathBuf, [u8; TOKEN_BYTES]), Error> {
    let path = std::env::var_os(ENV_SOCKET).ok_or_else(|| {
        Error::Environment(format!(
            "{ENV_SOCKET} is not set: a dockapp is launched by the dock, not run from a shell"
        ))
    })?;
    let token_hex = std::env::var(ENV_TOKEN).map_err(|_| {
        Error::Environment(format!(
            "{ENV_TOKEN} is not set; the dock mints it per slot at launch"
        ))
    })?;
    ...
    I: FnMut(&Ctx, InputEvent) -> bool,
{
    ...
    let mut pixmap = Pixmap::new(ctx.tile_px(), ctx.height()).ok_or(Error::Geometry {
        width: ctx.tile_px(),
        height: ctx.height(),
    })?;
    ...
    loop {
        ...
        let deadline = if ctx.visible() {
            next_draw
        } else {
            Instant::now() + options.redraw_interval
        };
...
mod tests {
    ...
    fn at_scale(scale: f32) -> ThemeState {
        ThemeState {
            scale,
            ..state("nextstep-classic", String::new())
        }
    }
    ...
    fn every_scale_a_real_desktop_uses_is_accepted() {
        ...
        for scale in [0.5, 1.0, 1.5, 2.0, 3.0, MAX_SCALE] {
            assert!(
                check_drawable(&at_scale(scale), 1).is_ok(),
                "scale {scale} should draw"
            );
        }
    }
    ...
    fn a_geometry_that_could_never_cross_the_socket_is_refused_before_it_is_allocated() {
        ...
        let huge = ThemeState {
            tile_px: chonk_dock_proto::MAX_TILE_PX,
            ..at_scale(1.0)
        };
        assert_eq!(
            check_drawable(&huge, chonk_dock_proto::MAX_TILE_UNITS).is_err(),
            !chonk_dock_proto::frame_fits(
                chonk_dock_proto::MAX_TILE_PX,
                chonk_dock_proto::MAX_TILE_UNITS
            ),
            "the SDK must agree with the transport about what fits"
        );
        assert!(check_drawable(
            &ThemeState {
                tile_px: 0,
                ..at_scale(1.0)
            },
        ...
        .is_err());
        assert!(
            check_drawable(&at_scale(1.0), 0).is_err(),
            "a zero-tall tile is not a tile"
        );
    }
    ...
    fn the_theme_arrives_already_scaled() {
        ...
        let unscaled = theme_from(&ThemeState {
            scale: 1.0,
            ..state("nextstep-classic", String::new())
        });
        let doubled = theme_from(&ThemeState {
            scale: 2.0,
            ..state("nextstep-classic", String::new())
        });
    ...
    fn the_context_reports_the_pixmap_geometry_the_shell_will_expect() {
        ...
        assert!(chonk_dock_proto::wire::frame_matches_tile(
            ctx.tile_px(),
            ctx.height(),
            112,
            2
    ...
    fn default_options_ask_for_one_tile_at_one_hertz() {
        ...
        assert!(
            chonk_dock_proto::frame_fits(168, options.tile_units),
            "and at scale 3"
        );
    }
}
