...
fn main() -> ExitCode {
    ...
    match mode.as_str() {
        ...
        "crash-loop" => {
            eprintln!(
                "torture: exiting {} immediately, before connecting",
                exit::CRASHED_IMMEDIATELY
            );
...
fn requested_mode() -> Option<String> {
    ...
    from_argv.or_else(|| {
        std::env::var("CHONKSTEP_TORTURE_MODE")
            .ok()
            .filter(|s| !s.is_empty())
    })
}
...
fn id() -> String {
    std::env::var("CHONKSTEP_TORTURE_ID")
        .ok()
        .filter(|s| !s.is_empty())
...
fn connection_details() -> Result<(std::path::PathBuf, [u8; TOKEN_BYTES]), String> {
    let path = std::env::var_os(ENV_SOCKET).ok_or_else(|| {
        format!("{ENV_SOCKET} is not set: a dockapp is launched by the dock, not run from a shell")
    })?;
...
fn connected() -> Result<(Seqpacket, ThemeState), ExitCode> {
    ...
    let state =
        handshake::client_handshake(&socket, &id(), 1, token, InputMask::all()).map_err(|e| {
            eprintln!("torture: handshake: {e}");
            ExitCode::from(exit::HANDSHAKE_FAILED)
        })?;
    eprintln!(
        "torture: connected; tile is {}px at scale {}",
        state.tile_px, state.scale
    );
    Ok((socket, state))
}
...
fn good_frame(state: &ThemeState, generation: u32, tint: u8) -> Vec<u8> {
    ...
    pixmap.fill(chonk_ui::tiny_skia::Color::from_rgba8(
        tint, 0x30, 0x30, 0xFF,
    ));
    ClientMessage::Frame {
        generation,
...
fn hang() -> ExitCode {
    ...
    let _ = socket.send(
        &ClientMessage::Log {
            level: LogLevel::Warn,
            text: "torture: about to stop reading this socket forever".into(),
        }
        .encode()
...
fn crash() -> ExitCode {
    ...
    eprintln!(
        "torture: alive for {delay:?}, then exiting {}",
        exit::CRASHED_AFTER_CONNECTING
    );
...
fn slow_handshake() -> ExitCode {
    ...
    loop {
        match socket.recv_until(&mut buffer, Instant::now() + Duration::from_secs(5)) {
            ...
            Ok(Some(0)) => {
                eprintln!(
                    "torture: the shell closed the connection on a handshake that never arrived"
                );
...
fn malformed() -> ExitCode {
    ...
    for (index, (name, bytes)) in battery.into_iter().enumerate() {
        eprintln!(
            "torture: [{}/{total}] sending {name} ({} bytes)",
            index + 1,
            bytes.len()
        );
...
fn battery(state: &ThemeState) -> Vec<(&'static str, Vec<u8>)> {
    ...
    out.push((
        "frame/lying-short: claims a full tile, carries 4 bytes",
        short,
    ));
    ...
    for (name, edge) in [
        (
            "frame/wrong-tile-larger: a legal frame for a tile this dock does not have",
            tile.saturating_add(1),
        ),
        (
            "frame/wrong-tile-smaller: ditto, one pixel under",
            tile.saturating_sub(1).max(1),
        ),
    ] {
    ...
    out.push((
        "frame/ceiling: 254x258, exactly MAX_MESSAGE_BYTES on the wire",
        ceiling,
    ));
    ...
    for (name, raw) in [
        (
            "hello/id-newline: forges a second line in any log",
            b"clock\nnet".to_vec(),
        ),
        (
            "hello/id-bidi: rewrites the text beside it when rendered",
            "clock\u{202E}kcolc".as_bytes().to_vec(),
        ),
        (
            "hello/id-traversal: an id used as a path component",
            b"../../etc/passwd".to_vec(),
        ),
        ("hello/id-nul", b"clock\0net".to_vec()),
        ("hello/id-empty", Vec::new()),
        (
            "hello/id-over-cap: 200 bytes where 64 is the limit",
            vec![b'a'; 200],
        ),
    ] {
    ...
    for (name, text) in [
        (
            "log/control-chars: ESC[2J and a newline",
            "wipe\u{1b}[2Jscreen\nsecond line".to_string(),
        ),
        (
            "log/bidi-override",
            "battery \u{202E}%08 gnigrahc\u{202C}".to_string(),
        ),
        (
            "log/zero-width: two ids that render identically",
            "cl\u{200B}ock".to_string(),
        ),
        ...
        (
            "log/unicode-paragraph-separator",
            "one\u{2029}two".to_string(),
        ),
        (
            "log/over-cap: 4 KB where 256 is the limit",
            "x".repeat(4096),
        ),
    ] {
    ...
    out.push((
        "log/invalid-utf8: including an overlong NUL encoding",
        bad_utf8,
    ));
    ...
    out.push((
        "pong/trailing-bytes: two byte strings meaning one message",
        trailing,
    ));

    out.push((
        "header-only: a bare header with no body at all",
        header(0x02, [0, 0, 0]),
    ));
