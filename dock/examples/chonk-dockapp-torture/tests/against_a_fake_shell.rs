...
use chonk_dock_proto::transport::{
    mint_token, token_to_hex, wait_readable, Seqpacket, SeqpacketListener,
};
...
impl Drop for Torture {
    fn drop(&mut self) {
        ...
        eprintln!(
            "torture[{}] did not die after SIGKILL; leaving it to the OS",
            self.mode
        );
    }
}
...
fn welcomed(mode: &'static str, tag: &str) -> (FakeShell, Torture) {
    ...
    let peer = loop {
        ...
        assert!(
            ...
            "the torture binary never connected"
        );
    ...
    assert!(
        peer.peer_is_this_user().expect("SO_PEERCRED"),
        "a dockapp must be a process of this same user"
    );
    assert!(
        peer.is_nonblocking().expect("F_GETFL"),
        "accept4 must hand back an O_NONBLOCK socket"
    );
    ...
    let accepted = handshake::validate_hello(&hello, &token, TILE_PX)
        .expect("the handshake should be accepted");
    assert_eq!(accepted.id, "torture");

    let state = theme_state();
    peer.send(
        &ServerMessage::Welcome(state.clone())
            .encode()
            .expect("encodes"),
    )
    .expect("send Welcome");
...
fn a_real_dockapp_process_that_stops_reading_costs_the_repaint_thread_nothing() {
    ...
    while Instant::now() < deadline && !got_frame {
        match shell
            .peer
            .recv_until(&mut buffer, Instant::now() + Duration::from_millis(200))
        {
            Ok(Some(n)) if n > 0 => {
                if let Ok(ClientMessage::Frame { width, height, .. }) =
                    ClientMessage::decode(&buffer[..n])
                {
                    assert!(
    ...
    assert!(
        filled > 0,
        "at least one message should fit before the buffer fills"
    );
    ...
    assert!(
        dropped > 0,
        "a wedged peer should have cost dropped messages; nothing backed up, so nothing was tested"
    );
    assert!(
        disconnects > 0,
        "the shell must eventually give up on a peer whose queue never drains"
    );
    ...
    assert!(
        torture
            .exit_code_within(Duration::from_millis(200))
            .is_none(),
        "the hang mode must not exit"
    );
    drop(shell.listener);
}
...
fn a_real_flooding_dockapp_is_coalesced_rather_than_queued() {
    ...
    assert!(
        received > 100,
        "the flood mode produced only {received} frames; it is not flooding"
    );
    assert!(
    ...
    assert!(
        limiter.coalesced() > 0,
        "frames were superseded but the limiter did not count them"
    );
    drop(shell.listener);
}
...
fn crash_mode_draws_once_and_then_exits_non_zero() {
    ...
    let mut torture = Torture::spawn_with(
        "crash",
        &socket_path,
        &token,
        &[("CHONKSTEP_TORTURE_DELAY_MS", "100")],
    );
    ...
    peer.send(&ServerMessage::Welcome(theme_state()).encode().unwrap())
        .expect("Welcome");
    ...
    assert!(
        matches!(
            ClientMessage::decode(&buffer[..n]),
            Ok(ClientMessage::Frame { .. })
        ),
        "crash mode draws once"
    );

    assert_eq!(
        torture.exit_code_within(PATIENCE),
        Some(70),
        "crash mode must exit 70"
    );
        ...
        match peer.recv(&mut buffer) {
            ...
            Err(e) if e.kind() == io::ErrorKind::WouldBlock => {
                assert!(
                    ...
                    "a dead peer must read back as EOF"
                );
...
fn nothing_in_the_malformed_battery_ever_becomes_a_tile() {
    ...
    while Instant::now() < deadline && idle_since.elapsed() < Duration::from_millis(750) {
        match shell.peer.recv(&mut buffer) {
            ...
            Ok(n) => {
                ...
                let _ = shell
                    .peer
                    .send(&ServerMessage::Ping { seq: received }.encode().unwrap());
            }
            Err(e) if e.kind() == io::ErrorKind::WouldBlock => {
                std::thread::sleep(Duration::from_millis(5))
            }
    ...
    assert!(
        received >= 20,
        "only {received} datagrams arrived; the battery should be much longer than that"
    );
    assert!(
        decoded_ok < received,
    ...
    assert!(
        rejected_frames >= 2,
        "the two legal-but-wrong-geometry frames must decode and then be refused"
    );
    ...
    assert!(
        !log_lines.is_empty(),
        "the battery's log lines should decode; sanitizing happens on the way in"
    );
    for text in &log_lines {
        assert!(
            !text.chars().any(char::is_control),
            "a control character reached the shell: {text:?}"
        );
        assert!(
            !text.contains('\u{202E}'),
            "a bidi override reached the shell: {text:?}"
        );
        assert!(
            !text.contains('\u{200B}'),
            "a zero-width space reached the shell: {text:?}"
        );
        assert!(
        ...
        assert!(
            text.len() <= 256,
            "an over-long log line reached the shell: {} bytes",
            text.len()
        );
    }
    assert_eq!(
...
fn a_peer_that_never_sends_hello_never_delays_a_pass() {
    ...
    assert!(
        torture
            .exit_code_within(Duration::from_millis(200))
            .is_none(),
        "slow-handshake should still be sitting"
    );
}
...
fn a_wrong_token_is_refused_and_the_dockapp_is_told_why() {
    ...
    let n = peer
        ...
        .expect("a Hello");
    let hello =
        ClientMessage::decode(&buffer[..n]).expect("a well-formed Hello — only the token is wrong");
    ...
    peer.send(&ServerMessage::Goodbye { reason }.encode().unwrap())
        .expect("Goodbye");
