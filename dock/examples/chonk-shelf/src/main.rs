    ...
    let result = dockapp::run_with(
        ...
        Handlers {
            ...
            input: move |ctx: &dockapp::Ctx, event: chonk_dock_proto::wire::InputEvent| {
                ...
                let unit = (event.y / ctx.tile_px().max(1) as i32)
                    .clamp(0, i32::from(SHELF_UNITS) - 1) as usize;
                let text = input_state
                    .entries
                    .lock()
                    .ok()
                    .and_then(|entries| entries.get(unit).cloned());
...
fn render(
    ...
    let preview_font = FontSpec {
        size: (size as f32 * 0.14).max(8.0),
        ..base.clone()
    };
    let label_font = FontSpec {
        size: (size as f32 * 0.12).max(7.0),
        ..base.clone()
    };
...
mod tests {
    ...
    fn a_repeated_copy_is_one_entry_and_a_recopy_floats_to_the_top() {
        ...
        push_entry(&s, "a".into());
        assert_eq!(
            s.entries.lock().unwrap().len(),
            1,
            "consecutive duplicates coalesce"
        );
        push_entry(&s, "b".into());
        push_entry(&s, "a".into());
        let entries = s.entries.lock().unwrap();
        assert_eq!(
            entries.len(),
            2,
            "re-copying an entry must not duplicate it"
        );
    ...
    fn every_change_bumps_the_version_and_a_no_op_does_not() {
        ...
        assert_eq!(
            ...
            after_first,
            "a duplicate is not a change the wire should see"
        );
    }
    ...
    fn the_shelf_geometry_fits_the_protocol() {
        ...
        for scale in [1.0f32, 1.5, 2.0] {
            ...
            assert!(
                chonk_dock_proto::frame_fits(tile, SHELF_UNITS),
                "{tile}px x {SHELF_UNITS}"
            );
        }
    }
}
