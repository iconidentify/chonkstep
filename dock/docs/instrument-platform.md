> Restored with Chonk Dock from the pre-0.6 dock. The host described here
> is now the standalone `chonk-dock` process. See [current setup](../README.md).
...
Install it and it appears in the dock at the next dock restart:

```
$ dock/chonk-get install ./hello-instrument
```
...
```
$ dock/chonk-get install dock/bindings/python       # a Python clock tile
$ dock/chonk-get install dock/examples/chonk-shelf  # the Shelf: clipboard history
$ dock/chonk-get install dock/examples/chonk-switch # the light/dark toggle
$ dock/chonk-get list
$ dock/chonk-get remove py-dockclock
```
...
bomb (`chonk-dock/src/dockapp/tile.rs`, `LaunchBudget`). A hung
...
(`chonk-dock/src/dockapp/tile.rs`, `launch`). It sees its own tile
...
**Dockapp lifetime follows the dock process.** Stopping Chonk Dock sends
`Goodbye { Shutdown }` and shuts down its dockapps. Starting it launches
fresh instances from the registry. The recovered protocol, SDKs and
`chonk-dock/src/dockapp/handoff.rs` retain the old reconnect/readoption
machinery, but the standalone application does not use hot handoff.
Theme changes still preserve each running dockapp's connection and state.
...
window. Widgets receive no keyboard input; the dock temporarily takes
keyboard focus to dismiss the popover on Escape.
...
The built-in instruments are not separate dockapps — they run in the
standalone dock process, against the `DockWidget` SDK
...
takes no widget-level keyboard input, and the only key involved is the
Escape the *dock* handles to dismiss it — which the widget is never told about, because
...
4. Anyone installs it with
   `dock/chonk-get install <your-git-url>` — clone, build
