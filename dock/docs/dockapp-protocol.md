...
normative implementation is `dock/crates/chonk-dock-proto`; where this
...
This section records the original host's handoff contract, preserved in
the SDKs and host helpers. The standalone `chonk-dock` currently performs
a full shutdown: stop/quit sends `Goodbye { Shutdown }`, and a later
launch creates fresh dockapp processes. It does not invoke hot handoff.
...
Dockapps installed mid-session appear at the next dock restart.
...
The reference client is `dock/crates/chonk-dock-client/src/dockapp.rs`; the
end-to-end sequence a shell expects is written out longhand in
`dock/crates/chonk-dock-client/tests/dockapp_conformance.rs`, and
