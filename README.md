...
Version 0.4.4 is an **alpha**. It is ready for adventurous users and bug
...
## Install ChonkStep 0.4.4 alpha

Native packages are available for `x86_64` and `aarch64` (including M1 Macs
running an Arch Linux ARM/Omarchy environment):

```sh
curl -fLO "https://github.com/iconidentify/chonkstep/releases/download/preview-v0.4.4/chonkstep-0.4.4-1-$(uname -m).pkg.tar.zst"
sudo pacman -U "./chonkstep-0.4.4-1-$(uname -m).pkg.tar.zst"
...
- **Traditional window management.** Windows float by default and can be
  ...
  moved between workspaces. Workspaces also support Mosaic and Flow;
...
- **Three workspace styles.** Freeform lets you place
...
curl -fL -o "$chonkstep_dir/$chonkstep_pkg" \
  "https://github.com/iconidentify/chonkstep/releases/download/preview-v0.4.4/$chonkstep_pkg"
curl -fL -o "$chonkstep_dir/SHA256SUMS" \
  "https://github.com/iconidentify/chonkstep/releases/download/preview-v0.4.4/SHA256SUMS"
